// pages/api/generateFlower.js
// 꽃배달 생성 핸들러. 복사 베이스: generateDaycare.js
// 기능 모듈(cleanText/removeDuplicates/insertInfoBlock/QC)은 업종 공통 — 복사 사용.
// 비의료·비제도 → injectExamValue(수치 강제) no-op 유지.
...
import { FLOWER_PLAY_CONFIG } from "../../lib/flower-playConfig.js";
import {
  FLOWER_TREATMENTS,
  FLOWER_PHOTO_POOL, // ⚠️ DEAD CODE (핸들러 미참조 — getImageAlts가 내부 사용): export 보존
} from "../../lib/flower-data.js";
import {
  SYSTEM_PROMPT,
  FORBIDDEN,
  buildPrompt,
...
function cleanText(text, fullKeyword) {
  let t = text;
  // [fix] GPT가 프롬프트의 매장명 placeholder를 본문에 복사하는 변동성(40%) 코드레벨 차단.
  //   폴백 수정(A)과 2단 방어 — 프롬프트가 새도 여기서 100% 제거. 매장명 본문 미노출(PHILOSOPHY 원칙1).
  t = t.replace(/\{storeName\}/g, "저희 매장");
...
// ★ 비의료·비제도 → 수치 강제 삽입 생략 (no-op 유지)
function injectExamValue(text) {
  // 꽃배달은 의료/제도 수치 없음. 가격은 단정 금지 → 정보블럭이 구조만 담당.
  return text; // no-op
}
...
export default async function handleFlower(req, res) {
  try {
    ...
    // [fix] 폴백을 "{storeName}" placeholder → "저희 매장" 안전표현.
    //   발행코치/단일호출이 storeName 미전달 시 buildPrompt에 placeholder가 들어가
    //   본문 노출되던 결함 차단(QA 5건 중 축하·개업 2건 재현). PHILOSOPHY 원칙1(매장명 본문 미노출) 정합.
    const storeName = (bodyStoreName || "저희 매장").trim();
    ...
    // 상품 매칭 (daycare 동형: program.id → name)
    const treatment =
      FLOWER_TREATMENTS.find((t) => t.id === program?.id) ||
      FLOWER_TREATMENTS.find((t) => t.name === program?.name) ||
      FLOWER_TREATMENTS[0];

    const fullKeyword = `${region} 꽃배달`;
    const systemPrompt = SYSTEM_PROMPT.replace(/\{region\}/g, region);
    const userPrompt = buildPrompt({ treatment, region, storeName });

    // ── GPT 호출 (단일 호출 — flower-prompts 설계 그대로. 섹션 루프 미이식) ──
    ...
    // 마무리 해시태그
    out += `\n\n#${region}꽃배달 #${region}꽃집 #근조화환 #개업화분 #꽃다발`;
    ...
    console.error("[generateFlower] error:", e);
    return res.status(500).json({ error: "flower_generation_failed", detail: String(e) });
  }
}

function pickTitle(treatment, region) {
  const pats = treatment.titlePatterns || [];
  const t = pats[Math.floor(Math.random() * pats.length)] || "{region} 꽃배달 안내";
  // {region} 외 placeholder({funeralName}/{bizType})는 지역으로 폴백 (단일호출 입력 미수신)
  return t
    .replace(/\{region\}/g, region)
    .replace(/\{funeralName\}/g, region)
    .replace(/\{bizType\}/g, "개업")
    .replace(/\{storeName\}/g, "");
}
