// ============================================================
// pages/api/generateCoating.js — 탄성코트(coating) 엔진 핸들러
// 정보형 + 시공선택 가이드형({region} 탄성코트 업체 1인칭).
// 후기형·내돈내산·만족도·추천·보장·효과단정·최저가·순위·브랜드 강추 금지.
// engineBootstrap에서 register("coating", handleCoating)로 편입.
...
import {
  ...
  COATING_INFO_BLOCKS,
  APT_DATA,
} from "../../lib/coating-data";
import {
  buildSystemPrompt,
  buildUserPrompt,
  getImageAlts,
  FORBIDDEN,
} from "../../lib/coating-prompts";
import { COATING_FLOW } from "../../lib/coating-playConfig";
...
//       "순위"→"우선순위" / "최고"→"최고령" / "반드시"→문장 파괴. 효과단정("완전 차단" 등)은 HARD 유지.
//   프롬프트 [금지] 목록에는 그대로 남아 있어 억제력은 유지된다.
const STRIP_SKIP = new Set(["순위", "최고", "완벽", "1위", "반드시"]);
...
function buildHashtags(region, kw, aptName) {
  ...
  // ★ [세션60] aptName 없을 때 `#${rg}탄성코트` 가 2회 생성되던 중복 제거 + 지역+메뉴 결합 태그 추가.
...
// ── 화자 도입 (탄성코트 업체 1인칭. 업체명 본문 미노출) ──
function buildAgentIntro(region, aptName) {
  if (aptName) {
    return `안녕하세요. ${region} 탄성코트 업체입니다.\n오늘은 ${aptName} 탄성코트 시공을 안내해 드리겠습니다.`;
  }
  return `안녕하세요. ${region} 탄성코트 업체입니다.\n오늘은 ${region} 탄성코트 시공을 안내해 드리겠습니다.`;
}

// ── 정보블럭 선택 (cat/treatment 기준) ──────────
function pickInfoBlock(treatment) {
  const cat = treatment.cat;
  if (cat === "베란다코트") return COATING_INFO_BLOCKS.veranda;
  if (cat === "결로방지") return COATING_INFO_BLOCKS.condense;
  if (cat === "곰팡이예방") return COATING_INFO_BLOCKS.mold;
  if (cat === "구축아파트") return COATING_INFO_BLOCKS.oldapt;
  if (cat === "보수") return COATING_INFO_BLOCKS.fix;
  if (cat === "재시공") return COATING_INFO_BLOCKS.redo;
  if (cat === "종류비교") return COATING_INFO_BLOCKS.compare;
  if (cat === "시공범위") return COATING_INFO_BLOCKS.scope;
  return COATING_INFO_BLOCKS.veranda;
}
...
function buildTitle(region, treatment, aptName, aptMeta) {
  const _t = buildIntentTitleOrNull(region, treatment, "coating");
  if (_t) return _t;
...
export default async function handleCoating(req, res) {
  try {
    ...
    // 탄성코트 = 출장/현장방문 업종. 발행코치에서 위치 5필드 채워 보내면 「찾아오시는 길」 삽입,
    ...
    const treatment =
      COATING_TREATMENTS.find((t) => t.id === program?.id) ||
      COATING_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `탄성코트 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    const PHOTO_ALT = {
      intro: "탄성코트 시공 범위 안내", axis1: "시공 범위 안내", axis2: "관리 방법 안내",
      axis3: "진행 순서 안내", axis4: "시공 전 확인 안내", closing: "탄성코트 시공 상담 안내",
    };

    const writtenSections = new Set();
    const sections = [];

    for (const sec of COATING_FLOW) {
      ...
      body = body.replace(/(^|\n)\s*운영자\s*(\n|$)/g, "\n").trim();
      // closing 섹션이 화자 인사("안녕하세요 …탄성코트 업체입니다")를 재등장시키는 패턴 제거(intro 중복)
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*탄성코트\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }
      body += "\n\n[이미지: " + (PHOTO_ALT[sec.key] || "탄성코트 시공 상담 안내") + "]";
      sections.push(body);
    }
    ...
    console.log(`[QC][coating] 메뉴(${kw}) aptName(${aptName || "-"}): kw ${kwCount}`);
    console.log(`[QC][coating] 글자수: ${content.length}`);
    console.log(`[QC][coating] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[coating] 오류:", err);
    return res.status(500).json({ error: err.message || "탄성코트 글 생성 오류" });
  }
}
