// pages/api/generateWelfarecare.js
// 복지용구 사업소 생성 핸들러. 복사 베이스: generateDaycare.js (단일호출형)
// 기능 모듈(cleanText/removeDuplicates/insertInfoBlock/QC)은 업종 공통 — 복사 사용.
// ★ flower storeName 결함(placeholder 본문 노출 40%) 2단 방어 처음부터 내장:
//    A) 폴백 = "저희 사업소" (placeholder 문자열 아님)
//    B) cleanText에 {storeName} → "저희 사업소" 치환 (GPT가 새도 100% 제거)
...
import { WELFARECARE_PLAY_CONFIG } from "../../lib/welfarecare-playConfig.js";
import {
  WELFARECARE_TREATMENTS,
  WELFARECARE_PHOTO_POOL, // ⚠️ DEAD CODE (핸들러 미참조 — getImageAlts가 내부 사용): export 보존
} from "../../lib/welfarecare-data.js";
import {
  SYSTEM_PROMPT,
  FORBIDDEN,
  buildPrompt,
  ...
  getImageAlts,
  renderInfoBlocks,
} from "../../lib/welfarecare-prompts.js";
...
function cleanText(text, fullKeyword) {
  ...
  t = t.replace(/\{storeName\}/g, "저희 사업소");
  // ★ [방어 D] 화자 이중출현·조사오류 안전망 (storeName 도입부 누수 회귀 차단)
  //   "노원구 묵동 저희 사업소 복지용구 사업소입니다" → "노원구 묵동 복지용구 사업소입니다"
  t = t.replace(/저희\s*사업소\s*복지용구\s*사업소/g, "복지용구 사업소");
  //   "○○ 복지용구 사업소 복지용구 사업소" 류 사업소 중복 1회로
  t = t.replace(/복지용구\s*사업소\s*복지용구\s*사업소/g, "복지용구 사업소");
  //   "사업소으로"(받침 없는데 으로) → "사업소로"
  t = t.replace(/사업소으로/g, "사업소로");
...
function injectExamValue(text) {
  // 복지용구는 의료 수치 없음. 제도 수치(등급/한도/부담률)는 정보블럭이 담당.
...
function runQC(text, fullKeyword) {
  const hasInfo = /이용대상|이용절차|본인부담금|한도|선택 기준/.test(text);
  const kwCount = fullKeyword
    ? (text.match(new RegExp(fullKeyword.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []).length
    : 0;
  const fullKwCount = (text.match(/복지용구 사업소|복지용구/g) || []).length;
  ...
  // ★ [방어 C 보강] 폴백값 화자 이중출현 누수 (도입부 "저희 사업소 복지용구 사업소" / "사업소으로")
  const speakerLeak =
    (text.match(/저희\s*사업소\s*복지용구\s*사업소/g) || []).length +
    (text.match(/복지용구\s*사업소\s*복지용구\s*사업소/g) || []).length +
    (text.match(/사업소으로/g) || []).length;
...
// ───────── 핸들러 ─────────
export default async function handleWelfarecare(req, res) {
  try {
    // [배선] 입력은 req.body (라우터 handle(req,res) 경로 — daycare/lawyer 동형)
    const {
      program, userRegion, region: regionFallback,
      storeName: bodyStoreName,
      // [A-7] 위치 5필드 — 발행코치=값 존재 / 일반글쓰기=빈값.
    ...
    //    storeName 미전달 시 "{storeName}"이 GPT로 전달되어 본문 복사되던 결함 차단.
    const storeName = (bodyStoreName || "저희 사업소").trim();
    const _locStore = { address, map_guide, transit, building_desc, parking_info };

    // 품목 매칭 (daycare 동형: program.id → name)
    const treatment =
      WELFARECARE_TREATMENTS.find((t) => t.id === program?.id) ||
      WELFARECARE_TREATMENTS.find((t) => t.name === program?.name) ||
      WELFARECARE_TREATMENTS[0];

    const fullKeyword = `${region} 복지용구`;
    const systemPrompt = SYSTEM_PROMPT.replace(/\{region\}/g, region);
    const userPrompt = buildPrompt({ treatment, region, storeName });

    // ── GPT 호출 (단일 호출 — welfarecare-prompts 설계 그대로) ──
    ...
    // 마무리 해시태그
    out += `\n\n#${region}복지용구 #복지용구사업소 #장기요양 #복지용구급여 #어르신복지용구`;
    ...
    console.error("[generateWelfarecare] error:", e);
    return res.status(500).json({ error: "welfarecare_generation_failed", detail: String(e) });
  }
}

function pickTitle(treatment, region) {
  const pats = treatment.titlePatterns || [];
  const t = pats[Math.floor(Math.random() * pats.length)] || "{region} 복지용구 사업소 안내";
