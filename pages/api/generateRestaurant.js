...
import {
  RESTAURANT_TREATMENTS, RESTAURANT_BLOCK_MAP,
  RESTAURANT_TITLE_MIDDLE, RESTAURANT_TITLE_SUFFIX,
  RESTAURANT_TITLE_SCENE, RESTAURANT_TITLE_SCENE_BY_CATEGORY,
...
const REST_FORBIDDEN_COMMERCIAL = [
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
// ============================================================
const REST_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("restaurant");
  } catch (e) {
    return [
      ...RESTAURANT_BLOCK_MAP.medical,
      ...RESTAURANT_BLOCK_MAP.cafe,
      ...RESTAURANT_BLOCK_MAP.study,
      ...RESTAURANT_BLOCK_MAP.ad,
    ];
  }
})();
...
let _lastTitleSuffix = "";

function buildRestaurantTitle(treatment, region, situation, purpose, seoData, mode) {
  ...
  if (mode === "commercial") {
    // ★ 제목 다양성 (FREEZE 확장): {region} {menu} {MIDDLE|SCENE}｜{SUFFIX}
    //   region+menu 선두 고정(PHILOSOPHY 원칙1). data가 풀 소유 / generator는 선택+조립만.
    //   3단 폴백: TITLE_SCENE[menu] → TITLE_SCENE_BY_CATEGORY[cat] → TITLE_MIDDLE.
    //   직전 MIDDLE/SUFFIX 각각 회피 → 연속 발행 시 반복 인상 감소.
    ...
    // ★ [v3 제목 배선 복구] 방문목적 선두 titlePatterns 우선 사용.
    //   기존 v3 설계(data.js titlePatterns 75:25)가 commercial에서 미소비되던 누락을 연결.
    //   {purpose}=방문목적 라벨(purpose 선택값 → 메뉴 base.titlePurpose 폴백)
    //   {searchword}=정보형 접미(위에서 고른 mid/suf 재활용 — data 풀 소유 유지)
    ...
    if (seoData?.titlePatterns?.length) {
      ...
      let t = raw
        ...
        .replace(/\{searchword\}/g, mid);   // 정보형 접미(MIDDLE) — 토큰 누락 복구
      // 빈 placeholder 자국·중복 공백·앞쪽 공백 정리
      t = t.replace(/\s+\|\s*$/g, "")
           .replace(/\s+｜\s*$/g, "")
           .replace(/\|\s+\|/g, "|")
           .replace(/｜\s+｜/g, "｜")
...
// ============================================================
function buildRestaurantHashtags(treatment, region, situation, purpose, mode) {
...
// ============================================================
// [세션51] 사전식/메뉴주어 시작 감지 (commercial 하드 강제)
//   실측 근거: 시스템 프롬프트가 "메뉴 설명 금지"를 강제해도 GPT가 첫 문장을
//   "짜장면은 ~다 / 대표적인 중식 메뉴 / 춘장을 볶아" 로 여는 관성이 잔존.
//   → 첫 문장이 '사람·상황'이 아니라 '메뉴·재료' 주어면 재생성 트리거.
//   반환 true = 사전식 시작(재생성 필요). commercial 전용.
function isDictionaryOpening(text, menu) {
  ...
  // 1) 메뉴명으로 시작 ("짜장면은/짜장면 한 그릇은/짜장면이")
  if (m && new RegExp(`^${m.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`).test(first)) return true;
  // 2) 지역+메뉴 소유격 시작 ("~의 짜장면은")
  if (m && new RegExp(`${m.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(은|는|이|가)`).test(first)
      && /^[가-힣\s]{0,12}(의|인근|근처)/.test(first)) return true;
  // 3) 사전식 정의 상투어
  const dictPatterns = [
    /대표적인.{0,6}(메뉴|요리|음식)/,
    /(중식|한식|일식|분식|양식).{0,4}(대표|메뉴|요리)/,
    /^(이|그)\s*(메뉴|음식|한\s*그릇)(은|는|이|가)/,
    /^매장에서(는)?\s*(흔히|주로|보통)/,
    /(으로|로)\s*만든.{0,6}(소스|음식|요리)/,
    /(춘장|양파|단무지|고기|채소|면발|국물)(을|를|은|는|이|가).{0,8}(볶|넣|들어|올|썰)/,
  ];
  if (dictPatterns.some((re) => re.test(first))) return true;
  return false;
}

// ============================================================
// [STEP6-FIX-B B-3 / C1] 대체어 뒤 조사 받침 재정합
//   현상: "돼지국밥 메뉴로"/"이 메뉴로" → "이 한 그릇" 치환 후 원문 조사 "로" 잔존 → "이 한 그릇로".
//   원칙: 치환 시 뒤 조사를 함께 캡처해 대체어 받침에 맞춰 재정합(조사 보존). 목록 밖·경계 불일치면 현행 유지.
// ============================================================
const _JOSA_TAIL_RE = "(?:(으로부터|로부터|으로는|으로도|으로만|으로서|으로써|이라도|이라면|이랑|이면|이나|이라|으로|로는|로도|로만|로서|로써|라도|라면|과|와|은|는|이|가|을|를|로|나|랑|면|라)(?=[\\s,.!?\"')\\]]|$))?";
const _JOSA_PAIR = {   // 받침無형 → 받침有형
  "로": "으로", "로는": "으로는", "로도": "으로도", "로만": "으로만", "로서": "으로서", "로써": "으로써", "로부터": "으로부터",
  "는": "은", "가": "이", "를": "을", "와": "과", "랑": "이랑", "면": "이면", "나": "이나", "라": "이라", "라도": "이라도", "라면": "이라면",
};
const _JOSA_PAIR_INV = Object.fromEntries(Object.entries(_JOSA_PAIR).map(([k, v]) => [v, k]));
const _RO_FAMILY = new Set(["로", "로는", "로도", "로만", "로서", "로써", "로부터"]);
function remapJosa(tail, altWord) {
  if (!tail) return "";
  const c = altWord.charCodeAt(altWord.length - 1);
  if (c < 0xac00 || c > 0xd7a3) return tail;
  const jong = (c - 0xac00) % 28;
  if (jong === 0) return _JOSA_PAIR_INV[tail] || tail;              // 받침無 → 받침無형
  const up = _JOSA_PAIR[tail] || tail;                               // 받침有 → 받침有형
  if (jong === 8) {                                                  // ㄹ받침: 로 계열은 받침無형 유지
    const down = _JOSA_PAIR_INV[up];
    if (down && _RO_FAMILY.has(down)) return down;
  }
  return up;
}

function cleanRestaurantText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  const direction = getRestaurantDirection(treatment, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  let result = text;

  const removeList = [...REST_FORBIDDEN_BASE, ...REST_FORBIDDEN_AI, ...REST_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...REST_FORBIDDEN_COMMERCIAL);
  ...
  if (menu) {
    ...
    // [STEP6-FIX-B B-3 / C1] 뒤 조사 캡처(_JOSA_TAIL_RE) → 대체어 받침에 맞춰 재정합. 대체어 단위 = servingUnit.
    const reA = new RegExp(`${menuEsc}(?:이|가|을|를|은|는|의|도|에)\\s+메뉴(?!판)${_JOSA_TAIL_RE}`, "g");
  ...
  // ─────────────────────────────────────────────────────
  // ★ [세션63] "이 메뉴"+조사 비문 차단 (V4 프롬프트 회귀 대응)
  //   현상: GPT가 ${menu}를 "이 메뉴"로 바꿔 쓴 뒤 조사 결합 → 이중조사·비문
  //         "이 메뉴는씩", "이 메뉴는으로", "이 메뉴으로", "이 메뉴이면"
  //   교정: "이 메뉴"(+조사)? → "이 한 그릇" + 받침 정규화 조사 (릇=받침有)
  //   주의: "메뉴판" 보존 / 받침 정규화로 "그릇는·그릇가" 재오류 방지
  // ─────────────────────────────────────────────────────
  {
    // [STEP6-FIX-B B-3 / C1] 대체어 단위 = servingUnit (접시 메뉴는 "이 한 접시" — 받침 판정 자동 추종)
    const ALT = `이 ${direction.servingUnit || "한 그릇"}`;
    const lastCh = ALT.charCodeAt(ALT.length - 1);
    const altBatchim = (lastCh - 0xac00) % 28 !== 0;   // 릇 → true
    const _topic = altBatchim ? "은" : "는";
    const _subj  = altBatchim ? "이" : "가";
    ...
    result = result.replace(new RegExp(_nb + /이\s*메뉴(는|은|이|가|을|를|의|도|에|와|과)/.source, "g"), (m, j, offset, str) => {
      ...
      if (j === "와" || j === "과") return ALT + (altBatchim ? "과" : "와");
  ...
  // [STEP6-FIX-B B-3 / C1] 단위 명사(그릇=받침有 / 접시=받침無) 조사 재정합 — 고정 명사 한정 말단 안전망
  result = result.replace(/그릇(로부터|로는|로도|로만|로서|로써|로|랑|와|가|면)(?=[\s,.!?]|$)/g, (m, j) => "그릇" + remapJosa(j, "그릇"));
  result = result.replace(/접시(으로부터|으로는|으로도|으로만|으로서|으로써|으로|이랑|과|이면)(?=[\s,.!?]|$)/g, (m, j) => "접시" + remapJosa(j, "접시"));
  ...
  if (particleErrorCount > 0) {
    console.log(`[restaurant][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[restaurant][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[restaurant][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[restaurant][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (particleDupCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[restaurant][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }

  // 외부 QC 집계용
  cleanRestaurantText.__lastParticleDup = particleDupCount;
  cleanRestaurantText.__lastSubjParticleFix = subjParticleFixCount;
  cleanRestaurantText.__lastEfficacyFix = efficacyFixCount;
  cleanRestaurantText.__lastStoreRefFix = storeRefFixCount;
  cleanRestaurantText.__lastMenuDup = menuDupCount;
  cleanRestaurantText.__lastSuperlativeFix = superlativeFixCount;
  cleanRestaurantText.__lastUnitFix = unitFixCount;
  cleanRestaurantText.__lastDupSentence = dupSentenceCount;
  cleanRestaurantText.__lastParticleErrors = particleErrorCount;
  cleanRestaurantText.__lastMenuMeta = menuMetaCount;
  cleanRestaurantText.__lastAdEvalFixed = adEvalFixed;
  cleanRestaurantText.__lastPlaceholderFix = placeholderFixCount;
...
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  if (mode === "commercial") {
    ...
    for (const sn of sents) {
      ...
      else if (/(?:다시|다음(?:번)?에도?)\s*.{0,12}?(?:찾|가|오|방문하)(?:고\s*싶|게\s*될)/.test(sn)) expLeakCount++;
    }
    josaUnitLeakCount = (bodyOnly.match(/그릇(?:로부터|로는|로도|로만|로|는|가|를|와|랑|면)(?=[\s,.!?]|$)|접시(?:으로|으로도|으로는|은|이|을|과|이랑|이면)(?=[\s,.!?]|$)/g) || []).length;
  }
  ...
  //   누적 = cleanRestaurantText가 치환한 총 횟수
  // ─────────────────────────────────────────────────────
  ...
  const adEvalFixed = (typeof cleanRestaurantText.__lastAdEvalFixed === "number")
    ? cleanRestaurantText.__lastAdEvalFixed : 0;

  // cleanRestaurantText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanRestaurantText.__lastParticleErrors === "number")
    ? cleanRestaurantText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanRestaurantText.__lastMenuMeta === "number")
    ? cleanRestaurantText.__lastMenuMeta : 0;
  ...
  console.log(`[restaurant][QC] 메뉴블럭: ${hasMealBlock}`);
  if (mode === "commercial") {
    // [STEP6-FIX-B B-3 §8] commercial은 매장정보 언급을 권장하지 않는다(확인형만 허용, 단정은 storeClaim·D3로 판정)
    console.log(`[restaurant][QC] 매장정보 언급: ${mealValueCount}/9 (commercial 권장치 없음 — 확인형만 허용)`);
  } else {
    console.log(`[restaurant][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  }
  console.log(`[restaurant][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[restaurant][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[restaurant][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[restaurant][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[restaurant][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[restaurant][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[restaurant][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[restaurant][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[restaurant][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[restaurant][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[restaurant][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
...
// ============================================================
// ============================================================
// [STEP6-FIX-B B-1] 매장 사실 단정 안전망 — 최후 안전망 (D3 재정의)
//   원칙: 업체가 입력하지 않은 정보는 특정 매장의 사실로 서술하지 않는다.
//   - 판정 단위: 문장 → 절. 단정이 매치된 "그 절"에 확인 톤이 있을 때만 면제.
//     (FIX-A는 문장 어디든 확인 톤이 있으면 전체 면제 → 절 안의 단정 통과)
//   - 제거 시 rule·원문을 로그로 남긴다 (DB 미저장).
//   - 규칙은 기존 6개 + 구조 규칙 2개(STORE_SUBJ·SVC)로 한정. 단어 나열 확장 금지.
//   - 발동 건수는 프롬프트 실패 지표. 제거 후 본문 청결을 품질 근거로 쓰지 않는다.
//   범위: 제목(첫 줄) 제외 본문. 해시태그·LocationBlock은 이후 부착(D5) → 무접촉.
// ============================================================
const FACT_GUARD_RULES = [
  { id: "PARK",    re: /주차\s*(?:공간|장|시설)?(?:[이가은는도]|까지)?(?:\s*(?:또한|역시))?\s*(?:마련|여유|넉넉|편리|편해|편하|갖춰|완비|가능)/ },
  { id: "TAKEOUT", re: /포장(?:\s*서비스)?(?:[이가은는도를]|까지)?\s*(?:가능|제공|된다|됩니다|되니|되어|돼)/ },
  { id: "SEAT",    re: /(?:좌석|자리|테이블|단체석|룸|혼밥석|1인석)(?:[이가은는도]|까지|부터)?\s*(?:마련|준비|넉넉|여유|넓게|갖춰|배치)/ },
  { id: "HOURS",   re: /아침\s*(?:일찍)?부터\s*늦은|아침\s*일찍부터\s*문을|새벽까지\s*.{0,12}(?:운영|영업|즐길|먹을|방문|찾을|열)|늦은\s*(?:시간|밤|저녁)까지\s*(?:운영|영업|문|방문)|24시간\s*(?:운영|영업)|연중무휴/ },
  { id: "REFILL",  re: /(?:공깃밥|사리|반찬)\s*(?:추가|리필)(?:[이가은는도])?\s*(?:가능|무료|됩니다)/ },
  { id: "PRICE",   re: /부담\s*없는\s*가격대/ },
  // [B-1 구조 규칙] 매장 지칭 주어 + 존재·제공·운영·대표 서술
...
function applyFactGuard(assembled, providedChecks = [], mode = "personal") {
  ...
  const head = nlIdx >= 0 ? assembled.slice(0, nlIdx) : assembled;
  const body = nlIdx >= 0 ? assembled.slice(nlIdx) : "";
  ...
  const outLines = body.split("\n").map((line) => {
    ...
    const kept = sents.filter((sn) => {
      ...
      if (!rule) return true;
      count++;
      removed.push({ rule, text: sn.slice(0, 120) });
      console.warn(`[restaurant][QC][D3] #${count} rule=${rule} "${sn.slice(0, 120)}"`);
      return false;
    });
  ...
  if (count > 0) {
    if (!/방문 전(?:에)?\s*확인/.test(guarded)) {
      ...
      console.warn(`[restaurant][QC][D3] appendSkipped=true (기존 '방문 전 확인' 문구 존재 → 안내 미부착)`);
    }
    console.warn(`[restaurant][QC] ⚠️ 사실 단정 안전망 발동 ${count}건 — 프롬프트 실패 신호`);
  }
  return { text: (head + guarded).replace(/\n{3,}/g, "\n\n"), count, removed };
}

export default async function handleRestaurant(req, res) {
  const {
    ...
    mode = "commercial",
    // [VISIT PILOT] 방문정보 활용 파일럿 게이트 — 인수인계 세션39 PHILOSOPHY.
    //   프론트: window.__RESTAURANT_VISIT_PILOT → 페이로드 visitPilot:true + visitInfo{}.
    //   OFF(미전달/false) 시 기존 경로 100% 무변경(부작용 0). FREEZE 유지·즉시 롤백 가능.
    //   locationBlock(주소/주차/교통)은 별도 후단 처리 → visitInfo에서 제외.
    visitPilot = false,
    visitInfo  = null,
  } = req.body;
  ...
  const isRest = REST_IDS.includes(program.id) || program.industry === "restaurant";
  if (!isRest) {
    console.error(`[restaurant] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  const treatmentData = RESTAURANT_TREATMENTS.find(t => t.id === program.id)
    || RESTAURANT_TREATMENTS[0];
  const cat  = treatmentData?.cat || "한식";
  ...
  console.log(`[restaurant] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getRestaurantDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[restaurant] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getRestaurantSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";
  let prevDigest  = "";   // [STEP6-FIX-B B-3 §5] commercial: 섹션별 여는 표현만 전달(본문 사실 전파 차단)

  for (const sec of SECTIONS) {
    const richPrompt = buildRestaurantPrompt(sec.key, treatmentData, region, {
    ...
    const prevBlock = validMode === "commercial"
      ? (prevDigest
          ? `\n[앞 섹션의 여는 표현 — 반복 금지]\n${prevDigest}※ 앞 섹션 내용은 표현 반복 방지용일 뿐 사실 근거가 아니다. 앞 내용의 매장 관련 서술을 재사용하지 말 것.\n[끝]\n`
          : "")
      : (prevTextRaw
...
${richPrompt}`;
    ...
    // [세션51] commercial 하드 강제 — 사전식/메뉴주어 시작이면 첫 문장 강제 재생성
    ...
    if (validMode === "commercial" && isDictionaryOpening(secText, menu)) {
      ...
      if (!isDictionaryOpening(fixed, menu) && calcCharCount(fixed) >= 100) {
        secText = fixed;
        console.log(`[restaurant] ${sec.label}: ✅ 첫 문장 교정 채택`);
      } else {
        console.log(`[restaurant] ${sec.label}: 교정 실패 — 원문 유지(추가 재생성 안 함)`);
      }
    }

    console.log(`[restaurant] ${sec.label}: ${calcCharCount(secText)}자`);
  ...
  const _REST_ALT_BY_KEY = {
    ...
    scene:   "장면 사진",
    revisit: "마무리 사진",
    // commercial 8섹션 [V2 세션52 — 라벨 정합, 로직 무변경]
    menuIntro:         "방문 상황 사진",
    menuScene:         "주문 고민 장면",
    menuComposition:   "메뉴 비교 사진",
    tasteFeature:      "대표 조합 상차림",
    pairing:           "곁들임·사이드 사진",
    decision:          "선택 참고 사진",
  ...
  const altList = SECTIONS.slice(0, _altCount).map(sec => {
    const label = _REST_ALT_BY_KEY[sec.key] || "메뉴 사진";
    return `[이미지: ${label}]`;
  });
  console.log(`[restaurant][ALT] mode=${validMode} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  let title = overrideTitle || buildRestaurantTitle(treatmentData, region, situation, purpose, seoData, validMode);
  // 의료·카페 어휘가 제목에 새면 차단
  ...
  if (sectionTexts[lastKey] && validMode === "commercial") {
    // commercial CTA — InfoBlock Spine 이관 (문구·랜덤 동일)
    sectionTexts[lastKey] = sectionTexts[lastKey].trimEnd()
      + renderInfoBlock("restaurant", "commercial", { region, menu });
  }
  ...
  //         제목은 buildRestaurantTitle 단계에서 이미 menu 포함 보장됨
  {
    const nlIdx = assembled.indexOf("\n");
    const titleLine = nlIdx >= 0 ? assembled.slice(0, nlIdx) : assembled;
    const bodyPart  = nlIdx >= 0 ? assembled.slice(nlIdx) : "";
    let cleanedBody = cleanRestaurantText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanRestaurantText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  if (qc.adPatternCount > 0)
    console.error(`[restaurant] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[restaurant] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[restaurant] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[restaurant] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[restaurant] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[restaurant] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[restaurant] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
  ...
  return res.status(200).json({
    ...
    qc: {
      ...
      factGuardRemoved, // [STEP6-FIX-B B-1] 제거 문장 {rule, text} — 응답 전용(DB 미저장)
      storeClaimCount:   qc.storeClaimCount,     // [STEP6-FIX-B B-3] 0 필수
      expLeakCount:      qc.expLeakCount,        // [STEP6-FIX-B B-3] 0 필수
      josaUnitLeakCount: qc.josaUnitLeakCount,   // [STEP6-FIX-B B-3] 0 필수
    },
