// ============================================================
// pages/api/generateDemolition.js — 철거공사(demolition) 엔진 핸들러
// 정보형 + 출장 해체 안내형({region} 철거 공사 업체 1인칭).
// 후기형·내돈내산·고객사례·추천·보장·최저가·비용유도·수명약속 금지.
// engineBootstrap에서 register("demolition", handleDemolition)로 편입.
//
// 복제 베이스: generateTile.js (섹션루프형). Runtime/FLOW/Handler 구조 무변경.
// tile 대비 변경 1점:
//   ① 해시태그 어휘 — 철거 검색축(철거/공사/해체)으로 교체. dedup 는 동형 Set 유지.
//   판단 축(symptom=발견 / part=원인군)은 동일 계약.
//
// 출장 해체 업종 → 고정 사업장 「찾아오시는 길」 미노출.
//   SOP PATCH-07: 위치 5필드 빈값이면 locationBlock 미삽입(부작용 0).
// ============================================================

import OpenAI from "openai";
import {
  DEMOLITION_TREATMENTS,
  DEMOLITION_INFO_BLOCKS,
  getInfoBlocks,
  formatSymptom,
  getPartNote,
...
import {
  ...
  getSectionAlt,
  FORBIDDEN,
} from "../../lib/demolition-prompts";
import { DEMOLITION_FLOW, DEMOLITION_SECTION_PHOTO } from "../../lib/demolition-playConfig";
...
// ── 마무리 해시태그 ─────────────────────────────
//   ★ 메뉴명이 "철거"로 끝나 `#{rg}{k}` 와 `#{rg}철거공사` 가 겹칠 수 있다 → Set 일괄 정리.
...
//   → 메뉴 확장은 demolition-data.js 단독 작업. 핸들러를 다시 열지 않는다.
function pickInfoBlock(t) {
  const B = (typeof getInfoBlocks === "function" ? getInfoBlocks(t.cat) : null)
    || DEMOLITION_INFO_BLOCKS;
  return B[t.infoKey] || B[t.cat] || DEMOLITION_INFO_BLOCKS.prebook;
}
...
//   ★ demolition 은 siteBlock 미사용(useSite:false) → 단지명 분기 없음.
...
function buildTitle(region, treatment) {
  const _t = buildIntentTitleOrNull(region, treatment, "demolition");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment);
}

export default async function handleDemolition(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
      symptom,                                   // 접수된 발견(속 빈 소리·매립 배선 등) — 미입력 시 무영향
      part,                                      // 원인군(structure·utility·ceiling·restore·carry)
    } = req.body;

    // 철거공사 = 출장 해체 업종 → 고정 사업장 위치블록 미노출(PATCH-07)
    const _locStore = {};
    ...
    const treatment =
      DEMOLITION_TREATMENTS.find((t) => t.id === program?.id) ||
      DEMOLITION_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `철거공사 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    for (const sec of DEMOLITION_FLOW) {
      ...
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*철거\s*공사\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }

      const slot = DEMOLITION_SECTION_PHOTO[sec.key];
    ...
    const rgKwCount = region
      ...
      : 0;
    console.log(`[QC][demolition] 메뉴(${kw}): kw ${kwCount} / 지역+kw 결합 ${rgKwCount} (3 이하 정상)`);
    console.log(`[QC][demolition] 글자수: ${content.length}`);
    console.log(`[QC][demolition] 발견: ${sym || "미입력"} / 원인군: ${pt ? pt.label : "미입력"}`);
    console.log(`[QC][demolition] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[demolition] 오류:", err);
    return res.status(500).json({ error: err.message || "철거공사 글 생성 오류" });
  }
}
