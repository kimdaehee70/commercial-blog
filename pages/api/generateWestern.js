// ============================================================
// generateWestern.js — 양식(레스토랑) 블로그 생성기 v1.0 (Western Engine 독립)
//
...
import {
  WESTERN_TREATMENTS, WESTERN_BLOCK_MAP,
  WESTERN_TITLE_MIDDLE, WESTERN_TITLE_SUFFIX,
  WESTERN_TITLE_SCENE, WESTERN_TITLE_SCENE_BY_CATEGORY,
  WESTERN_PURPOSE_TITLE_LABEL, WESTERN_PURPOSE_TITLE_FORMS,
  WESTERN_PURPOSE_TITLE_FORMS_MENU_AUX, getPurposeProfile,
} from "../../lib/western-data";
import {
  buildWesternPrompt, getWesternDirection,
} from "../../lib/western-prompts";
import {
  getWesternSections,
} from "../../lib/western-playConfig";
import {
...
// [세션20] Platform Spine — 제목 조립 공통 위임 (data·prompt·본문 무변경. 풀 주입식)
import { buildTitle } from "../../lib/spine/titleEngine";
import { DEFAULT_TITLE_SUFFIX, resolveTitleSuffix } from "../../lib/spine/titleSuffixRegistry";
// [Restaurant Image SoT] 사진 5슬롯 단일 SoT — 슬롯 정의는 registry 소유, 엔진은 삽입만.
import { getRestaurantPhotoLabels } from "../../lib/photoPolicyRegistry";

// ============================================================
// 0. 금지 키워드 (FORBIDDEN) — Phase 9.5 핵심
// ============================================================
const WEST_FORBIDDEN_BASE = [
...
const WEST_FORBIDDEN_COMMERCIAL = [
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
// ============================================================
const WEST_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("western");
  } catch (e) {
    return [
      ...WESTERN_BLOCK_MAP.medical,
      ...WESTERN_BLOCK_MAP.cafe,
      ...WESTERN_BLOCK_MAP.study,
      ...WESTERN_BLOCK_MAP.ad,
    ];
  }
})();
...
// [세션20] Spine 위임 — 연속 발행 회피 상태(_lastTitleMiddle/Suffix 대체)
let _titleState = {};

function buildWesternTitle(treatment, region, situation, purpose, seoData, mode, titleSuffixOpts) {
  const menu = treatment.menu || treatment.menuRef || "";
  const cat  = treatment.cat || "양식";
  const sit  = situation || "";
  const pur  = purpose   || "";

  // ★ [Pilot] purpose 모드 — 목적 우선 제목 (menu 보조 소수). Spine 미경유.
  //   축: `{region} {purposeLabel} 레스토랑{｜menu 보조 변형 소수}`
  //   commercial/personal 제목 경로는 아래 그대로 무변경.
  if (mode === "purpose") {
    const prof = getPurposeProfile(pur);
    const purLabel = (prof && prof.titleLabel)
      || WESTERN_PURPOSE_TITLE_LABEL[pur]
      || pur
      || "방문";
    // ★ [세션47] 노출 기준선 확보 — 메뉴 있으면 항상 menu 보조 노출(관련도 신호 1회).
    //   Purpose 축 유지, 템플릿·prompts·data 무변경. 게이트만 15%→상시.
    const useMenuAux = !!menu;
    const pool = useMenuAux
      ? WESTERN_PURPOSE_TITLE_FORMS_MENU_AUX
      : WESTERN_PURPOSE_TITLE_FORMS;
    ...
    let t = raw
      ...
      .replace(/\s{2,}/g, " ")
      .trim();
    // ★ [세션47] 제목 후처리 정규화 (출력 품질만 — 생성 로직·템플릿 무변경).
    //   ① 연속 중복 어절 축약("식사 식사"→"식사"). ② 전각 구분자 좌우 공백 정리.
    t = t
      .replace(/([^\s｜|]+)(\s+)\1(?=\s|｜|\||$)/g, "$1")   // 인접 동일 어절 1개로
      .replace(/\s*｜\s*/g, "｜")
      .replace(/\s{2,}/g, " ")
      .trim();
    // ★ [세션47] purpose 제목도 상호 suffix 결합 (Spine 미경유 경로라 여기서 직접 적용).
    //   resolveTitleSuffix는 객체 반환 → 필드 추출 조립. enabled+실업체명일 때만 부착.
    const _ps = titleSuffixOpts ? resolveTitleSuffix(titleSuffixOpts) : DEFAULT_TITLE_SUFFIX;
    if (_ps && _ps.enabled && _ps.storeName) t = `${t}${_ps.separator}${_ps.storeName}`;
    return t;
  }

  if (mode === "commercial") {
    // [세션20] Platform Spine 위임.
    //   - commercial 제목은 purpose 미사용 → buildTitle purpose 빈값(폴백 경로).
    //     산출 `${region} ${menu}${mid? " "+mid:""}${suf? "｜"+suf:""}` ≡ 기존 `${region} ${menu} ${mid}｜${suf}`.
    //   - SCENE/MIDDLE/SUFFIX는 data 소유 → 풀 주입. titlePatterns 미주입(commercial 미사용).
    //   - 연속 발행 회피: state 모듈 보관(_titleState).
    const pools = {
      titlePatterns: [],
      SCENE:        WESTERN_TITLE_SCENE,
      SCENE_BY_CAT: WESTERN_TITLE_SCENE_BY_CATEGORY,
      MIDDLE:       WESTERN_TITLE_MIDDLE,
      SUFFIX:       WESTERN_TITLE_SUFFIX,
    };
    const suffix = titleSuffixOpts
      ? resolveTitleSuffix(titleSuffixOpts)
      : DEFAULT_TITLE_SUFFIX;
    const out = buildTitle({
      region, menu, cat, situation: sit, purpose: "",
      mode: "commercial",
      pools,
      purposeTitleLabel: "",
      state: _titleState,
...
// ============================================================
function buildWesternHashtags(treatment, region, situation, purpose, mode) {
  ...
  const catKw = (treatment.cat || "양식").replace(/\s/g, "");
  const sitKw = (situation || "").replace(/\s/g, "");
  const purKw = (purpose || "").replace(/\s/g, "");

  if (mode === "purpose") {
    // ★ [Pilot] 목적 우선 해시태그 (menu 보조)
    const prof = getPurposeProfile(purpose);
    const purLabel = ((prof && prof.titleLabel) || WESTERN_PURPOSE_TITLE_LABEL[purpose] || purKw || "").replace(/\s/g, "");
    return [
      purLabel ? `#${reg}${purLabel}` : "",
      purLabel ? `#${reg}${purLabel}레스토랑` : "",
      `#${reg}레스토랑`, `#${reg}맛집`,
      purLabel ? `#${purLabel}레스토랑` : "",
      `#${reg}${catKw}`, menu ? `#${reg}${menu}` : "",
    ].filter(t => t && t.length > 2).slice(0, 8).join(" ");
  }
...
// ============================================================
function cleanWesternText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  const direction = getWesternDirection(treatment, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  let result = text;

  const removeList = [...WEST_FORBIDDEN_BASE, ...WEST_FORBIDDEN_AI, ...WEST_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...WEST_FORBIDDEN_COMMERCIAL);
  ...
  if (particleErrorCount > 0) {
    console.log(`[western][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[western][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[western][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[western][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (particleDupCount > 0) {
    console.log(`[western][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[western][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[western][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[western][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[western][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[western][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[western][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[western][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }

  // ─────────────────────────────────────────────────────
  // ★ STEP G — 중식 검수(4종 실생성) 기반 후처리 보강
  ...
  //   "짜장면 짜장 소스" → "짜장 소스" / "삼선짜장 조합은 삼선짜장의 …" → "이러한 구성은 …"
  ...
  let repeatCapCount = 0;
  {
    ...
    // "깊은 풍미" 초과분 → "진한 맛" (춘장/국물 문맥 공통)
    result = capPhrase(result, /깊은 풍미/g, () => "진한 맛");
  }

  // ─────────────────────────────────────────────────────
  // ★ STEP G2 — 중식 검수(양장피/팔보채/칠리새우) 잔여 6룰 (G-4 산출물까지 정정 위해 G-4 뒤 배치)
  ...
  let stepG2Count = 0;
  {
    ...
    // G2-c. 단위+조사 깨짐 "한 그릇가" → "한 접시가" (주격조사 '가'는 받침없는 '접시'에 정합)
    result = result
      .replace(/이 한 그릇가/g, "이 한 접시가")
      .replace(/한 그릇가/g, "한 접시가");
    ...
    // G2-e. "전문점의 중식당들은/중식당들은" 군더더기 → "중식당에서는"
    result = result
      .replace(/[가-힣]{2,8}\s*전문점의\s*중식당들은/g, "중식당에서는")
      .replace(/중식당들은(?=\s)/g, "중식당에서는");
    ...
    // G2-i. "중식당 전문점에서는 / OO 전문점에서는(중식당 문맥)" → "중식당에서는"
    result = result
      .replace(/중식당\s*전문점에서는/g, "중식당에서는")
      .replace(/중식\s*전문점에서는/g, "중식당에서는");

    if (before !== result) stepG2Count = 1;
  }
  if (stepG2Count > 0) console.log(`[western][QC] ★ STEP G2 잔여정정: 매장안내참고/이거의/한그릇가/메뉴명조합/전문점중식당들은/가게마다/방문시중복/맛조사/중식당전문점`);

  if (transFixCount > 0)     console.log(`[western][QC] ★ STEP G 번역체 정정: ${transFixCount}건 (그것의/그것을/그것이 → 메뉴)`);
  if (phraseDupCount > 0)    console.log(`[western][QC] ★ STEP G 어절중복 제거: ${phraseDupCount}건 (메뉴명 군더더기 인접반복)`);
  if (generalizeFixCount > 0) console.log(`[western][QC] ★ STEP G 일반화 안전화: ${generalizeFixCount}건 (일부 가게/대부분 → 매장에 따라)`);
  if (repeatCapCount > 0)    console.log(`[western][QC] ★ STEP G 과반복 제한: ${repeatCapCount}건 (3회째+ 축약 · 첫 2회·메뉴명 보존)`);

  // 외부 QC 집계용
  cleanWesternText.__lastParticleDup = particleDupCount;
  cleanWesternText.__lastSubjParticleFix = subjParticleFixCount;
  cleanWesternText.__lastEfficacyFix = efficacyFixCount;
  cleanWesternText.__lastStoreRefFix = storeRefFixCount;
  cleanWesternText.__lastMenuDup = menuDupCount;
  cleanWesternText.__lastSuperlativeFix = superlativeFixCount;
  cleanWesternText.__lastUnitFix = unitFixCount;
  cleanWesternText.__lastDupSentence = dupSentenceCount;
  cleanWesternText.__lastParticleErrors = particleErrorCount;
  cleanWesternText.__lastMenuMeta = menuMetaCount;
  cleanWesternText.__lastAdEvalFixed = adEvalFixed;
  cleanWesternText.__lastPlaceholderFix = placeholderFixCount;
  cleanWesternText.__lastTransFix = transFixCount;
  cleanWesternText.__lastPhraseDup = phraseDupCount;
  cleanWesternText.__lastGeneralizeFix = generalizeFixCount;
  cleanWesternText.__lastRepeatCap = repeatCapCount;
  cleanWesternText.__lastStepG2 = stepG2Count;
...
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  const adEvalFixed = (typeof cleanWesternText.__lastAdEvalFixed === "number")
    ? cleanWesternText.__lastAdEvalFixed : 0;

  // cleanWesternText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanWesternText.__lastParticleErrors === "number")
    ? cleanWesternText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanWesternText.__lastMenuMeta === "number")
    ? cleanWesternText.__lastMenuMeta : 0;
  ...
  console.log(`[western][QC] 메뉴블럭: ${hasMealBlock}`);
  console.log(`[western][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  console.log(`[western][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[western][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[western][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[western][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[western][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[western][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[western][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[western][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[western][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[western][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[western][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
    console.log(`[western][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[western][QC] 가격명시(commercial 위반): ${priceCount}건`);
  }
...
// 메인 핸들러
// ============================================================
export default async function handleWestern(req, res) {
  const {
    ...
    visit_info: bodyVisitInfo,    // ★ [Purpose 품질] 방문정보 — 예약/영업시간/주차/라스트오더 등 자연 편입용
  ...
  const industry  = "western";
  // [v2] personal은 명시적으로 요청된 경우에만. 그 외(미지정·commercial) = commercial.
  // ★ [Pilot] purpose = 목적 중심 엔진(명시 요청 시). commercial/personal 무영향.
  const validMode = (mode === "personal") ? "personal"
  ...
  // ★ [Purpose 품질] 방문정보 — 실입력값만 프롬프트에 태움(가격 숫자 제외 = 광고법·PHILOSOPHY 정합).
  //   빈값이면 조립 결과 "" → 프롬프트 무변화(부작용 0). commercial/personal 무영향(purpose 프롬프트에서만 사용).
  const _vi = (bodyVisitInfo && typeof bodyVisitInfo === "object") ? bodyVisitInfo : {};
  // ★ [세션47] seats 값 내부 단체 문구 서브필터 (배열 조립 전에 실행 → 폴백 경로도 커버)
  const _GROUP_KEEP = new Set(['가족모임', '모임']);
  const _stripGroupFromSeats = (v) => {
    if (!v || _GROUP_KEEP.has(purpose)) return v;
    return String(v)
      .split(/[,·/]/)
      .map(s => s.trim())
      .filter(s => s && !/루프탑|단체|\d+\s*인\s*(?:이상|룸|룸형|단체)|\d+\s*명|넓은\s*홀|홀\s*수용|파티|행사룸/.test(s))
      .join(', ')
      .replace(/(^[\s,]+|[\s,]+$)/g, '');
  };
  if (_vi.seats) _vi.seats = _stripGroupFromSeats(_vi.seats);
  const _visitLines = [
    _vi.reservation && `예약: ${String(_vi.reservation).trim()}`,
    _vi.hours       && `영업시간: ${String(_vi.hours).trim()}`,
    _vi.last_order  && `라스트오더: ${String(_vi.last_order).trim()}`,
    _vi.parking     && `주차: ${String(_vi.parking).trim()}`,
    _vi.seats       && `좌석: ${String(_vi.seats).trim()}`,
    _vi.group       && `단체: ${String(_vi.group).trim()}`,
  ].filter(Boolean);
  // ★ [세션46] Purpose별 방문정보 우선 노출 필터 — 목적과 무관한 정보(예: 데이트에 30명 단체) 억제.
  //   REGISTRY 무변경(스코프 한정). 매핑 테이블만 여기서 관리. 대형 단체(루프탑 30명)는 '모임'에서만 기본 노출.
  //   빈 매핑/미지정 purpose → 전체 통과(기존 동작 보존). commercial/personal 무영향(purpose 프롬프트 전용).
  const _VISIT_PRIORITY = {
    '데이트':   ['reservation', 'seats'],
    '기념일':   ['reservation', 'seats'],
    '가족모임': ['seats', 'group', 'hours', 'reservation'],
    '모임':     ['group', 'reservation', 'seats'],
    '혼밥':     ['hours', 'last_order', 'seats'],
    '친구':     ['seats', 'reservation'],
    '간단히':   ['hours', 'last_order'],
  };
  const _allowKeys = _VISIT_PRIORITY[purpose] || null;   // null = 전체 통과(폴백)
  const _visitLinesFiltered = _allowKeys
    ? [
        _allowKeys.includes('reservation') && _vi.reservation && `예약: ${String(_vi.reservation).trim()}`,
        _allowKeys.includes('hours')       && _vi.hours       && `영업시간: ${String(_vi.hours).trim()}`,
        _allowKeys.includes('last_order')  && _vi.last_order  && `라스트오더: ${String(_vi.last_order).trim()}`,
        _allowKeys.includes('parking')     && _vi.parking     && `주차: ${String(_vi.parking).trim()}`,
        _allowKeys.includes('seats')       && _vi.seats       && `좌석: ${String(_vi.seats).trim()}`,
        _allowKeys.includes('group')       && _vi.group       && `단체: ${String(_vi.group).trim()}`,
      ].filter(Boolean)
    : _visitLines;
  const visitInfoBlock = _visitLinesFiltered.length ? _visitLinesFiltered.join(" / ") : "";
  // 매장 고유 특징(memo) — 실입력 있을 때만. 없으면 일반론 방지 지시만.
  const storeMemo = (memo || "").trim();

  console.log(`[western] mode: ${validMode} | 상황: ${situation || "(미지정)"} | 목적: ${purpose || "(미지정)"}`);
  console.log(`[western][BUILD] western-v1.0-stepG2 (Western Engine 독립 · STEP G2 잔여정정 10룰 · 배포검증: 이 줄이 안 보이면 구버전 실행 중)`);

  // ── restaurant 조합 검증 ─────────────────────────────────
  const WEST_IDS = WESTERN_TREATMENTS.map(t => t.id);
  const isRest = WEST_IDS.includes(program.id) || program.industry === "western";
  if (!isRest) {
    console.error(`[western] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  const treatmentData = WESTERN_TREATMENTS.find(t => t.id === program.id)
    || WESTERN_TREATMENTS[0];
  const cat  = treatmentData?.cat || "양식";
  ...
  console.log(`[western] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getWesternDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[western] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);

  // ── 시스템 프롬프트 ────────────────────
  const _pProfSys = getPurposeProfile(purpose);
  const _pLabelSys = (_pProfSys && _pProfSys.titleLabel) || WESTERN_PURPOSE_TITLE_LABEL[purpose] || purpose || "방문";
  const systemPrompt = validMode === "purpose"
    ? `당신은 ${region} 지역 양식 레스토랑을 '${_pLabelSys}' 목적으로 찾는 사람에게 도움이 되는 정보를 정리하는 블로그 작가입니다.
업종: 양식 레스토랑 | 지역: ${region} | 방문 목적: ${_pLabelSys} | 참고 메뉴 계열: ${cat}

[최상위 원칙 ★★ 목적 중심]
- 주인공은 '메뉴'가 아니라 '${_pLabelSys}'라는 방문 목적이다.
- 글은 검색자의 상황에서 출발한다 ("${(_pProfSys && _pProfSys.openingSituations && _pProfSys.openingSituations[0]) || region + '에서 레스토랑을 찾고 있다면'}" 같은 결).
- 메뉴는 목적을 푸는 수단으로만 등장한다 — 메뉴 설명글로 회귀 금지.
- 공간 경험(분위기·좌석·조명·간격·창가/예약석)을 목적과 연결해 구체적으로.

[★ 메뉴 고정 — 임의 확장 금지]
- 이 글의 메뉴는 '${menu}' 하나다. '${menu}'만 언급한다.
- 입력되지 않은 다른 메뉴(면·밥·고기·피자 등 계열 내 다른 품목)를 대표 메뉴처럼 끌어오지 마라.
- 메뉴 계열(${cat})은 참고일 뿐, 계열 내 다른 품목을 새로 등장시키지 않는다.
- '${menu}' 외 곁들임은 '샐러드·빵' 수준의 일반 곁들임까지만 허용.

[★ 매장 고유성 — 일반론 탈출]
- "어느 양식당에나 해당되는 일반론"으로 흐르지 마라. ${region}·${_pLabelSys}에 특정된 장면으로 쓴다.
${storeMemo ? `- 이 매장의 실제 특징: "${storeMemo}" — 이 특징을 자연스럽게 녹인다(문구 그대로 복붙 금지, 장면으로 풀어서).` : "- 매장 고유 특징 입력이 없으므로, 최소한 지역·목적에 맞는 구체 장면으로 차별화한다."}

[★ 방문정보 편입 — storeInfo 섹션 중심]
${visitInfoBlock ? `- 실제 방문정보: ${visitInfoBlock}\n- 이 정보를 나열(체크리스트)이 아니라 문장 흐름에 자연스럽게 녹인다. 없는 정보는 지어내지 않는다.` : "- 입력된 방문정보가 없으므로 예약/영업시간/주차/라스트오더를 임의로 지어내지 않는다."}

[절대 금지]
- ❌ 메뉴("파스타는~"·"스테이크는~")로 글 시작
- ❌ 광고 단정: 최고/원조/찐맛집/강추/꼭 가보세요/인생
- ❌ 매장명 본문 직접 노출 → 지역·골목·일대·공간 지시어로 치환
- ❌ 가격 숫자 명시, 효능·의학 단정
- ❌ 의료·카페·학습 어휘(시술·카공·콘센트·공부하기 좋은 등)
- ❌ 입력 메뉴('${menu}') 외 메뉴 대표화

[반복 억제 ★]
- 앞 섹션에서 이미 쓴 표현(분위기·조명·창가·예약·대화 등)을 그대로 반복하지 마라.
- 같은 개념은 섹션마다 다른 각도·다른 단어로. 동일 문형 반복 금지.

[권장 표현]
- "~라면" 상황 제시체 / "이 자리는 ~에 맞는다" / "~목적엔 이렇게 고르면 좋다"

[문단 길이] 한 문단 2~4줄 유지 (5줄 이상 ❌).
[문장 종결] 모든 문장은 "다."/"요."/"습니다." 등으로 종결. 미완성·비문 금지.`
    : validMode === "commercial"
    ? `당신은 ${region} 지역 ${menu} ${cat} 매장 정보를 정리하는 정보형 블로그 작가입니다.
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getWesternSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";

  for (const sec of SECTIONS) {
    const richPrompt = buildWesternPrompt(sec.key, treatmentData, region, {
      mode: validMode, situation, purpose,
    });
    const prevBlock = prevTextRaw
      ? `\n[지금까지 작성된 내용 — 아래 표현·문형을 반복하지 말 것]\n${prevTextRaw.slice(0, 1500)}\n[끝]\n${validMode === "purpose" ? "⚠️ 특히 '분위기·조명·창가·예약·대화' 같은 이미 나온 표현은 다른 각도/다른 단어로 바꿔 쓸 것. 같은 말 반복 금지.\n" : ""}`
      : "";

    // ★ [Purpose 품질] 섹션별 보조 지시 — 메뉴 고정·고유성·방문정보. purpose 모드만. 빈값이면 미출력.
    const _purposeHints = (validMode === "purpose") ? [
      `⚠️ 이 글의 메뉴는 '${menu}' 하나뿐. '${menu}' 외 다른 메뉴명을 새로 등장시키지 말 것.`,
      storeMemo ? `⚠️ 매장 특징 반영: "${storeMemo}"(장면으로 자연스럽게).` : `⚠️ 일반론 금지 — ${region}·${purpose}에 특정된 구체 장면으로.`,
      (sec.key === "storeInfo" && visitInfoBlock) ? `⚠️ 방문정보를 문장에 자연스럽게 녹일 것(나열 금지): ${visitInfoBlock}` : "",
    ].filter(Boolean).join("\n") : "";

    const userPrompt = `업종: western | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat} | 상황: ${situation} | 목적: ${purpose} | 모드: ${validMode}
${prevBlock}
---
[현재 섹션: ${sec.label} (${sec.key})]
⚠️ 이 섹션만 작성. 의료·카페·학습·광고 어휘 금지. ${sec.minLength}자 이상.
${_purposeHints ? _purposeHints + "\n" : ""}${richPrompt}`;

    let secText = await generateSection({ systemPrompt, userPrompt });
    secText = cleanWesternText(secText, treatmentData, region, situation, purpose, validMode);
    secText = stripInlineImages(secText);
    // ★ [Purpose 품질] restoreKeyword는 메뉴명을 문맥 무시하고 삽입("리소토 지역/공간" 비문 유발) → purpose 스킵.
    //   purpose는 메뉴 반복 강제가 불필요(메뉴=수단). commercial/personal은 기존대로 복원.
    if (validMode !== "purpose") secText = restoreKeyword(secText, menu);

    if (calcCharCount(secText) < 100) {
      console.log(`[western] ${sec.label}: 빈 섹션 → 재생성`);
      let retry = await generateSection({
        systemPrompt,
        userPrompt: `${userPrompt}\n\n[중요] 반드시 ${sec.minLength}자 이상 실제 내용으로 작성.`,
        temperature: 0.72,
      });
      retry = cleanWesternText(retry, treatmentData, region, situation, purpose, validMode);
      retry = stripInlineImages(retry);
      if (validMode !== "purpose") retry = restoreKeyword(retry, menu);
      if (calcCharCount(retry) > calcCharCount(secText)) secText = retry;
    }

    console.log(`[western] ${sec.label}: ${calcCharCount(secText)}자`);
    sectionTexts[sec.key] = secText;
    prevTextRaw += "\n" + secText;
  }

  // ── 이미지 ALT (맛집 5종 풀: 외관/메뉴판/상차림/장면/마무리) ─────────
  // ── [DEAD CODE — Restaurant Image SoT 전환 후 미사용] ──
  //   본문 마커는 photoPolicyRegistry 5슬롯이 SoT. 아래 섹션키→alt 매핑은 참조용 보존.
  // eslint-disable-next-line no-unused-vars
  const _WEST_ALT_BY_KEY = {
    // personal 6섹션
    visit:   "외관 사진",
    arrive:  "외관 사진",
    order:   "메뉴판 사진",
    ...
    menuIntro:         "메뉴 사진",
    menuScene:         "메뉴 클로즈업",
    menuComposition:   "구성·상차림 사진",
    tasteFeature:      "국물·식감 디테일",
    pairing:           "반찬·곁들임 사진",
    decision:          "메뉴 비교 사진",
    recommendSituation:"테이블 분위기",
    storeFeature:      "매장 내부 사진",
    // ★ [Pilot] purpose 4섹션
    purposeIntro:      "레스토랑 외관·입구 사진",
    placeExperience:   "좌석·창가 분위기 사진",
    menuAsMeans:       "목적별 메뉴 구성 사진",
    storeInfo:         "매장 좌석·외관 사진",
  };
  // ── [Restaurant Image SoT] 본문 마커 = registry 5슬롯 고정 ──
  //   슬롯 정의는 photoPolicyRegistry 단일 SoT. 엔진은 섹션 경계에 순서대로 소비만.
  //   섹션 경계(섹션 사이) 우선 배치 → 슬롯이 남으면 마지막 섹션 앞에 순차 보충.
  //   → 섹션 수(purpose 4 / personal 5~6 / commercial 8)와 무관하게 항상 5장 보장.
  const _restLabels = getRestaurantPhotoLabels(); // ["외관","실내/좌석","대표 메뉴","메뉴판","분위기/디테일"]
  const altList = _restLabels.map(lb => `[이미지: ${lb}]`);
  console.log(`[western][ALT] mode=${validMode} slots=${_restLabels.length} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  // ★ [Pilot] purpose 모드는 overrideTitle 무시 — 목적 제목 엔진이 항상 적용(A/B 비교 보존).
  //   commercial/personal은 기존대로 overrideTitle 우선(무변경).
  let title = (validMode === "purpose")
    ? buildWesternTitle(treatmentData, region, situation, purpose, seoData, validMode, _titleSuffixOpts)
    : (overrideTitle || buildWesternTitle(treatmentData, region, situation, purpose, seoData, validMode, _titleSuffixOpts));
  // 의료·카페 어휘가 제목에 새면 차단
  const TITLE_BLOCK = /시술|치료|성형|진료|병원|카공|콘센트|드립|라떼아트/;
  if (TITLE_BLOCK.test(title)) {
    const _pProf = getPurposeProfile(purpose);
    const _pLabel = (_pProf && _pProf.titleLabel) || WESTERN_PURPOSE_TITLE_LABEL[purpose] || purpose || "방문";
  ...
  // ★ purpose 모드: 제목에 menu 미포함이 정상 → region만 보장. commercial/personal은 기존 유지.
  if (validMode === "purpose") {
    const _pProf = getPurposeProfile(purpose);
    const _pLabel = (_pProf && _pProf.titleLabel) || WESTERN_PURPOSE_TITLE_LABEL[purpose] || purpose || "방문";
    if (!title.includes(region) || !title.includes(_pLabel)) {
      title = `${region} ${_pLabel} 레스토랑`;
    }
  } else if (!title.includes(region) || !title.includes(menu)) {
    title = validMode === "commercial"
      ? `${region} ${menu} 안내`
  ...
  let assembled = `# ${title}\n\n`;
  // 유효 섹션(50자↑)만 추림 → 섹션 경계 개수 산출
  const _validKeys = secKeys.filter(k => calcCharCount(sectionTexts[k] || "") >= 50);
  const _nSec = _validKeys.length;
  // 경계 배치 규칙:
  //   섹션 사이 경계(첫 섹션~마지막 직전)에 슬롯을 순서대로 1개씩 소비.
  //   경계 수(_nSec-1)가 5보다 적으면 → 남은 슬롯을 마지막 섹션 "앞"에 순차 보충.
  //   경계 수가 5 이상이면 → 앞 5개 경계에만 배치(문단 중간 삽입 없음).
  let _slotIdx = 0;
  _validKeys.forEach((key, i) => {
    // 마지막 섹션 진입 직전: 남은 슬롯 전부를 이 섹션 앞에 순차 보충
    if (i === _nSec - 1) {
      while (_slotIdx < altList.length) {
        assembled += altList[_slotIdx] + "\n\n";
        _slotIdx++;
      }
      assembled += (sectionTexts[key] || "") + "\n\n";
      return;
    }
    assembled += (sectionTexts[key] || "") + "\n\n";
    if (_slotIdx < altList.length) {
      assembled += altList[_slotIdx] + "\n\n";
      _slotIdx++;
    }
  });
  ...
  assembled += "\n\n" + buildWesternHashtags(treatmentData, region, situation, purpose, validMode);
  ...
  //         제목은 buildWesternTitle 단계에서 이미 menu 포함 보장됨
  {
    ...
    let cleanedBody = cleanWesternText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanWesternText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  // ─────────────────────────────────────────────
  // alt 강제 정규화 — Restaurant 5슬롯 SoT (외관/실내·좌석/대표 메뉴/메뉴판/분위기·디테일)
  //   registry 라벨은 그대로 보존, 잡마커(GPT 누수 등)만 5슬롯으로 재매핑
  // ─────────────────────────────────────────────
  const _restLabelSet = new Set(_restLabels); // registry SoT 라벨
  assembled = assembled.replace(/\[이미지:\s*([^\]]+)\]/g, (_m, inner) => {
    const s = String(inner).trim();
    if (_restLabelSet.has(s)) return `[이미지: ${s}]`;                 // registry 라벨 보존
    if (/외관|입구|간판|건물|골목/.test(s))                  return "[이미지: 외관]";
    if (/실내|좌석|자리|홀|테이블 배치/.test(s))              return "[이미지: 실내/좌석]";
    if (/대표|메뉴판/.test(s) && /메뉴판/.test(s))            return "[이미지: 메뉴판]";
    if (/메뉴판|가격표/.test(s))                             return "[이미지: 메뉴판]";
    if (/대표\s*메뉴|한 접시|클로즈업|상차림|플레이팅|음식|메뉴/.test(s)) return "[이미지: 대표 메뉴]";
    if (/분위기|디테일|조명|창가|동행|장면/.test(s))          return "[이미지: 분위기/디테일]";
    return "[이미지: 분위기/디테일]";
  });

  const _altAll = assembled.match(/\[이미지:[^\]]+\]/g) || [];
  const _altOk  = _altAll.filter(a => {
    const inner = a.replace(/^\[이미지:\s*/, "").replace(/\]$/, "").trim();
    return _restLabelSet.has(inner);
  });
  ...
  console.log(`[western] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  // 경고
  if (qc.adPatternCount > 0)
    console.error(`[western] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[western] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[western] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[western] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[western] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[western] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[western] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
