// ============================================================
// pages/api/generateLighting.js — 조명(lighting) 엔진 핸들러
// 정보형 + 출장 시공 안내형({region} 조명 시공 업체 1인칭).
// 후기형·내돈내산·고객사례·추천·보장·최저가·비용유도·수명약속 금지.
// engineBootstrap에서 register("lighting", handleLighting)로 편입.
//
// 복제 베이스: generateWindow.js (섹션루프형). Runtime/FLOW/Handler 구조 무변경.
// window 대비 변경 1점:
//   ① 해시태그 어휘 — 조명 검색축(조명시공/조명업체)으로 교체. dedup 는 동형 Set 유지.
//      ★ [세션68 원칙] Set dedup 은 완전일치만 제거한다. cat 명이 태그 어휘를 포함하면
//        부분문자열 중복이 남는다 → lighting cat 6종은 전부 「조명/등」으로 끝나므로
//        `#{rg}조명시공` 과 `#{rg}{k}` 는 서로 포함 관계가 아니다(실측 확인).
...
import {
  LIGHTING_TREATMENTS,
  LIGHTING_INFO_BLOCKS,
  getInfoBlocks,
  formatSymptom,
  getPartNote,
} from "../../lib/lighting-data";
import {
  buildSystemPrompt,
  buildUserPrompt,
  getImageAlts,
  getSectionAlt,
  FORBIDDEN,
} from "../../lib/lighting-prompts";
import { LIGHTING_FLOW, LIGHTING_SECTION_PHOTO } from "../../lib/lighting-playConfig";
...
//   → 메뉴 확장은 lighting-data.js 단독 작업. 핸들러를 다시 열지 않는다.
function pickInfoBlock(t) {
  const B = (typeof getInfoBlocks === "function" ? getInfoBlocks(t.cat) : null)
    || LIGHTING_INFO_BLOCKS;
  return B[t.infoKey] || B[t.cat] || LIGHTING_INFO_BLOCKS.prebook;
}
...
//   ★ lighting 은 siteBlock 미사용(useSite:false) → 단지명 분기 없음.
...
export default async function handleLighting(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
      symptom,                                   // 접수된 상태(구석만 어둡다·눈부심 등) — 미입력 시 무영향
      part,                                      // 계측 대상(cove·line·recess·magnetic·track·pendant)
    } = req.body;

    // 조명 = 출장 시공 업종 → 고정 사업장 위치블록 미노출(PATCH-07)
    const _locStore = {};
    ...
    const treatment =
      LIGHTING_TREATMENTS.find((t) => t.id === program?.id) ||
      LIGHTING_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `조명 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    for (const sec of LIGHTING_FLOW) {
      ...
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*조명\s*시공\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }

      const slot = LIGHTING_SECTION_PHOTO[sec.key];
    ...
    const rgKwCount = region
      ...
      : 0;
    console.log(`[QC][lighting] 메뉴(${kw}): kw ${kwCount} / 지역+kw 결합 ${rgKwCount} (3 이하 정상)`);
    console.log(`[QC][lighting] 글자수: ${content.length}`);
    console.log(`[QC][lighting] 발견: ${sym || "미입력"} / 계측 대상: ${pt ? pt.label : "미입력"}`);
    console.log(`[QC][lighting] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[lighting] 오류:", err);
    return res.status(500).json({ error: err.message || "조명 글 생성 오류" });
  }
}
