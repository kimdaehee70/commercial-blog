// ╔══════════════════════════════════════════════════════════╗
// ║ pages/api/generateUrologyV2.js — 비뇨기과 V2 Purpose       ║
// ║ generateEntV2 골격 복제 + 비뇨기과 치환.                   ║
// ║ self-contained — v1(generateUrology) 보존·미호출.         ║
...
import { UROLOGY_V2_TREATMENTS } from "../../lib/urology-v2-data";
import {
  buildUrologyV2Prompt,
  UROLOGY_V2_SYSTEM_PROMPT,
  getUrologyV2ImageAlts,
  getUrologyV2Direction,
} from "../../lib/urology-v2-prompts";
...
// ============================================================
const UROLOGY_CAT_FOCUS = {
  "검사":       { note: "검사 선택 기준과 확인 범위가 축입니다. 질환 치료 설명으로 흐르지 않습니다." },
  "전립선":     { note: "배뇨 증상과 전립선 상태 확인이 판단의 출발점. 크기·흐름·잔뇨 소견에 따라 방향이 나뉩니다. 암 진단 단정 금지 — 추가 확인 필요 여부까지만." },
  "방광·배뇨":  { note: "배뇨 기록과 소변 소견이 출발점. 방광이 예민해진 쪽인지 통로가 좁아진 쪽인지를 나누는 흐름이 축입니다." },
  "요로·감염":  { note: "소변 소견 확인이 출발점. 염증·결석·혈뇨 중 어느 범위인지를 가르는 흐름이 축입니다. 특정 질환 단정 금지." },
  "남성건강":   { note: "혈관·호르몬·생활 요인 중 원인 범위를 가르는 것이 축. 성적 수행·만족도 서술 절대 금지. 시술·기구 언급 금지." },
};
...
function countViolations(text) {
  let n = 0;
  UROLOGY_V2_FORBIDDEN.forEach(w => { if (text.includes(w)) n++; });
...
function finalClean(text) {
  ...
  // 암 단정 차단 (PSA·전립선 계열 — 추가 확인 필요 여부까지만)
  ...
  // 성적 수행·과장 (남성건강 계열)
  t = t.replace(/정력\S*|성기능 회복\S*|자신감 회복\S*|남성성\S*|만족도 상승\S*/g, "");
  // 광고
...
function buildTitle(treatment, region) {
  ...
  const d    = getUrologyV2Direction(treatment.id);
...
function buildHashtags(name, region) {
  ...
  return "#" + reg + base + " #" + reg + "비뇨기과 #" + base + "정보 #배뇨증상진료안내";
}
...
export default async function handler(req, res) {
  ...
  const treatment =
    UROLOGY_V2_TREATMENTS.find(t => t.id === program?.id) ||
    UROLOGY_V2_TREATMENTS[0];
  ...
  const dir     = getUrologyV2Direction(tid);
  const axis    = dir.decisionAxis === "exam" ? "exam" : "disease";
  const catNote = (UROLOGY_CAT_FOCUS[treatment.cat] || { note: "확인 항목과 결정 기준을 중심으로 서술합니다." }).note;

  const finalTitle = buildTitle(treatment, region);
  let result = "# " + finalTitle + "\n\n";

  const imgAlts = getUrologyV2ImageAlts(treatment, region, activeKeyword);
  ...
  // ── 섹션 루프 ──
  for (const sec of UROLOGY_V2_FLOW_ENGINE.sections) {
    const basePrompt = buildUrologyV2Prompt(sec.key, treatment, region);
  ...
  //     urology 대명사 = "이 검사"·"이 진료" → 둘 다 무받침. ent와 동일 잠복(무해) 상태.
  //   부가: NAME_THRESHOLD 4.
  // ============================================================
  {
