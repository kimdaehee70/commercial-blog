// ╔══════════════════════════════════════════════════════════╗
// ║ pages/api/generateEyeV2.js — 안과 V2 Purpose               ║
// ║ ★ 재구축(2026-07-13): 목적축 5섹션 → decisionAxis 7섹션    ║
// ║   기존 generateEyeV2(dental-v2 골격·목적축) 전면 교체.      ║
// ║   generateEntV2 골격 복제 + 안과 치환. 병원군 철학 통일.    ║
// ║ self-contained — v1(generateEye) 보존·미호출.             ║
...
import { EYE_V2_TREATMENTS } from "../../lib/eye-v2-data";
import {
  buildEyeV2Prompt,
  EYE_V2_SYSTEM_PROMPT,
  getEyeV2ImageAlts,
  getEyeV2Direction,
} from "../../lib/eye-v2-prompts";
import { EYE_V2_FLOW_ENGINE } from "../../lib/eye-v2-playConfig";
import { insertLocationBeforeHashtags } from "../../lib/locationBlock.js"; // PATCH-07
import { insertVisitBeforeHashtags } from "../../lib/visitBlock.js";       // [VISIT-01] 방문정보 후단 블록
...
const EYE_CAT_FOCUS = {
  ...
  "백내장·노안":  { note: "시력·굴절 확인과 눈 앞쪽 현미경 관찰이 출발점. 혼탁 정도만이 아니라 일상 불편 정도가 함께 판단 근거입니다. 수술 과정·렌즈 종류 비교 금지." },
  "망막·녹내장":  { note: "안압·시신경·시야·안저 소견을 함께 보는 것이 축. 한 가지 수치만으로 판단하지 않는 구조를 유지합니다. 실명 등 불안 조장 표현 절대 금지." },
  "안구표면":     { note: "현미경으로 눈물층과 각막·결막 표면을 확인하는 것이 출발점. 건조 양상인지 염증 양상인지를 나누는 흐름이 축입니다." },
  "소아안과":     { note: "이전 도수 기록과 조절 마비 상태의 굴절 값 확인이 축. 진행 속도와 생활 요인이 판단 근거입니다. 드림렌즈·시력교정 수술 언급 금지." },
};
...
function countViolations(text) {
  let n = 0;
  EYE_V2_FORBIDDEN.forEach(w => { if (text.includes(w)) n++; });
...
function finalClean(text) {
  ...
  // 시력 회복 보장 차단
  t = t.replace(/시력이 돌아옵니다|시력 회복 보장\S*|안경을 벗을 수 있습니다/g, "경과에 따라 달라질 수 있습니다");
  // 불안 조장·응급 유도 (안과 고질 — 실명 표현)
  t = t.replace(/실명\S*|영구 손상\S*|평생 안 보입니다/g, "경과");
  t = t.replace(/골든타임\S*/g, "경과");
  t = t.replace(/지금 바로 병원\S*|즉시 내원\S*|지체 없이|서둘러\S*|한시라도\S*/g, "");
  t = t.replace(/응급 대처\S*/g, "확인 항목");
  // 감성 ending (v1 안과 고질 패턴 — 문장 단위 제거)
  t = t.replace(/[^.!?\n]*(?:또렷한 시야|밝아진 세상|새로운 세상|환해진 일상|세상이 달라)[^.!?\n]*[.!?]\s*/g, "");
  // 신체 부위 불일치 (안과 글에 청각·미각·후각 혼입)
  t = t.replace(/[^.!?\n]*(?:소리가 잘 들|청력|미각|후각)[^.!?\n]*[.!?]\s*/g, "");
  // 광고
...
function buildTitle(treatment, region) {
  ...
  const d    = getEyeV2Direction(treatment.id);
...
function buildHashtags(name, region) {
  ...
  return "#" + reg + base + " #" + reg + "안과 #" + base + "정보 #눈검사진료안내";
}
...
export default async function handler(req, res) {
  ...
  const dir     = getEyeV2Direction(tid);
  const axis    = dir.decisionAxis === "exam" ? "exam" : "disease";
  const catNote = (EYE_CAT_FOCUS[treatment.cat] || { note: "확인 항목과 결정 기준을 중심으로 서술합니다." }).note;

  const finalTitle = buildTitle(treatment, region);
  let result = "# " + finalTitle + "\n\n";

  const imgAlts = getEyeV2ImageAlts(treatment, region, activeKeyword);
  ...
  // ── 섹션 루프 ──
  for (const sec of EYE_V2_FLOW_ENGINE.sections) {
    const basePrompt = buildEyeV2Prompt(sec.key, treatment, region);
    ...
    const userPrompt =
      ...
      "🔒 감성 마무리 금지. 시력교정 수술(라식·라섹·ICL) 언급 금지.\n\n" +
      basePrompt;
  ...
  // ============================================================
  // 반복 분산 후처리 — 라인 단위 단일 패스   [3종 결함 수정본 · ent 이식]
  ...
  //     eye 대명사 = "이 검사"·"이 진료" → 둘 다 무받침. ent/urology와 동일 잠복(무해) 상태.
  //   부가: NAME_THRESHOLD 4.
  // ============================================================
  {
  ...
  // ── [VISIT-01] visitBlock → PATCH-07 locationBlock 순서 고정 ──
  //   출력: 🏥 방문 안내 → 📍 찾아오시는 길 → #해시태그
