// ============================================================
// pages/api/generateGrout.js — 줄눈(grout) 엔진 핸들러
// 정보형 + 시공선택 가이드형({region} 줄눈 시공 업체 1인칭).
// 후기형·내돈내산·만족도·추천·보장·최저가·순위·브랜드 강추 금지.
// engineBootstrap에서 register("grout", handleGrout)로 편입.
// 복제 베이스: generateCleaning.js (섹션루프형). 기존 업종 무수정.
//   ★ 제목 region 중복 방어: aptName 없을 때 {aptName} 토큰 삭제(region 폴백 금지) — interior v1.0 버그 반영.
...
import {
  ...
  getImageAlts,
  FORBIDDEN,
} from "../../lib/grout-prompts";
import { GROUT_FLOW } from "../../lib/grout-playConfig";
...
// ── 화자 도입 (줄눈 시공 업체 1인칭. 업체명 본문 미노출) ──
function buildAgentIntro(region, aptName) {
  if (aptName) {
    return `안녕하세요. ${region} 줄눈 시공 업체입니다.\n오늘은 ${aptName} 줄눈 시공을 안내해 드리겠습니다.`;
  }
  return `안녕하세요. ${region} 줄눈 시공 업체입니다.\n오늘은 ${region} 줄눈 시공을 안내해 드리겠습니다.`;
}

// ── 정보블럭 선택 (cat/treatment 기준) ──────────
function pickInfoBlock(treatment) {
  const cat = treatment.cat;
  if (cat === "욕실줄눈") return GROUT_INFO_BLOCKS.bath;
  if (cat === "주방줄눈") return GROUT_INFO_BLOCKS.kitchen;
  if (cat === "현관줄눈") return GROUT_INFO_BLOCKS.entrance;
  if (cat === "베란다줄눈") return GROUT_INFO_BLOCKS.veranda;
  if (cat === "구축아파트") return GROUT_INFO_BLOCKS.oldapt;
  if (cat === "재시공") return GROUT_INFO_BLOCKS.redo;
  if (cat === "종류비교") return GROUT_INFO_BLOCKS.compare;
  if (cat === "체크리스트") return GROUT_INFO_BLOCKS.check;
  return GROUT_INFO_BLOCKS.check;
}
...
// ── 제목 생성 ───────────────────────────────────
//   ★ interior v1.0 버그 반영: aptName/livingArea 없을 때 토큰을 (선행공백 포함) 삭제.
//      region 폴백 치환 금지 → "노원구 하계동 노원구 하계동" 중복 차단.
...
export default async function handleGrout(req, res) {
  try {
    const {
    ...
    // 줄눈 = 출장/현장방문 업종. 발행코치에서 위치 5필드 채워 보내면 「찾아오시는 길」 삽입,
    ...
    const treatment =
      GROUT_TREATMENTS.find((t) => t.id === program?.id) ||
      GROUT_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `줄눈 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    const PHOTO_ALT = {
      intro: "줄눈 시공 범위 안내", axis1: "시공 범위 안내", axis2: "관리 방법 안내",
      axis3: "진행 순서 안내", axis4: "시공 전 확인 안내", closing: "줄눈 시공 상담 안내",
    };

    const writtenSections = new Set();
    const sections = [];

    for (const sec of GROUT_FLOW) {
      ...
      body = body.replace(/(^|\n)\s*운영자\s*(\n|$)/g, "\n").trim();
      // closing 섹션이 화자 인사("안녕하세요 …줄눈 시공 업체입니다")를 재등장시키는 패턴 제거(intro 중복)
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*줄눈\s*시공\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }
      body += "\n\n[이미지: " + (PHOTO_ALT[sec.key] || "줄눈 시공 상담 안내") + "]";
      sections.push(body);
    }
    ...
    console.log(`[QC][grout] 메뉴(${kw}) aptName(${aptName || "-"}): kw ${kwCount}`);
    console.log(`[QC][grout] 글자수: ${content.length}`);
    console.log(`[QC][grout] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[grout] 오류:", err);
    return res.status(500).json({ error: err.message || "줄눈 글 생성 오류" });
  }
}
