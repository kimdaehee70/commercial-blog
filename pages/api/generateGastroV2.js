// ╔══════════════════════════════════════════════════════════╗
// ║ pages/api/generateGastroV2.js — 소화기내과 V2 Purpose      ║
// ║ generateRadioV2 골격 복제 + 소화기 치환.                   ║
// ║ self-contained — v1 4파일 의존 0 (data만 공유).           ║
// ║ ★ 고유 축: decisionAxis 1필드로 4섹션 분기                 ║
// ║   exam(6종) = 검사 선택 기준 / disease(16종) = 치료 결정   ║
...
import {
  buildGastroV2Prompt,
  GASTRO_V2_SYSTEM_PROMPT,
  getGastroV2ImageAlts,
  getGastroV2Direction,
} from "../../lib/gastro-v2-prompts";
import { GASTRO_V2_FLOW_ENGINE } from "../../lib/gastro-v2-playConfig";
...
// ============================================================
const GASTRO_CAT_FOCUS = {
  "내시경":     { note: "검사 선택과 준비·회복 조건이 축입니다. 질환 치료 설명으로 흐르지 않습니다." },
  "검진":       { note: "선별 목적과 주기·이전 결과 반영이 축입니다. 치료 설명 금지." },
  "위":         { note: "점막 상태 확인 → 약물 → 필요 시 내시경적 처치 순의 판단 축입니다." },
  "식도":       { note: "증상 양상 확인 → 약물 → 필요 시 내시경 확인·처치 순의 판단 축입니다." },
  "대장":       { note: "다른 원인 배제 → 생활·식이 조정 → 약물 → 필요 시 처치 순의 판단 축입니다." },
  "간":         { note: "수치·영상 추적이 중심 축. 원인 요인 조정과 정기 확인이 관리 항목입니다." },
  "담낭·담도":  { note: "증상 유무에 따라 경과 관찰과 처치가 갈립니다. 수술은 외과 위임 수준으로만." },
  "췌장":       { note: "원인 확인과 췌장 안정이 중심 축. 재발 방지 관리가 이어집니다." },
};
...
// ============================================================
// QC — 진단/효과 단정 · 광고 · 수술 상세 · 후기 잔재 · AI 논문투
// ============================================================
const GASTRO_V2_FORBIDDEN = [
...
// 1인칭 검출 — 어절 경계 기준. substring 매칭 금지("절제가/배제가/완하제가" 오탐 방지).
const FIRST_PERSON_RE = /(^|[\s"'(\[])(제가|저는|저도|내가|나는|제\s|저의|제\s?케이스)/;

function countViolations(text) {
  let n = 0;
  GASTRO_V2_FORBIDDEN.forEach(w => { if (text.includes(w)) n++; });
...
function buildTitle(treatment, region) {
  ...
  const d    = getGastroV2Direction(treatment.id);
  const isExam = d.decisionAxis === "exam";
  // V2 정보형 제목 — v1 후기형 titlePatterns(1인칭 후기) 미사용
  const t = isExam
...
function buildHashtags(name, region) {
  ...
  // [축A · 2026-07-14] 해시태그 지역 공백 제거 — 2어절 생활권(「잠실 송파구」)에서 태그가
  //   공백에서 끊겨 무효화(#잠실 + 송파구대장내시경). 1어절(강남)에서는 미발현 → 그간 은폐.
  //   region 은 태그 문자열 내부에서만 공백 제거. 본문·제목·locationBlock 의 region 은 무손상.
  const reg  = String(region || "").replace(/[\s·()]/g, "");
  return "#" + reg + base + " #" + reg + "소화기내과 #" + base + "정보 #소화기진료안내";
}
...
export default async function handler(req, res) {
  ...
  const treatment =
    GASTRO_TREATMENTS.find(t => t.id === program?.id) ||
    GASTRO_TREATMENTS[0];
  ...
  const dir     = getGastroV2Direction(tid);
  const axis    = dir.decisionAxis === "exam" ? "exam" : "disease";
  const catNote = (GASTRO_CAT_FOCUS[treatment.cat] || { note: "확인 항목과 결정 기준을 중심으로 서술합니다." }).note;

  const finalTitle = buildTitle(treatment, region);
  let result = "# " + finalTitle + "\n\n";

  const imgAlts = getGastroV2ImageAlts(treatment, region, activeKeyword);
  ...
  const systemPrompt =
    ...
    "- 판단 축: " + (axis === "exam"
        ...
        : "질환형 — '검사·치료가 어떤 기준으로 결정되는가'가 4번째 섹션의 축입니다. 수술 상세 설명 금지(다른 진료과 위임 수준만).") + "\n" +
    "- 계열 유의사항: " + catNote + "\n" +
    "- 모든 섹션이 이 주제 중심으로 일관되게 작성되어야 합니다.\n" +
    (photoCtx ? "\n[사진 컨텍스트]\n" + photoCtx + "\n" : "");

  // ── 섹션 루프 ──
  for (const sec of GASTRO_V2_FLOW_ENGINE.sections) {
    const basePrompt = buildGastroV2Prompt(sec.key, treatment, region);
