...
import {
  MEAT_TREATMENTS, MEAT_BLOCK_MAP,
  MEAT_TITLE_MIDDLE, MEAT_TITLE_SUFFIX,
  MEAT_TITLE_SCENE, MEAT_TITLE_SCENE_BY_CATEGORY,
  MEAT_TITLE_SEARCHWORD,
} from "../../lib/meat-data";
import {
  buildMeatPrompt, getMeatDirection,
} from "../../lib/meat-prompts";
import {
  getMeatSections,
} from "../../lib/meat-playConfig";
import {
...
// [세션15] 지역 입력 정규화 — 전 업종 공유. "구+구" 오입력 보정(SEO 무관).
import { normalizeRegion } from "../../lib/regionNormalize";
// [세션19] Platform Spine — 제목 조립 공통 위임 (data·prompt·본문 무변경. 풀 주입식)
...
const REST_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("meat");
  } catch (e) {
    return [
      ...MEAT_BLOCK_MAP.medical,
      ...MEAT_BLOCK_MAP.cafe,
      ...MEAT_BLOCK_MAP.study,
      ...MEAT_BLOCK_MAP.ad,
    ];
  }
})();
...
// [세션19] Spine state로 위임. 모듈 스코프 보관 유지(연속 발행 회피 동작 동일).
let _titleState = {};

function buildMeatTitle(treatment, region, situation, purpose, seoData, mode, titleSuffixOpts) {
  const menu = treatment.menu || treatment.menuRef || "";
  const cat  = treatment.cat || "한식";
  const sit  = situation || "";
  const pur  = purpose   || "";

  // [세션19] Platform Spine 위임.
  //   - data/prompt/본문 무변경. titlePatterns·SCENE·MIDDLE·SUFFIX·SEARCHWORD는 data 소유 → 풀 주입.
  //   - purpose 라벨: Registry(resolvePurpose)가 MEAT_PURPOSE_TITLE_LABEL과 동일값 소유 → 동치.
  //   - purposeTitleLabel 폴백: 기존 getMeatDirection().purposeLabel 경로 보존(Registry 미선택 시).
  //   - mode === 'commercial' : Spine 조립. 그 외(personal) : 기존 로직 유지(아래).
  if (mode === "commercial") {
    const dir = getMeatDirection(treatment, situation, purpose) || {};
    const pools = {
      titlePatterns: seoData?.titlePatterns || [],   // STORE_PROFILE 소유(평면 배열)
      SCENE:        MEAT_TITLE_SCENE,
      SCENE_BY_CAT: MEAT_TITLE_SCENE_BY_CATEGORY,
      MIDDLE:       MEAT_TITLE_MIDDLE,
      SUFFIX:       MEAT_TITLE_SUFFIX,
      SEARCHWORD:   MEAT_TITLE_SEARCHWORD,
    };
    const suffix = titleSuffixOpts
      ? resolveTitleSuffix(titleSuffixOpts)   // {enabled, storeName}
      : DEFAULT_TITLE_SUFFIX;                 // 기본 OFF
    const out = buildTitle({
      region, menu, cat, situation: sit, purpose: pur,
      mode: "commercial",
      pools,
      purposeTitleLabel: dir.purposeLabel || pur || "",
      state: _titleState,
      suffix,
      sceneProbability: 0.4,
      maxLen: 40,
    });
    _titleState = out.state;   // 연속 발행 회피 상태 보관(기존 _lastTitleMiddle/Suffix 대체)
    return out.title;
  }

  // ── personal — 기존 로직 유지 (Spine 미적용, 동치 보존) ──
...
// ============================================================
function buildMeatHashtags(treatment, region, situation, purpose, mode) {
...
// ============================================================
function cleanMeatText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  ...
  // 2) 일반화: 고기 부위 어미 우선 + (잔여) 한식/분식 어미 + 6종 조사 + 장소명사
  //    (고기집: 삼겹살·목살·갈비·차돌박이 등은 1) 동적 menu 정규식이 1차 커버)
  //    (국/탕/찌개/밥/면/구이/볶음/전/회/탕면/국밥/덮밥/비빔밥)
  //    + 고기 어미 (삼겹살/목살/갈비/차돌박이/생고기) + 분식 어미(잔여)
  const genericMenuRe = new RegExp(
    `[가-힣]{0,4}(?:삼겹살|목살|항정살|돼지갈비|소갈비|갈비|차돌박이|생고기|국|탕|찌개|밥|면|구이|볶음|전|회|탕면|국밥|덮밥|비빔밥|떡볶이|김밥|튀김|순대|어묵|라면|만두)${PARTICLES}\\s+${PLACE_NOUNS}`,
    "g"
  );
  ...
  const safetyFoodRe = /[가-힣]{0,4}(?:삼겹살|목살|항정살|돼지갈비|소갈비|차돌박이|생고기|갈비|고깃집|고기집)(이|가|은|는|에|을|를)\s+(동네|일대|집|골목|쪽|근처|거리|상권|먹자골목)/g;
  ...
  if (particleErrorCount > 0) {
    console.log(`[meat][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[meat][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[meat][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[meat][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  // ★ FREEZE 예외 #1 (placeholder 결합 교정) — meat 전용
  ...
  {
    const genRe = /([가-힣]{0,5}(?:삼겹살|목살|항정살|돼지갈비|소갈비|갈비|차돌박이|생고기|떡볶이|김밥|튀김|꼬치|순대|어묵|라면|만두))(?:이|가)\s*(한\s*그릇|조합|접시|한\s*점|한\s*판)/g;
  ...
  if (menu) {
    ...
    {
      const bizDup = [
        [/전문점에서는\s*(?:분식점들은|가게들은|식당들은|한식당들은|매장들은|음식점들은|고깃집들은|고기집들은)/g, "전문점에서는"],
        [/전문점의\s*(?:가게들은|분식집에서는|식당들은|한식당들은|매장들은|고깃집들은|고기집들은|고깃집|고기집)/g, "전문점에서는"],
    ...
    // ★ [신규 v2.6] 동일 메뉴명 "전문점" 2회 이중주입 → 1회로 축약 (실측 재현)
    //   "소갈비 전문점에서는 다양한 소갈비 전문점을 쉽게 찾을 수 있어" 류.
    //   기존 bizDup(업종복수형)은 못 잡음 — 동일 menu 명사 2회 패턴 별도 처리.
    //   menu가 있을 때만. 앞단 "전문점+조사"는 살리고 "다양한 menu 전문점"의 menu만 제거.
    if (menu) {
      const mEsc3 = menu.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      // A) "menu 전문점…다양한 menu 전문점(을/이…)" → "menu 전문점…다양한 전문점\1"
      const dupSpecRe = new RegExp(
        `(${mEsc3}\\s*전문점[가-힣\\s]{0,12}?다양한)\\s*${mEsc3}\\s*전문점`, "g"
      );
      result = result.replace(dupSpecRe, (m, head) => { menuDupCount++; return `${head} 전문점`; });
      // B) "다양한 menu 전문점…menu 전문점" 역순도 안전망 (다양한+menu 살리고 뒤 menu 제거)
      const dupSpecRe2 = new RegExp(
        `(다양한\\s*${mEsc3}\\s*전문점[가-힣\\s,]{0,20}?)${mEsc3}\\s*전문점`, "g"
      );
      result = result.replace(dupSpecRe2, (m, head) => { menuDupCount++; return `${head}전문점`; });
    }
    // ★ [신규 v2.6] 조사 누락 "menu 특별한/특별/다양한 N" → "menu로 N" (실측: "소갈비 특별한 경험")
    //   "menu + 공백 + 관형사(특별한/색다른/특별/다양한) + 명사" 에서 menu 뒤 조사 누락.
    //   menu 직후 관형어가 바로 오면 "menu로" 보정.
    if (menu) {
      const mEsc4 = menu.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const last4 = menu.charCodeAt(menu.length - 1);
      const jong4 = (last4 - 0xac00) % 28 !== 0;       // 받침 有 → "으로", 無 → "로"
      const ro = jong4 ? "으로" : "로";
      const missJosaRe = new RegExp(`${mEsc4}\\s+(특별한|색다른|특별|남다른)\\s+(경험|시간|맛|자리|하루)`, "g");
      result = result.replace(missJosaRe, (m, adj, noun) => {
        menuDupCount++;
        return `${menu}${ro} ${adj === "특별" ? "특별한" : adj} ${noun}`;
      });
      // ★ [신규 v2.6] 조사 누락 "menu 경우" → "menu의 경우" (실측: "목살 경우")
      //   menu 직후 "경우"가 조사 없이 바로 오면 관형격 '의' 삽입.
      const caseRe = new RegExp(`${mEsc4}\\s+경우(?=[는를가에\\s,.])`, "g");
      result = result.replace(caseRe, () => { menuDupCount++; return `${menu}의 경우`; });
    }
    // ★ [신규 v2.6] 부사격 조사 중복 "에서는에서는" → "에서는" (치환 잔여물 안전청소)
    //   현상(실측): "목살 전문점에서는에서는". 지역치환/업종어 치환이 겹치며 발생.
    //   menu 무관 — 어미 중복 일반 청소. 다른 조사 중복도 함께.
    {
      const dupJosa = [
        [/에서는\s*에서는/g, "에서는"],
        [/에서\s*에서/g, "에서"],
  ...
  let unitFixCount = 0;
  {
    ...
    } else if (!su.includes("그릇")) {
      // ★ 구이류(servingUnit='1인분' 등 그릇계열 아님): 떠도는 "한 그릇" → "한 점"
      //   현상(실측 4종 재현): 소갈비/차돌박이/돼지갈비/갈비에서 GPT가 "이 한 그릇",
      //     "한 그릇을 찾는" 출력. data는 1인분이나 프롬프트 잔상으로 누수.
      //   원칙: '그릇'은 국밥·탕류 단위. 구이류는 점/판이 맞음 → "한 점"으로 통일.
      //   ⚠ servingUnit에 '그릇'이 명시된 메뉴(국밥류 등)는 이 분기 진입 안 함(과교정 방지).
      const grillConv = [
        [/이\s*한\s*그릇을(?=\s|[,.!?])/g, "이 한 점을"],
        [/이\s*한\s*그릇은(?=\s|[,.!?])/g, "이 한 점은"],
        [/이\s*한\s*그릇이(?=\s|[,.!?])/g, "이 한 점이"],
        [/이\s*한\s*그릇(?=[\s,.])/g, "이 한 점"],
        [/한\s*그릇을(?=\s|[,.!?])/g, "한 점을"],
        [/한\s*그릇은(?=\s|[,.!?])/g, "한 점은"],
        [/한\s*그릇이(?=\s|[,.!?])/g, "한 점이"],
        [/한\s*그릇으로(?=\s|[,.!?])/g, "한 점으로"],
  ...
  if (particleDupCount > 0) {
    console.log(`[meat][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[meat][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[meat][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[meat][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[meat][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[meat][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[meat][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[meat][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }

  // 외부 QC 집계용
  cleanMeatText.__lastParticleDup = particleDupCount;
  cleanMeatText.__lastSubjParticleFix = subjParticleFixCount;
  cleanMeatText.__lastEfficacyFix = efficacyFixCount;
  cleanMeatText.__lastStoreRefFix = storeRefFixCount;
  cleanMeatText.__lastMenuDup = menuDupCount;
  cleanMeatText.__lastSuperlativeFix = superlativeFixCount;
  cleanMeatText.__lastUnitFix = unitFixCount;
  cleanMeatText.__lastDupSentence = dupSentenceCount;
  cleanMeatText.__lastParticleErrors = particleErrorCount;
  cleanMeatText.__lastMenuMeta = menuMetaCount;
  cleanMeatText.__lastAdEvalFixed = adEvalFixed;
  cleanMeatText.__lastPlaceholderFix = placeholderFixCount;
...
// ============================================================
// 8. QC 체크 — cafe runQC + meat 추가 검사 (조합·침투·광고)
// ============================================================
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  //   - genericLeak: 고기 부위+한식 메뉴 어미+조사+장소 패턴 (일반)
  ...
  const genericLeakRe = /[가-힣]{0,4}(?:삼겹살|목살|항정살|돼지갈비|소갈비|갈비|차돌박이|생고기|국|탕|찌개|밥|면|구이|볶음|전|회|탕면|국밥|덮밥|비빔밥|떡볶이|김밥|튀김|순대|어묵|라면|만두)(?:이|가|은|는|에|을|를)\s+(?:동네|일대|집|골목|쪽|근처|거리|상권|먹자골목|가게|매장|식당|집안|안쪽|입구)/g;
  ...
  const adEvalFixed = (typeof cleanMeatText.__lastAdEvalFixed === "number")
    ? cleanMeatText.__lastAdEvalFixed : 0;

  // cleanMeatText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanMeatText.__lastParticleErrors === "number")
    ? cleanMeatText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanMeatText.__lastMenuMeta === "number")
    ? cleanMeatText.__lastMenuMeta : 0;
  ...
  console.log(`[meat][QC] 메뉴블럭: ${hasMealBlock}`);
  console.log(`[meat][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  console.log(`[meat][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[meat][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[meat][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[meat][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[meat][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[meat][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[meat][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[meat][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[meat][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[meat][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[meat][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
    console.log(`[meat][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[meat][QC] 가격명시(commercial 위반): ${priceCount}건`);
  }
...
// 메인 핸들러
// ============================================================
export default async function handleMeat(req, res) {
  const {
  ...
  // ★ [세션15] 지역 정규화 — 공통 모듈(regionNormalize) 사용. 엔진별 인라인 방어 폐기.
  //   "노원구 성북구"(구+구 오입력) → "성북구" / "노원구 공릉동"·"태릉입구" → 유지.
  //   SEO 아닌 입력값 보정. 전 업종 공유(locationBlock 동형). 근본 해결=생활권 동 단위 입력.
  ...
  console.log(`[meat] mode: ${validMode} | 상황: ${situation || "(미지정)"} | 목적: ${purpose || "(미지정)"}`);
  console.log(`[meat][BUILD] v4.0-people-subject (배포검증: 이 줄이 안 보이면 구버전 실행 중)`);

  // ── meat 조합 검증 ─────────────────────────────────
  const REST_IDS = MEAT_TREATMENTS.map(t => t.id);
  const isRest = REST_IDS.includes(program.id) || program.industry === "meat";
  if (!isRest) {
    console.error(`[meat] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  const treatmentData = MEAT_TREATMENTS.find(t => t.id === program.id)
    || MEAT_TREATMENTS[0];
  ...
  console.log(`[meat] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getMeatDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[meat] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);
...
업종: 고깃집 | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat}
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getMeatSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";

  for (const sec of SECTIONS) {
    const richPrompt = buildMeatPrompt(sec.key, treatmentData, region, {
    ...
    const userPrompt = `업종: meat | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat} | 상황: ${situation} | 목적: ${purpose} | 모드: ${validMode}
${prevBlock}
---
...
${richPrompt}`;
  ...
  console.log(`[meat][ALT] mode=${validMode} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  let title = overrideTitle || buildMeatTitle(treatmentData, region, situation, purpose, seoData, validMode, _titleSuffixOpts);
  // 의료·카페 어휘가 제목에 새면 차단
  ...
  assembled += "\n\n" + buildMeatHashtags(treatmentData, region, situation, purpose, validMode);

  // ── 최종 클리닝 (조립 후 누수 방지) ──────
  // ★ FREEZE 예외 #2 (제목 보존) — meat 전용
  ...
  //         제목은 buildMeatTitle 단계에서 이미 menu 포함 보장됨
  {
    ...
    let cleanedBody = cleanMeatText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanMeatText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  console.log(`[meat] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  // 경고
  if (qc.adPatternCount > 0)
    console.error(`[meat] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[meat] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[meat] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[meat] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[meat] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[meat] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[meat] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
