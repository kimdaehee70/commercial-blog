// ============================================================
// pages/api/generateNeuroV2.js — 신경외과 정보형 V2 핸들러 (완전 독립)
// ⚠️ clinic / derma / dental / ent / oriental / ortho / pain 절대 참조 금지
// [V2 Purpose · 2026-07-12] 정보형 단일(commercial 톤). neuro v1(후기형)은 generateNeuro.js — 무손상 A/B 보존.
//   engineBootstrap neuro 래퍼가 mode==='purpose'일 때만 이 핸들러로 위임(Pilot Gate 유지).
// [기준 엔진] generateOrthoV2.js(정식 승격본) One Axis 복제. 신경외과 데이터만 치환.
// [화이트리스트 없음] neuro-data 전 시술(24종)이 신경외과 자산 → ALLOWED 필터 불필요.
// 골격: 핸들러 → 제목 → Purpose 7섹션 루프 → callGPT → 정보형 후처리 → QC → 응답
// [self-contained] generateNeuro(v1) import 없음. 유틸 전부 내장. v1 의존성 0 → FREEZE 안전.
// ============================================================

import { NEURO_TREATMENTS }                                                     from "../../lib/neuro-data";
import { buildNeuroPromptV2, NEURO_SYSTEM_PROMPT_V2, getNeuroImageAltsV2, NEURO_DIRECTION } from "../../lib/neuro-v2-prompts";
import { NEURO_FLOW_ENGINE_V2, NEURO_TREATMENT_OVERRIDES_V2 }                    from "../../lib/neuro-v2-playConfig";
...
// ------------------------------------------------------------
// 계열 키워드 (제목·activeKeyword 보조) — 신경외과 부위/증상 축
// ------------------------------------------------------------
const SITE_KEYWORDS = [
  "허리", "목", "경추", "요추", "척추", "엉덩이", "다리", "종아리",
  "팔", "손", "손목", "손가락", "팔꿈치", "발", "발바닥",
  "얼굴", "머리", "뒷머리", "눈", "귀",
  "저림", "신경통", "방사통",
];

// ------------------------------------------------------------
// 계열 혼용 차단 MAP — 신경외과 카테고리 교차 침범 방지
//   treatment.cat → focus/forbid.
// ------------------------------------------------------------
const NEURO_CAT_FOCUS = {
  "척추·디스크":      { focus: "척추·디스크·신경 압박 관점",          forbid: "두통·어지럼·이명·손저림·얼굴 통증" },
  "두통·신경통":      { focus: "두통·신경통·얼굴 통증 관점",          forbid: "허리·다리 저림·보행 거리·계단·손목" },
  "신경차단·통증":    { focus: "신경차단·비수술 시술 관점",           forbid: "도수치료·프롤로·PRP·수술 단정" },
  "말초신경·손저림":  { focus: "말초신경·손저림·감각 저하 관점",      forbid: "허리·보행 거리·두통·어지럼" },
  "어지럼·뇌신경":    { focus: "어지럼·뇌신경·감별 검사 관점",        forbid: "허리·무릎·보행 거리·손목터널" },
};

function getNeuroFocus(treatment) {
  const m = NEURO_CAT_FOCUS[treatment?.cat];
  if (!m) return null;
...
// ------------------------------------------------------------
function buildNeuroTitleV2(treatment, region, activeKeyword) {
  const ak = activeKeyword || treatment.name;
  return region + " " + ak + ", 신경외과 진료 전 알아둘 점";
}

// ------------------------------------------------------------
// LLM 호출 (OpenAI gpt-4o) — ortho/clinic/dental v2 동형
// ------------------------------------------------------------
...
// 정보형 후처리(경량 안전망)
// ------------------------------------------------------------
function cleanNeuroTextV2(text, keyword) {
  let t = String(text || "");
  ...
  t = t
    ...
    .replace(/완벽(히|하게|한)/g, "")
    .replace(/(저림이|통증이|증상이|어지럼이)\s*(사라|없어)졌(어요|습니다|다)/g, "$1 변화하는지 여부는 상태에 따라 다릅니다")
  ...
  // 조사 오류(질환명 직결) 교정 — activeKeyword 받침 기반
  if (keyword) {
...
// QC
// ------------------------------------------------------------
function runNeuroQCV2(text, keyword, fullKeyword) {
  ...
  const hasAllSections = NEURO_FLOW_ENGINE_V2.sections.every(s =>
...
export default async function handler(req, res) {
  ...
  // ── 시술 선택 (neuro-data 전체가 신경외과 자산 → 화이트리스트 필터 없음) ──
  const reqId = program?.id;
  const treatment =
    (reqId && NEURO_TREATMENTS.find(t => t.id === reqId)) ||
    NEURO_TREATMENTS.find(t => t.id === "neuro_disc") ||
    NEURO_TREATMENTS[0];
  ...
  const focus   = getNeuroFocus(treatment);
  const compare = (NEURO_DIRECTION[tid]?.compare) || treatment.compareWith || "다른 치료";

  const finalTitle = buildNeuroTitleV2(treatment, region, activeKeyword);
  ...
  const systemPrompt =
    ...
    "  - 효과 단정 금지: \"확실히/100%/완치/나았다/저림이 사라졌다\" → \"상태에 따라 안내됩니다\"\n" +
    ...
    "## 치료는 어떤 기준으로 결정되나요? (180~260자 — 의사결정 흐름, 핵심 축)\n" +
    ...
    "[나열 제약]\n" +
    "- 검사 종류는 한 번에 최대 2개, 치료 종류는 최대 2개까지만 언급.\n" +
    "- 같은 검사·치료 내용을 반복 설명 금지. 한방·한의 치료, 도수치료·프롤로·PRP는 다루지 않는다.\n" +
    ...
    "- 최소 1개 단락에서 실제 공간 장면 묘사 포함 (예: \"진료실에서 모니터로 MRI 영상을 함께 보며 설명을 듣는다\")\n" +
    "- 동일 패턴(\"진행된다/작용한다/확인된다\") 3회 이상 연속 금지\n\n" +
    "[출력 형식]\n" +
    "마크다운 / 제목(# 시작) / 섹션(## 유지) / 마지막 해시태그\n\n" +
    "[금지]\n" +
    "❌ 같은 문단·구조 반복\n" +
    "❌ 계열 혼용(척추·두통·손저림·어지럼 교차 금지)\n" +
  ...
  const overrides = NEURO_TREATMENT_OVERRIDES_V2[tid] || {};
  const SECTION_CAP = {};
  for (const sec of NEURO_FLOW_ENGINE_V2.sections) {
  ...
  for (const sec of NEURO_FLOW_ENGINE_V2.sections) {
    ...
    const basePrompt = buildNeuroPromptV2(sec.key, treatment, region, "commercial");
  ...
  // ── 후처리 ──
  result = cleanNeuroTextV2(result, activeKeyword);
  ...
  // ── QC ──
  const qc = runNeuroQCV2(result, keyword, fullKeyword);

  console.log("[NEURO-V2] tid=" + tid + " site=" + (detectedSite || "-") + " kw=" + activeKeyword);
