// ╔══════════════════════════════════════════════════════════╗
// ║ pages/api/generateEndoV2.js — 내분비내과 V2 Purpose        ║
// ║ generateCardV2 골격 복제(치환 3단 fix 포함) + 내분비 치환.  ║
// ║ self-contained — v1 없음(신규 업종, V2 단독).             ║
// ║ ★ 고유 축: decisionAxis 1필드로 4섹션 분기                 ║
// ║   exam(5종) = 검사 선택 기준 / disease(9종) = 관리 결정    ║
// ║ SOP v4.2 PATCH-07: locationBlock 후단 1줄.                ║
// ║ ⚠ 관측 전. FREEZE 아님.                                   ║
// ╚══════════════════════════════════════════════════════════╝

import { ENDO_TREATMENTS } from "../../lib/endo-data";
import {
  buildEndoV2Prompt,
  ENDO_V2_SYSTEM_PROMPT,
  getEndoV2ImageAlts,
  getEndoV2Direction,
} from "../../lib/endo-v2-prompts";
...
// ============================================================
const ENDO_CAT_FOCUS = {
  "검사":     { note: "검사 선택 기준과 확인 범위가 축입니다. 질환 관리 설명으로 흐르지 않습니다." },
  "당뇨":     { note: "수치 확인 → 다른 대사 위험 요인 확인 → 생활 관리 → 약물 접근 → 정해진 간격의 경과 확인 순의 판단 축입니다. 만성 관리 구조를 유지하고, 완치·단기 종료로 서술하지 않습니다." },
  "갑상선":   { note: "호르몬 수치가 어느 방향으로 기울었는지가 중심 축. 수치와 증상을 함께 해석하고, 재확인 간격으로 조절해 나가는 흐름이 이어집니다. 수술·조직검사는 위임 수준만." },
  "골대사":   { note: "증상이 아니라 수치로 확인되는 영역이 중심 축. 골밀도 수치와 골절 위험 요인을 종합해 생활 관리와 약물 접근이 나뉩니다." },
  "호르몬":   { note: "흔한 원인이 먼저 배제된 뒤 호르몬 축의 균형을 확인하는 흐름이 축. 채혈 조건에 따라 값이 달라질 수 있다는 점을 함께 서술합니다." },
};
...
// ============================================================
const ENDO_V2_FORBIDDEN = [
  "완치됩니다", "재발하지 않습니다", "반드시 좋아집니다", "확실히", "무조건",
  "정상입니다", "이상 없습니다", "당뇨입니다", "암입니다", "악성입니다", "진단됩니다",
  "최고", "완벽", "최신 장비", "잘하는 곳", "무통", "강력 추천", "후회 없", "꼭 받으세요", "예약하세요",
  "정리하면", "결론적으로", "따라서", "체계적인", "살펴보겠습니다",
  "절개", "전신마취", "봉합", "수술 방법", "갑상선 절제", "고주파절제", "세침흡인",
  "실명", "투석", "절단", "사망률", "생존율",
  "감량 보장", "살빼는", "다이어트 주사", "식욕억제제", "단기간에",
...
function countViolations(text) {
  let n = 0;
  ENDO_V2_FORBIDDEN.forEach(w => { if (text.includes(w)) n++; });
...
function finalClean(text) {
  ...
  t = t.replace(/당뇨입니다|암입니다|악성입니다/g, "진료에서 확인하는 항목입니다");
  // 불안 조장 (합병증 공포 서술 차단)
  t = t.replace(/사망률\S*|생존율\S*/g, "경과");
  t = t.replace(/실명\S*|투석\S*|절단\S*/g, "합병증 확인 항목");
  // 다이어트 광고형 차단 (비만 주제 방어)
  t = t.replace(/감량 보장\S*|살빼는\S*|다이어트 주사\S*|식욕억제제\S*/g, "");
  t = t.replace(/\d+\s?kg\s?감량|\d+\s?주 만에|단기간에/g, "");
  // 광고
...
function buildTitle(treatment, region) {
  ...
  const d    = getEndoV2Direction(treatment.id);
  const isExam = d.decisionAxis === "exam";
  const t = isExam
    ? region + " " + name + " 정보｜어떤 경우에 검토되는지 안내"
    : region + " " + name + " 정보｜검사·관리 결정 기준 안내";
  return t.trim();
}
...
function buildHashtags(name, region) {
  ...
  return "#" + reg + base + " #" + reg + "내분비내과 #" + base + "정보 #호르몬대사진료안내";
}
...
export default async function handler(req, res) {
  ...
  const treatment =
    ENDO_TREATMENTS.find(t => t.id === program?.id) ||
    ENDO_TREATMENTS[0];
  ...
  const dir     = getEndoV2Direction(tid);
  const axis    = dir.decisionAxis === "exam" ? "exam" : "disease";
  const catNote = (ENDO_CAT_FOCUS[treatment.cat] || { note: "확인 항목과 결정 기준을 중심으로 서술합니다." }).note;

  const finalTitle = buildTitle(treatment, region);
  let result = "# " + finalTitle + "\n\n";

  const imgAlts = getEndoV2ImageAlts(treatment, region, activeKeyword);
  ...
  // ── 섹션 루프 ──
  for (const sec of ENDO_V2_FLOW_ENGINE.sections) {
    const basePrompt = buildEndoV2Prompt(sec.key, treatment, region);
