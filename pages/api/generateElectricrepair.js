...
//     조회 순서: treatment.infoKey → treatment.cat → 기본(leak).
//     기존 8메뉴는 data.js infoKey 부여로 매핑 100% 동일. 신규 메뉴는 infoKey만 지정하면 됨
//     → 이후 메뉴 확장은 data.js 단독 작업(핸들러 무수정).
function pickInfoBlock(treatment) {
  return (
    ELECTRICREPAIR_INFO_BLOCKS[treatment.infoKey] ||
    ELECTRICREPAIR_INFO_BLOCKS[treatment.cat] ||
    ELECTRICREPAIR_INFO_BLOCKS.leak
  );
}
...
function buildTitle(region, treatment) {
  const _t = buildIntentTitleOrNull(region, treatment, "electricrepair");
  if (_t) return _t;
...
export default async function handleElectricrepair(req, res) {
  try {
    ...
    const PHOTO_ALT = {
      ...
      process: "진행 절차 안내", manage: "작업 후 확인 안내", closing: "전기수리 관리·안전 확인 안내",
    };
