// pages/api/generateHomecare.js
// 방문요양 생성 핸들러. 복사 베이스: generateDaycare.js
// 기능 모듈(cleanText/removeDuplicates/insertInfoBlock/QC)은 업종 공통 — 복사 사용.
// 비의료 → injectExamValue(수치 강제) 생략/대체.
// ★ narrative 분리: 시설 톤(모시다·다니다·송영) → 가정 방문 톤(찾아가다·방문·동행).
...
import { HOMECARE_PLAY_CONFIG } from "../../lib/homecare-playConfig.js";
import {
  HOMECARE_TREATMENTS,
  HOMECARE_PHOTO_POOL, // ⚠️ DEAD CODE (핸들러 미참조 — getImageAlts가 내부 사용): export 보존
} from "../../lib/homecare-data.js";
import {
  SYSTEM_PROMPT,
  FORBIDDEN,
  buildPrompt,
...
function injectExamValue(text) {
  // 방문요양은 의료 수치 없음. 제도 수치(등급/부담률)는 정보블럭이 담당.
...
function runQC(text, fullKeyword) {
  // [narrative 정합] 정보블럭/서비스 키워드는 가정 방문 기준(이용대상·본인부담금·선택 기준·방문목욕)
  const hasInfo = /이용대상|본인부담금|선택 기준|방문목욕/.test(text);
  const kwCount = fullKeyword
    ? (text.match(new RegExp(fullKeyword.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []).length
    : 0;
  const fullKwCount = (text.match(/방문요양센터|방문요양/g) || []).length;
...
// ───────── 핸들러 ─────────
export default async function handleHomecare(req, res) {
  try {
    ...
    const treatment =
      HOMECARE_TREATMENTS.find((t) => t.id === program?.id) ||
      HOMECARE_TREATMENTS.find((t) => t.name === program?.name) ||
      HOMECARE_TREATMENTS[0];

    const fullKeyword = `${region} 방문요양`;
    const systemPrompt = SYSTEM_PROMPT.replace(/\{region\}/g, region);
    const userPrompt = buildPrompt({ treatment, region, storeName });

    // ── GPT 호출 (단일 호출 — homecare-prompts 설계 그대로. 섹션 루프 미이식) ──
    ...
    // 마무리 해시태그 — 가정 방문 narrative 기준
    out += `\n\n#${region}방문요양 #${region}방문요양센터 #장기요양 #재가요양 #가족요양`;
    ...
    console.error("[generateHomecare] error:", e);
    return res.status(500).json({ error: "homecare_generation_failed", detail: String(e) });
  }
}

function pickTitle(treatment, region) {
  const pats = treatment.titlePatterns || [];
  const t = pats[Math.floor(Math.random() * pats.length)] || "{region} 방문요양 안내";
