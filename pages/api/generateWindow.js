// ============================================================
// pages/api/generateWindow.js — 창호시공(window) 엔진 핸들러
// 정보형 + 출장 시공 안내형({region} 창호 시공 업체 1인칭).
// 후기형·내돈내산·고객사례·추천·보장·최저가·비용유도·수명약속 금지.
// engineBootstrap에서 register("window", handleWindow)로 편입.
//
// 복제 베이스: generateTile.js (섹션루프형). Runtime/FLOW/Handler 구조 무변경.
// waterproof 대비 변경 1점:
//   ① 해시태그 어휘 — 창호 검색축(창호/샷시/시공)으로 교체. dedup 는 동형 Set 유지.
...
// ── 마무리 해시태그 ─────────────────────────────
//   ★ 메뉴명이 "교체/시공"으로 끝나 `#{rg}{k}` 와 `#{rg}창호시공` 이 겹칠 수 있다 → Set 일괄 정리.
...
//   → 메뉴 확장은 window-data.js 단독 작업. 핸들러를 다시 열지 않는다.
function pickInfoBlock(t) {
  const B = (typeof getInfoBlocks === "function" ? getInfoBlocks(t.cat) : null)
    || WINDOW_INFO_BLOCKS;
  return B[t.infoKey] || B[t.cat] || WINDOW_INFO_BLOCKS.prebook;
}
...
//   ★ window 는 siteBlock 미사용(useSite:false) → 단지명 분기 없음.
...
function buildTitle(region, treatment) {
  const _t = buildIntentTitleOrNull(region, treatment, "window");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment);
}

export default async function handleWindow(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
      symptom,                                   // 접수된 발견(외풍·결로 등) — 미입력 시 무영향
      part,                                      // 원인군(frame·glass·leak·seal·insul·anchor)
    } = req.body;

    // 창호시공 = 출장 시공 업종 → 고정 사업장 위치블록 미노출(PATCH-07)
    const _locStore = {};
    ...
    const treatment =
      WINDOW_TREATMENTS.find((t) => t.id === program?.id) ||
      WINDOW_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `창호시공 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    const rgKwCount = region
      ...
      : 0;
    console.log(`[QC][window] 메뉴(${kw}): kw ${kwCount} / 지역+kw 결합 ${rgKwCount} (3 이하 정상)`);
    console.log(`[QC][window] 글자수: ${content.length}`);
    console.log(`[QC][window] 발견: ${sym || "미입력"} / 원인군: ${pt ? pt.label : "미입력"}`);
    console.log(`[QC][window] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[window] 오류:", err);
    return res.status(500).json({ error: err.message || "창호시공 글 생성 오류" });
  }
}
