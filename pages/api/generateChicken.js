// ============================================================
// generateChicken.js — 치킨 블로그 생성기 v1.0 (Chicken Engine 독립, Japanese 복사 베이스)
//
...
import {
  CHICKEN_TREATMENTS, CHICKEN_BLOCK_MAP, CHICKEN_CROSS_CAT_BLOCK,
  CHICKEN_TITLE_MIDDLE, CHICKEN_TITLE_SUFFIX,
  CHICKEN_TITLE_SCENE, CHICKEN_TITLE_SCENE_BY_CATEGORY,
} from "../../lib/chicken-data";
import {
  buildChickenPrompt, getChickenDirection,
} from "../../lib/chicken-prompts";
import {
  getChickenSections,
} from "../../lib/chicken-playConfig";
import {
...
const CHKN_FORBIDDEN_COMMERCIAL = [
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
// ============================================================
const CHKN_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("chicken");
  } catch (e) {
    return [
      ...CHICKEN_BLOCK_MAP.medical,
      ...CHICKEN_BLOCK_MAP.cafe,
      ...CHICKEN_BLOCK_MAP.study,
      ...CHICKEN_BLOCK_MAP.ad,
    ];
  }
})();
...
function buildChickenTitle(treatment, region, situation, purpose, seoData, mode, titleSuffixOpts) {
  ...
  if (mode === "commercial") {
    ...
    const pools = {
      titlePatterns: [],
      SCENE:        CHICKEN_TITLE_SCENE,
      SCENE_BY_CAT: CHICKEN_TITLE_SCENE_BY_CATEGORY,
      MIDDLE:       CHICKEN_TITLE_MIDDLE,
      SUFFIX:       CHICKEN_TITLE_SUFFIX,
    };
...
// ============================================================
function buildJapaneseHashtags(treatment, region, situation, purpose, mode) {
  ...
  // ★ [세션6→7] cat 영문값 해시태그 노출 차단 + seasoned 계열 라벨 오염 차단.
  //   기존 버그: seasoned 계열(마늘/허니/고추 등) 전체가 catKw='양념치킨'을 공유
  //     → menu='마늘치킨'인데 #양념치킨 오삽입(실측). cat 라벨은 계열 대표어일 뿐 메뉴어 아님.
  //   원칙: 해시태그는 menu(검색의도) 우선. catTag은 '계열 상위검색어' 보강 용도로만,
  //     menu가 상위어('치킨')를 이미 담으면 #${reg}${menu}가 그 역할 → catTag 제거.
  //   → 결과: 마늘치킨/허니치킨 등 '치킨'으로 끝나는 menu = catTag 미생성(오염 0).
  //     닭강정 등 '치킨' 미포함 menu만 #${reg}치킨 보강(정상).
  const CAT_FALLBACK = "치킨";   // 계열 무관 상위 검색어 단일화 (cat 라벨 강제 폐기)
  const catTag = menu.includes(CAT_FALLBACK) ? "" : `#${reg}${CAT_FALLBACK}`;
...
// ============================================================
function cleanChickenText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  const direction = getChickenDirection(treatment, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  let result = text;

  const removeList = [...CHKN_FORBIDDEN_BASE, ...CHKN_FORBIDDEN_AI, ...CHKN_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...CHKN_FORBIDDEN_COMMERCIAL);
  // ★ 계열 교차차단 — 현재 메뉴의 cat 기준 "다른 계열 핵심 표현"만 제거 (taste 결 충돌 방지)
  //   CHICKEN_CROSS_CAT_BLOCK[cat] = 그 계열에서 차단할 타계열 묘사어. cat 미매칭 시 무동작.
  {
    const _cat = treatment.cat || treatment.catRef || "";
    const _ccb = (CHICKEN_CROSS_CAT_BLOCK && CHICKEN_CROSS_CAT_BLOCK[_cat]) || [];
    if (_ccb.length) removeList.push(..._ccb);
  }
  ...
  if (mode === "commercial") {
    ...
    if (menu) {
      ...
      // ★ [세션6] "후라이드치킨 치킨" 중복 제거 — menu가 이미 "…치킨"으로 끝날 때 뒤따르는 카테고리어 "치킨" 삭제
      //   GPT가 메뉴명 뒤에 카테고리어를 덧붙이는 패턴(후라이드치킨 치킨은/이/…). 조사·문장부호 경계만 잡고 "치킨무/치킨집" 등은 보존.
      if (/치킨$/.test(menu)) {
        result = result.replace(new RegExp(`(${mEsc})\\s+치킨(?=[은는이가을를도만의와과에로라랑나며\\s.,!?)、]|$)`, "g"), "$1");
      }
  ...
  if (particleErrorCount > 0) {
    console.log(`[chicken][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[chicken][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[chicken][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[chicken][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  ...
  let efficacyFixCount = 0;
  {
    const effConv = [
      // 숙취/해장 효능 단정 → 행동 표현
      [/숙취\s*해소에\s*도움(?:을)?\s*(?:줄\s*수\s*있다|준다|된다)\.?/g, "가볍게 찾는 사람들도 있다."],
      [/숙취\s*해소에\s*좋다\.?/g, "가볍게 찾는 사람들도 있다."],
      [/해장에\s*(?:좋다|효과적이다|그만이다)\.?/g, "가볍게 찾는 경우도 있다."],
  ...
  if (particleDupCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[chicken][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }
  ...
  //   "라멘 육수" 중복 → "육수" / "초밥 조합은 초밥의 …" → "이러한 구성은 …"
  let phraseDupCount = 0;
  {
    ...
    if (menu) {
      const mE = menu.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      // "라멘 육수" 류 (메뉴명 직후 그 메뉴의 핵심어가 다시 오는 경우만)
      result = result.replace(new RegExp(`${mE}\\s+(육수|소스|국물|튀김옷|간장)\\s`, "g"), "$1 ");
  ...
  let generalizeFixCount = 0;
  {
    ...
    result = result
      ...
      .replace(/일부 치킨집은/g, "매장에 따라")
      .replace(/일부 치킨집에서는/g, "매장에 따라")
      .replace(/매장은 대부분 좌석이 넉넉하여/g, "매장에 따라 좌석 구성은 다를 수 있으며")
      .replace(/대부분 좌석이 넉넉하여/g, "좌석 구성은 매장에 따라 다를 수 있으며");
    generalizeFixCount = (before.match(/일부 가게에서는|일부 치킨집은|일부 치킨집에서는|매장은 대부분 좌석이 넉넉하여|대부분 좌석이 넉넉하여/g) || []).length;
  }
  ...
  let stepG2Count = 0;
  {
    ...
    // G2-e. "전문점의 치킨집…" 군더더기(전문점+치킨집 이중 노출) → "치킨집+어미"
    //   기존: '들은'만 처리 → '에서는/에서/은/는' 등 누락(실측: "마늘치킨 전문점의 치킨집에서는")
    //   보완: "○○ 전문점의 치킨집<어미>" 어미 보존 단일화. 복수/무어미는 '에서는' 기본.
    result = result
      .replace(
        /[가-힣]{2,8}\s*전문점의\s*치킨집(에서는|에서|에는|은|는|들은|들이|마다|별로|의)?/g,
        (m, josa) => `치킨집${josa && josa !== "들은" && josa !== "들이" ? josa : "에서는"}`
      )
      .replace(/치킨집들은(?=\s)/g, "치킨집에서는");

    // G2-e2. "전문점의 (치킨 )?전문점…" 명사 중복 일반화 (실측: "로스트치킨 전문점의 치킨 전문점들은")
    //   '전문점의'가 앞에 명시된 경우만 처리 → 뒤 '(치킨 )?전문점<어미>'를 단일화.
    //   조건 한정(전문점의 선행)이라 정상문 손상 위험 없음. 어미 보존, 복수/무어미는 '에서는'.
    result = result
      .replace(
        /[가-힣]{2,8}\s*전문점의\s*(?:치킨\s*)?전문점(에서는|에서|에는|은|는|들은|들이|마다|별로|의)?/g,
        (m, josa) => `치킨 전문점${josa && josa !== "들은" && josa !== "들이" ? josa : "에서는"}`
      );
    ...
    result = result
      .replace(/치킨집\s*전문점에서는/g, "치킨집에서는")
      .replace(/치킨\s*전문점에서는/g, "치킨집에서는");

    if (before !== result) stepG2Count = 1;
  }
  if (stepG2Count > 0) console.log(`[chicken][QC] ★ STEP G2 잔여정정: 매장안내참고/이거의/한그릇가/메뉴명조합/전문점중식당들은/가게마다/방문시중복/맛조사/중식당전문점`);

  if (transFixCount > 0)     console.log(`[chicken][QC] ★ STEP G 번역체 정정: ${transFixCount}건 (그것의/그것을/그것이 → 메뉴)`);
  if (phraseDupCount > 0)    console.log(`[chicken][QC] ★ STEP G 어절중복 제거: ${phraseDupCount}건 (메뉴명 군더더기 인접반복)`);
  if (generalizeFixCount > 0) console.log(`[chicken][QC] ★ STEP G 일반화 안전화: ${generalizeFixCount}건 (일부 가게/대부분 → 매장에 따라)`);
  if (repeatCapCount > 0)    console.log(`[chicken][QC] ★ STEP G 과반복 제한: ${repeatCapCount}건 (3회째+ 축약 · 첫 2회·메뉴명 보존)`);

  // 외부 QC 집계용
  cleanChickenText.__lastParticleDup = particleDupCount;
  cleanChickenText.__lastSubjParticleFix = subjParticleFixCount;
  cleanChickenText.__lastEfficacyFix = efficacyFixCount;
  cleanChickenText.__lastStoreRefFix = storeRefFixCount;
  cleanChickenText.__lastMenuDup = menuDupCount;
  cleanChickenText.__lastSuperlativeFix = superlativeFixCount;
  cleanChickenText.__lastUnitFix = unitFixCount;
  cleanChickenText.__lastDupSentence = dupSentenceCount;
  cleanChickenText.__lastParticleErrors = particleErrorCount;
  cleanChickenText.__lastMenuMeta = menuMetaCount;
  cleanChickenText.__lastAdEvalFixed = adEvalFixed;
  cleanChickenText.__lastPlaceholderFix = placeholderFixCount;
  cleanChickenText.__lastTransFix = transFixCount;
  cleanChickenText.__lastPhraseDup = phraseDupCount;
  cleanChickenText.__lastGeneralizeFix = generalizeFixCount;
  cleanChickenText.__lastRepeatCap = repeatCapCount;
  cleanChickenText.__lastStepG2 = stepG2Count;
...
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  const adEvalFixed = (typeof cleanChickenText.__lastAdEvalFixed === "number")
    ? cleanChickenText.__lastAdEvalFixed : 0;

  // cleanChickenText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanChickenText.__lastParticleErrors === "number")
    ? cleanChickenText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanChickenText.__lastMenuMeta === "number")
    ? cleanChickenText.__lastMenuMeta : 0;
  ...
  console.log(`[chicken][QC] 메뉴블럭: ${hasMealBlock}`);
  console.log(`[chicken][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  console.log(`[chicken][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[chicken][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[chicken][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[chicken][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[chicken][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[chicken][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[chicken][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[chicken][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[chicken][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[chicken][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[chicken][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
    console.log(`[chicken][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[chicken][QC] 가격명시(commercial 위반): ${priceCount}건`);
  }
...
// 메인 핸들러
// ============================================================
export default async function handleJapanese(req, res) {
  const {
  ...
  console.log(`[chicken] mode: ${validMode} | 상황: ${situation || "(미지정)"} | 목적: ${purpose || "(미지정)"}`);
  console.log(`[chicken][BUILD] chicken-v1.0-base (Chicken Engine 독립 · japanese v1.0 베이스 · cat 4계열 · 배포검증: 이 줄이 안 보이면 구버전 실행 중)`);

  // ── restaurant 조합 검증 ─────────────────────────────────
  const CHKN_IDS = CHICKEN_TREATMENTS.map(t => t.id);
  const isRest = CHKN_IDS.includes(program.id) || program.industry === "chicken";
  if (!isRest) {
    console.error(`[chicken] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  const treatmentData = CHICKEN_TREATMENTS.find(t => t.id === program.id)
    || CHICKEN_TREATMENTS[0];
  const cat  = treatmentData?.cat || "fried";
  ...
  console.log(`[chicken] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getChickenDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[chicken] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getChickenSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";

  for (const sec of SECTIONS) {
    const richPrompt = buildChickenPrompt(sec.key, treatmentData, region, {
    ...
    const userPrompt = `업종: chicken | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat} | 상황: ${situation} | 목적: ${purpose} | 모드: ${validMode}
${prevBlock}
---
...
${richPrompt}`;
  ...
  const altList = SECTIONS.slice(0, _altCount).map(sec => {
    const label = _CHKN_ALT_BY_KEY[sec.key] || "메뉴 사진";
    return `[이미지: ${label}]`;
  });
  console.log(`[chicken][ALT] mode=${validMode} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  let title = overrideTitle || buildChickenTitle(treatmentData, region, situation, purpose, seoData, validMode, _titleSuffixOpts);
  // 의료·카페 어휘가 제목에 새면 차단
  const TITLE_BLOCK = /시술|치료|성형|진료|병원|카공|콘센트|드립|라떼아트/;
  if (TITLE_BLOCK.test(title)) {
    title = validMode === "commercial"
      ? `${region} ${menu} 어떤 맛이 특징일까｜선택 전 참고`
      : `${region} ${menu} 다녀온 솔직 후기`;
  }
  if (!title.includes(region) || !title.includes(menu)) {
    title = validMode === "commercial"
      ? `${region} ${menu} 어떤 맛이 특징일까`
  ...
  assembled += "\n\n" + buildJapaneseHashtags(treatmentData, region, situation, purpose, validMode);
  ...
  //         제목은 buildChickenTitle 단계에서 이미 menu 포함 보장됨
  {
    ...
    let cleanedBody = cleanChickenText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanChickenText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  console.log(`[chicken] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  // 경고
  if (qc.adPatternCount > 0)
    console.error(`[chicken] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[chicken] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[chicken] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[chicken] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[chicken] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[chicken] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[chicken] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
