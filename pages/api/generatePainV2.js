// ============================================================
// pages/api/generatePainV2.js — 통증의학과 정보형 V2 핸들러 (완전 독립)
// ⚠️ clinic / derma / dental / ent / oriental / ortho / neuro 절대 참조 금지
// [V2 Purpose · 2026-07-12] 정보형 단일(commercial 톤). pain v1(후기형)은 generatePain.js — 무손상 A/B 보존.
//   engineBootstrap pain 래퍼가 mode==='purpose'일 때만 이 핸들러로 위임(Pilot Gate).
// [기준 엔진] generateNeuroV2.js(= ortho-v2 계열) One Axis 복제. 통증의학과 데이터만 치환.
// [화이트리스트 없음] pain-data 전 시술(28종)이 통증의학과 자산 → ALLOWED 필터 불필요.
// 골격: 핸들러 → 제목 → Purpose 7섹션 루프 → callGPT → 정보형 후처리 → QC → 응답
// [self-contained] generatePain(v1) import 없음. 유틸 전부 내장. v1 의존성 0 → FREEZE 안전.
// ============================================================

import { PAIN_TREATMENTS }                                                    from "../../lib/pain-data";
import { buildPainPromptV2, PAIN_SYSTEM_PROMPT_V2, getPainImageAltsV2, PAIN_DIRECTION } from "../../lib/pain-v2-prompts";
import { PAIN_FLOW_ENGINE_V2, PAIN_TREATMENT_OVERRIDES_V2 }                     from "../../lib/pain-v2-playConfig";
...
// ------------------------------------------------------------
const SITE_KEYWORDS = [
  "허리", "목", "경추", "요추", "척추", "엉덩이", "골반", "꼬리뼈",
  "무릎", "어깨", "다리", "종아리", "허벅지",
  "팔", "손", "손목", "손가락", "팔꿈치", "발", "발목", "발바닥", "뒤꿈치",
  "머리", "뒷머리", "턱",
  "저림", "신경통", "방사통", "결림", "뭉침",
];

// ------------------------------------------------------------
// 계열 혼용 차단 MAP — 통증의학과 카테고리 교차 침범 방지
//   treatment.cat → focus/forbid.
// ------------------------------------------------------------
const PAIN_CAT_FOCUS = {
  "척추·디스크":  { focus: "척추·신경 압박·저림 경로 관점",            forbid: "무릎·어깨·턱·발바닥·두통·전신 통증" },
  "관절·인대":    { focus: "관절·인대·힘줄 손상과 가동 범위 관점",     forbid: "허리 저림·보행 거리·두통·전신 통증" },
  "재활·물리":    { focus: "근육·근막·자세와 재활 관점",               forbid: "두통·전신 통증·수술 단정" },
  "두통·신경":    { focus: "두통·신경병증성 통증 관점",                forbid: "무릎·어깨·발바닥·보행 거리·자세 교정" },
  "족부·하지":    { focus: "발·발목·손목·꼬리뼈 등 국소 부위 관점",    forbid: "허리 저림·두통·전신 통증·보행 거리" },
};

function getPainFocus(treatment) {
  const m = PAIN_CAT_FOCUS[treatment?.cat];
  if (!m) return null;
...
// ------------------------------------------------------------
function buildPainTitleV2(treatment, region, activeKeyword) {
  const ak = activeKeyword || treatment.name;
  return region + " " + ak + ", 통증의학과 진료 전 알아둘 점";
}
...
// 정보형 후처리(경량 안전망)
// ------------------------------------------------------------
function cleanPainTextV2(text, keyword) {
  let t = String(text || "");
  ...
  t = t
    ...
    .replace(/완벽(히|하게|한)/g, "")
    .replace(/(통증이|저림이|증상이|결림이)\s*(사라|없어)졌(어요|습니다|다)/g, "$1 변화하는지 여부는 상태에 따라 다릅니다")
...
// QC
// ------------------------------------------------------------
function runPainQCV2(text, keyword, fullKeyword) {
  ...
  const hasAllSections = PAIN_FLOW_ENGINE_V2.sections.every(s =>
...
export default async function handler(req, res) {
  ...
  // ── 시술 선택 (pain-data 전체가 통증의학과 자산 → 화이트리스트 필터 없음) ──
  const reqId = program?.id;
  const treatment =
    (reqId && PAIN_TREATMENTS.find(t => t.id === reqId)) ||
    PAIN_TREATMENTS.find(t => t.id === "lumbar_nerve_block") ||
    PAIN_TREATMENTS[0];
  ...
  const focus   = getPainFocus(treatment);
  const compare = (PAIN_DIRECTION[tid]?.compare) || treatment.compareWith || "다른 치료";

  const finalTitle = buildPainTitleV2(treatment, region, activeKeyword);
  ...
  const systemPrompt =
    ...
    "## 진료실과 치료실에서 확인하는 과정 (150~250자 — 장면+방문 안내)\n" +
    "## 마무리 (100~150자)\n\n" +
    "[Purpose 원칙 — 설명보다 결정]\n" +
    ...
    "- 같은 검사·치료 내용을 반복 설명 금지. 한방·한의 치료(추나·침·한약)는 다루지 않는다. 수술 방법·과정 설명 금지(통증의학과 = 보존적 관리 → 중재적 시술 축).\n" +
    ...
    "- 최소 1개 단락에서 실제 공간 장면 묘사 포함 (예: \"진료실에서 통증 부위를 직접 눌러 확인하며 설명을 듣는다\")\n" +
    "- 동일 패턴(\"진행된다/작용한다/확인된다\") 3회 이상 연속 금지\n\n" +
    "[출력 형식]\n" +
    "마크다운 / 제목(# 시작) / 섹션(## 유지) / 마지막 해시태그\n\n" +
    "[금지]\n" +
    "❌ 같은 문단·구조 반복\n" +
    "❌ 계열 혼용(척추·관절·근막·두통·족부 교차 금지)\n" +
  ...
  const overrides = PAIN_TREATMENT_OVERRIDES_V2[tid] || {};
  const SECTION_CAP = {};
  for (const sec of PAIN_FLOW_ENGINE_V2.sections) {
  ...
  for (const sec of PAIN_FLOW_ENGINE_V2.sections) {
    ...
    const basePrompt = buildPainPromptV2(sec.key, treatment, region, "commercial");
  ...
  // ── 후처리 ──
  result = cleanPainTextV2(result, activeKeyword);
  ...
  // ── QC ──
  const qc = runPainQCV2(result, keyword, fullKeyword);

  console.log("[PAIN-V2] tid=" + tid + " site=" + (detectedSite || "-") + " kw=" + activeKeyword);
