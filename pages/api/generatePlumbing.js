// ============================================================
// pages/api/generatePlumbing.js — 수도설비(plumbing) 엔진 핸들러
// 정보형 + 범위·원인·진행절차·관리 안내 가이드형({region} 수도설비 업체 1인칭).
// 후기형·체험형·다녀왔습니다·고쳐드렸습니다·추천·즉시해결·당일해결·확실히해결·완벽해결·최저가·1등업체·출동·전후사진 금지.
// engineBootstrap에서 register("plumbing", handlePlumbing)로 편입.
// 복제 베이스: generateSewer.js (섹션루프형). 기존 업종 무수정.
...
import {
  PLUMBING_TREATMENTS,
  PLUMBING_INFO_BLOCKS,
} from "../../lib/plumbing-data";
import {
  buildSystemPrompt,
  buildUserPrompt,
  getImageAlts,
  FORBIDDEN,
} from "../../lib/plumbing-prompts";
import { PLUMBING_FLOW } from "../../lib/plumbing-playConfig";
...
// ── 화자 도입 (수도설비 업체 1인칭. 업체명 본문 미노출) ──
function buildAgentIntro(region, kw) {
  return `안녕하세요. ${region} 수도설비 업체입니다.\n오늘은 ${region} ${kw}를 안내해 드리겠습니다.`;
}

// ── 정보블럭 선택 (cat 기준 1:1 매핑, UNDEFINED 0) ──────────
//   data cat 8종: 수도설비/수도배관설치/수도배관수리/수도계량기설치/
//                 상하수도배관공사/싱크대수도설치/전기온수기설치/수도배관위치변경
//   INFO_BLOCKS 8종: pipetype/supplydrain/meter/aging/heatercheck/structure/maintain/checklist
function pickInfoBlock(treatment) {
  const cat = treatment.cat;
  switch (cat) {
    case "수도설비":         return PLUMBING_INFO_BLOCKS.checklist;
    case "수도배관설치":     return PLUMBING_INFO_BLOCKS.pipetype;
    case "수도배관수리":     return PLUMBING_INFO_BLOCKS.aging;
    case "수도계량기설치":   return PLUMBING_INFO_BLOCKS.meter;
    case "상하수도배관공사": return PLUMBING_INFO_BLOCKS.structure;
    case "싱크대수도설치":   return PLUMBING_INFO_BLOCKS.supplydrain;
    case "전기온수기설치":   return PLUMBING_INFO_BLOCKS.heatercheck;
    case "수도배관위치변경": return PLUMBING_INFO_BLOCKS.maintain;
    default:                 return PLUMBING_INFO_BLOCKS.checklist;
  }
}
...
function buildTitle(region, treatment) {
  const _t = buildIntentTitleOrNull(region, treatment, "plumbing");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment);
}

export default async function handlePlumbing(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
    } = req.body;
    // 수도설비 = 출장/현장출동 업종 → 고정 사업장 「찾아오시는 길」 미노출.
    ...
    const treatment =
      PLUMBING_TREATMENTS.find((t) => t.id === program?.id) ||
      PLUMBING_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `수도설비 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    const PHOTO_ALT = {
      intro: "수도설비 작업 범위 안내", axis1: "작업 범위 안내", axis2: "진행 절차 안내",
      axis3: "원인·배경 안내", axis4: "작업 후 확인 안내", closing: "수도설비 상담 안내",
    };

    const writtenSections = new Set();
    const sections = [];

    for (const sec of PLUMBING_FLOW) {
    ...
    console.log(`[QC][plumbing] 메뉴(${kw}): kw ${kwCount}`);
    console.log(`[QC][plumbing] 글자수: ${content.length}`);
    ...
    console.error("[plumbing] 오류:", err);
    return res.status(500).json({ error: err.message || "수도설비 글 생성 오류" });
  }
}
