// ╔══════════════════════════════════════════════════════════╗
// ║ pages/api/generateEntV2.js — 이비인후과 V2 Purpose         ║
// ║ generateCardV2 골격 복제 + 이비인후과 치환.                ║
// ║ self-contained — v1(generateEnt) 보존·미호출.             ║
// ║ ★ 고유 축: decisionAxis 1필드로 4섹션 분기                 ║
// ║   exam(4종) = 검사 선택 기준 / disease(10종) = 치료 결정   ║
// ║ SOP v4.2 PATCH-07: locationBlock 후단 1줄.                ║
// ║ ⚠ 관측 전. FREEZE 아님.                                   ║
// ╚══════════════════════════════════════════════════════════╝

import { ENT_V2_TREATMENTS } from "../../lib/ent-v2-data";
import {
  buildEntV2Prompt,
  ENT_V2_SYSTEM_PROMPT,
  getEntV2ImageAlts,
  getEntV2Direction,
} from "../../lib/ent-v2-prompts";
import { ENT_V2_FLOW_ENGINE } from "../../lib/ent-v2-playConfig";
...
const ENT_CAT_FOCUS = {
  ...
  "귀질환":   { note: "고막·외이도 진찰과 청력 확인이 판단의 출발점. 청력 저하 동반 여부에 따라 방향이 나뉩니다." },
  "코질환":   { note: "비내시경으로 점막과 통로를 확인하는 것이 출발점. 구조 원인인지 점막 원인인지를 나누는 흐름이 축입니다. 미용 목적 서술 금지." },
  "목질환":   { note: "목 진찰과 후두 소견 확인이 출발점. 반복 양상과 증상 지속 기간이 판단 축입니다." },
  "어지럼":   { note: "귀 안쪽 평형기관 원인인지 아닌지를 가르는 것이 축. 평형기관으로 설명되지 않으면 다른 진료과 연계 수준으로만 스칩니다." },
};
...
function countViolations(text) {
  let n = 0;
  ENT_V2_FORBIDDEN.forEach(w => { if (text.includes(w)) n++; });
...
function finalClean(text) {
  ...
  // 불안 조장·응급 유도
  t = t.replace(/영구 손상\S*|평생 안 들립니다|골든타임\S*/g, "경과");
  t = t.replace(/지금 바로 병원\S*|즉시 내원\S*|지체 없이|서둘러\S*|한시라도\S*/g, "");
  t = t.replace(/응급 대처\S*/g, "확인 항목");
  // 경계 침범 (미용 목적 서술 차단 — 비중격만곡은 기능 개선만)
  t = t.replace(/미용 목적\S*/g, "기능 개선 목적");
  // 광고
...
function buildTitle(treatment, region) {
  ...
  const d    = getEntV2Direction(treatment.id);
...
function buildHashtags(name, region) {
  ...
  return "#" + reg + base + " #" + reg + "이비인후과 #" + base + "정보 #귀코목진료안내";
}
...
export default async function handler(req, res) {
  ...
  const treatment =
    ENT_V2_TREATMENTS.find(t => t.id === program?.id) ||
    ENT_V2_TREATMENTS[0];
  ...
  const dir     = getEntV2Direction(tid);
  const axis    = dir.decisionAxis === "exam" ? "exam" : "disease";
  const catNote = (ENT_CAT_FOCUS[treatment.cat] || { note: "확인 항목과 결정 기준을 중심으로 서술합니다." }).note;

  const finalTitle = buildTitle(treatment, region);
  let result = "# " + finalTitle + "\n\n";

  const imgAlts = getEntV2ImageAlts(treatment, region, activeKeyword);
  ...
  // ── 섹션 루프 ──
  for (const sec of ENT_V2_FLOW_ENGINE.sections) {
    const basePrompt = buildEntV2Prompt(sec.key, treatment, region);
  ...
  // ============================================================
  // 반복 분산 후처리 — 라인 단위 단일 패스   [3종 결함 수정본 · card 이식]
  ...
  //     ent 대명사 = "이 검사"·"이 진료" → 둘 다 무받침. fix-3는 잠복(무해) 상태로 통일 보존.
  //   부가: NAME_THRESHOLD 4.
  // ============================================================
  {
