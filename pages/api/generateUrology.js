// ============================================================
// generateUrology.js — 비뇨기과 블로그 생성기 v3.4
//
...
//   ⑤ 이전 버그 수정:
//      - buildUrologyHashtags 안에 cleanText 코드 잘못 들어가 있던 문제 해결
//      - runQC certaintyCount/recommendCount/reviewFlowCount 미정의 버그 해결
// ============================================================
import { UROLOGY_TREATMENTS }                  from "../../lib/urology-data";
import { buildUrologyPrompt, getUrologyDirection } from "../../lib/urology-prompts";
import { UROLOGY_FLOW_ENGINE }                 from "../../lib/urology-playConfig";
import {
...
const UROLOGY_FORBIDDEN_COMMERCIAL = [
  // ── 1인칭 시점 ──
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
const UROLOGY_CROSS_BLOCK = getCrossBlocks("urology");
...
const INFO_BLOCKS = {
  prostate: {
    title: "전립선비대증 치료법 일반 비교",
    rows: [
      ["항목", "약물치료", "결찰술(Urolift)", "수술(TURP·홀렙)"],
      ["방식", "복용", "비절개 결찰", "조직 제거"],
      ["회복 기간", "복용 중", "1~2일", "1~2주"],
      ["적합 케이스", "경증~중등도", "약물 효과 부족", "중증"],
    ],
  },
  ed: {
    title: "발기부전 치료법 일반 비교",
    rows: [
      ["항목", "약물치료", "충격파 치료", "음경보형물 수술"],
      ["방식", "복용", "비침습 시술", "수술"],
      ["효과", "복용 시", "수개월 지속", "영구적"],
      ["적합 케이스", "경증·중등도", "약 반응 부족", "중증"],
    ],
  },
  kidney_stone: {
    title: "요로결석 치료법 일반 비교",
    rows: [
      ["항목", "자연배출", "체외충격파(ESWL)", "내시경 수술"],
      ["방식", "수분·약물", "충격파 분쇄", "내시경 제거"],
      ["적합 케이스", "5mm 이하", "5~20mm", "20mm 이상·복잡"],
      ["회복 기간", "1~2주", "1~3일", "1~2주"],
    ],
  },
  varicocele: {
    title: "정계정맥류 치료법 일반 비교",
    rows: [
      ["항목", "현미경 수술", "복강경 수술", "색전술"],
      ["방식", "정맥 결찰", "복강경 결찰", "혈관 폐색"],
      ["회복 기간", "1주 내외", "1~2주", "1주 내외"],
      ["적합 케이스", "표준", "양측·재발", "수술 부담 큰 경우"],
    ],
  },
  default: {
    title: "비뇨기과 진료 검토 시 일반 안내",
    rows: [
      ["확인 항목", "내용"],
      ["전문의 자격", "비뇨의학과 전문의 여부 확인"],
      ["검사 장비", "초음파·요역동학·내시경 장비 보유"],
      ["사후 관리", "치료 후 정기 점검 여부"],
...
// ============================================================
const EXAM_VALUES = {
  prostate:     { duration: "약물 장기 / 수술 30~60분", recovery: "수술 후 1~2주", pain: "수술 후 통증점수 3~4점" },
  circumcision: { duration: "수술 30분",               recovery: "1주 내외",      pain: "통증점수 2~3점" },
  kidney_stone: { duration: "ESWL 30~60분",            recovery: "결석별 1주~1개월", pain: "시술 중 통증점수 3~5점" },
  ed:           { duration: "충격파 회당 20~30분",     recovery: "당일 회복",      pain: "통증 거의 없음" },
...
// 3. 제목 생성 (mode 분기)
// ============================================================
function buildUrologyTitle(treatmentName, region, seoData, blogTypeId, mode) {
  if (mode === "commercial") {
    const defaults = [
      `${region} ${treatmentName} 진료 안내｜치료 과정과 일반 정보 정리`,
      `${region} ${treatmentName} 정보 가이드｜비뇨기과 검토 시 확인할 항목`,
  ...
  const defaults = [
    `${region} ${treatmentName} 후기｜혼자 고민하다 받은 후기`,
    `${treatmentName} 고민 한 달｜${region} 비뇨기과에서 받고 나서 드는 생각`,
    `${region} ${treatmentName}｜원인 찾고 해결한 이야기`,
  ];
  return defaults[Math.floor(Math.random() * defaults.length)];
}
...
// 4. 해시태그 (mode 분기)
// ============================================================
function buildUrologyHashtags(treatmentName, region, mode) {
  const kw = treatmentName.replace(/\s/g, "");
  if (mode === "commercial") {
    return [
      `#${region}${kw}`, `#${kw}정보`, `#${kw}안내`,
      `#${kw}`, `#${region}비뇨기과`, `#비뇨기과정보`,
      `#${region}진료안내`, `#비뇨기과진료`,
    ].slice(0, 8).join(" ");
  }
  return [
    `#${region}${kw}`, `#${kw}후기`, `#${kw}상담`,
    `#${kw}`, `#${region}비뇨기과`, `#비뇨기과후기`,
    `#${region}후기`, `#남성건강`, `#${region}비뇨기과추천`,
  ].slice(0, 10).join(" ");
}
...
// ============================================================
function cleanUrologyText(text, treatmentName, region, mode = "personal") {
  let result = text;

  // 공통: 기본 + AI 냄새 금지어 + 과별 침투 차단
  const removeList = [...UROLOGY_FORBIDDEN_BASE, ...UROLOGY_FORBIDDEN_AI, ...UROLOGY_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...UROLOGY_FORBIDDEN_COMMERCIAL);
  ...
  result = result
    .replace(/전립선를/g, "전립선을")
    .replace(/요로결석를/g, "요로결석을")
    .replace(/방광염를/g, "방광염을")
    .replace(/포경수술를/g, "포경수술을")
    .replace(/정관수술를/g, "정관수술을");
  ...
  */
  ...
  result = result.split("\n").map(line => {
    ...
    if (/[.?!"”’」』)\]》〉]$/.test(trimmed)) return line;
    if (/(증상이|기능이|소변이|결과가|모습이)\s*$/.test(trimmed)) {
...
// ============================================================
function insertUrologyTimeline(text, treatmentName, mode) {
  if (mode === "commercial") return text;
  const hasTimeline = /일차|일째|주일|개월/.test(text);
  if (!hasTimeline) return text;
  const isSurgery = /수술|결찰|확대/.test(treatmentName);
  const isShockwave = /충격파|결석|발기부전/.test(treatmentName);
  const day7Note = isSurgery ? "수술 부위 안정화"
                 : isShockwave ? "회차별 변화 체감 시작"
                 : "증상 변화 체감";
  const timeline = `\n\n**일반 회복 경과 (개인차 있음)**\n- 1~2일차: 시술 직후 반응 관찰\n- 7일차: ${day7Note}\n- 1개월차: 증상·기능 안정화 여부\n- 3개월차: 최종 상태 확인`;
...
// 7. 추천 대상 (personal 전용)
// ============================================================
const UROLOGY_REC_MAP = {
  "전립선비대증치료": ["야간뇨가 잦은 경우", "소변 줄기가 약해진 경우"],
  "포경수술":         ["위생 관리가 어려운 경우", "기능적 필요가 있는 경우"],
  "요로결석치료":     ["옆구리 통증이 반복되는 경우", "혈뇨가 보이는 경우"],
  "방광염치료":       ["방광염이 자주 재발하는 경우", "배뇨 통증이 있는 경우"],
  "발기부전치료":     ["기능 변화가 있는 경우", "근본 원인 진단을 원하는 경우"],
  "정관수술":         ["가족 계획이 정리된 경우"],
  "요실금치료":       ["기침·운동 시 소변 새는 경우", "외출이 부담되는 경우"],
  "정계정맥류치료":   ["임신 시도 중 진단받은 경우", "음낭 통증이 있는 경우"],
};
...
function runQC(text, treatmentName, mode) {
  ...
  console.log(`[urology][QC] 정보블럭: ${hasInfoBlock}`);
  console.log(`[urology][QC] 수치: ${hasExamValue}`);
  console.log(`[urology][QC] 키워드반복: ${kwCount}`);
  if (mode === "commercial") {
    console.log(`[urology][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[urology][QC] 가격(commercial 위반): ${priceCount}건`);
    console.log(`[urology][QC] 효과단정(commercial 위반): ${certaintyCount}건`);
    console.log(`[urology][QC] 추천유도(commercial 위반): ${recommendCount}건`);
    console.log(`[urology][QC] 후기흐름(commercial 위반): ${reviewFlowCount}건`);
  }
...
// 메인 핸들러
// ============================================================
export default async function handleUrology(req, res) {
  const {
  ...
  const industry   = "urology";
  const validMode  = (mode === "commercial") ? "commercial" : "personal";
  console.log(`[urology] mode: ${validMode}`);

  // ── ent 진료 검증 ─────────────────────────────────
  // 검증 생략: UROLOGY_TREATMENTS 테이블에 없는 진료는 자동으로 fallback 처리됨
  console.log(`[urology] 치료 진입: ${subKw}`);

  // ── 시술 데이터 로드 ─────────────────────────────────
  const treatmentData = UROLOGY_TREATMENTS.find(t => t.id === program.id || t.name === program.name)
    || UROLOGY_TREATMENTS[0];
  ...
  // ── 민감 시술 여부 ──
  const isSensitive = /음경|발기|조루|성병|귀두|정관|포경|정계|발기부전/.test(subKw);
  const sensitiveAdd = isSensitive
    ? `\n[민감 시술 표현 가이드]\n- 직접적 신체 표현 반복 금지\n- "남성 자신감", "기능 개선" 완화 표현 사용\n- 자극적 묘사 금지`
    : "";

  // ── 시스템 프롬프트 (mode 분기) ────────────────────
  const systemPrompt = validMode === "commercial"
    ? `당신은 ${region} 지역 ${subKw} 진료 정보를 정리하는 정보형 블로그 작가입니다.
업종: 비뇨기과 | 치료: ${subKw} | 지역: ${region}
...
🚫 절대 금지 패턴 (생성하지 말 것):
  ❌ "${subKw} 치료"  ← 이중 표현 (예: "전립선비대증치료 치료" 금지)
  ❌ "${subKw} 시술"  ← 이중 표현
  ❌ "${subKw} 진료"  ← 이중 표현
  ❌ "${subKw}이 치료"  ← 어순 붕괴
  ❌ "${subKw}이 시술"  ← 어순 붕괴
...
- ❌ 후기 의존 표현 금지 (후기 보고 결정/체감 변화)${sensitiveAdd}
...
업종: 비뇨기과 | 치료: ${subKw} | 지역: ${region}
[절대 금지] 성형/피부/치과/이비인후과 관련 표현 일절 사용 금지${sensitiveAdd}
...
- 미완성 문장·비문 금지`;
  ...
  // ── 섹션별 순차 생성 ─────────────────────────────────
  const SECTIONS = UROLOGY_FLOW_ENGINE.sections;
  const sectionTexts = {};
  let prevTextRaw = "";

  for (const sec of SECTIONS) {
    const richPrompt = buildUrologyPrompt(sec.key, treatmentData, region, { mode: validMode });
    const prevBlock  = prevTextRaw
      ? `\n[지금까지 작성된 내용 — 표현 반복 금지]\n${prevTextRaw.slice(0, 1500)}\n[끝]\n`
      : "";

    const userPrompt = `업종: urology | 키워드: ${subKw} | 지역: ${region} | 모드: ${validMode}
${prevBlock}
---
...
${richPrompt}`;
  ...
  //   풀: 검사 / 상담 / 치료 / 시술 / 일상
  const UROLOGY_ALT_POOL = ["검사 사진", "상담 사진", "치료 사진", "시술 사진", "일상 사진"];
  const _UROLOGY_ALT_BY_KEY = {
    concern:  "일상 사진",
    search:   "상담 사진",
    consult:  "검사 사진",
    decision: "상담 사진",
  ...
  const altList = SECTIONS.slice(0, 5).map(sec => {
    const label = _UROLOGY_ALT_BY_KEY[sec.key] || "상담 사진";
    return `[이미지: ${label}]`;
  });

  // ── 제목 생성 (mode 분기) ─────────────────────────
  let title = overrideTitle || buildUrologyTitle(subKw, region, seoData, blogTypeId, validMode);
  const UROLOGY_TITLE_BLOCK = /쌍꺼풀|눈매|리프팅|울쎄라|써마지|필러|보톡스|피코레이저|성형외과|임플란트|치아|잇몸|스케일링|사랑니|비염|축농증|편도|이명|난청/;
  if (UROLOGY_TITLE_BLOCK.test(title)) {
    title = validMode === "commercial"
      ? `${region} ${subKw} 진료 안내｜치료 과정과 일반 정보`
      : `${region} ${subKw} 후기｜혼자 고민하다 받은 후기`;
  }
  ...
  if (sectionTexts["result"]) {
    sectionTexts["result"] = insertUrologyTimeline(sectionTexts["result"], subKw, validMode);
  }
  ...
  if (sectionTexts[lastKey]) {
    if (validMode === "commercial") {
      const commercialCTAs = [
        ...
        `\n\n위 내용은 ${subKw} 진료에 대한 일반 정보 안내입니다. 적합 여부·회복 경과는 개인차가 있으므로, ${region} 내 비뇨기과에서 충분한 상담 후 결정하시는 것이 좋습니다.`,
        `\n\n${subKw} 관련 일반 정보를 정리한 내용입니다. 본인 상태에 맞는 치료 방법은 비뇨기과 진료 후 안내받는 것이 권장되며, 궁금한 부분은 상담 시 직접 문의해보시기 바랍니다.`,
      ];
      sectionTexts[lastKey] = sectionTexts[lastKey].trimEnd()
        + commercialCTAs[Math.floor(Math.random() * commercialCTAs.length)];
    } else {
      const recList = UROLOGY_REC_MAP[subKw] || [];
      const recBlock = recList.length > 0
        ? `\n\n**이런 분들께 추천**\n${recList.map(r => `- ${r}`).join("\n")}`
        : "";
      sectionTexts[lastKey] = sectionTexts[lastKey].trimEnd()
        + recBlock
        + "\n\n비슷한 고민이라면 비뇨기과 상담 한 번 받아보는 것도 도움이 됩니다.";
    }
  }
  ...
  // ── 최종 클리닝 (조립 후 누수 방지) ──────
  // ★ [v2] 해시태그는 cleanText 이후 추가 — oriental v3.6.1 교훈
  //   cleanText 내 키워드 치환 ("이 치료" 변환) 이 해시태그 안 키워드까지 깨뜨리는 문제 방지
  assembled = cleanUrologyText(assembled, subKw, region, validMode);
  // commercial 모드는 2회 통과
  if (validMode === "commercial") {
    assembled = cleanUrologyText(assembled, subKw, region, validMode);
  }

  // 해시태그는 cleanText 이후에 추가 (깨짐 방지)
  assembled += "\n\n" + buildUrologyHashtags(subKw, region, validMode);
  ...
  assembled = assembled.replace(/\[이미지:\s*([^\]]+)\]/g, (_m, inner) => {
    ...
    if (/검사|초음파|소변검사|혈액|영상|진단|소견|요속/i.test(s)) return "[이미지: 검사 사진]";
    if (/시술|수술|체외충격파|결석파쇄|레이저|포경|정관/.test(s)) return "[이미지: 시술 사진]";
    if (/치료|약물|처방|호르몬|주사|요법/.test(s))            return "[이미지: 치료 사진]";
  ...
  console.log(`[urology] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[urology] ⚠️ commercial 모드 1인칭 ${qc.firstPersonCount}건 잔존`);
