// ============================================================
// pages/api/generateScreen.js — 방충망(screen) 엔진 핸들러
// 정보형({region} 방충망 업체 1인칭). 홈케어·시공/관리 업종.
// 후기형·내돈내산·만족도·설치후기·추천·보장·최저가·순위·할인·이벤트 금지.
// engineBootstrap에서 register("screen", handleScreen)로 편입.
// 복제 베이스: generateAirclean.js (섹션루프형). 기존 업종 무수정.
//   ★ useApt=true: APT_DATA 실데이터 기반 단지명 결합(지역+아파트명 허용 — 관측 결과).
//     - resolveAptName이 region→regionKey 추론 후 단지 풀에서 선택(또는 body 지정).
//     - 제목 {aptName} 토큰 치환, 본문 프롬프트에 단지 1회 결합 힌트 주입.
//   ★ aptName 없을 때 {aptName} 토큰 삭제(region 폴백 금지) — 제목 region 중복 방어.
...
import { SCREEN_FLOW } from "../../lib/screen-playConfig";
...
//       "추천"(단독)은 "선택을 추천합니다" 같은 정상 문장을 파괴한다. "추천드립니다"는 HARD 유지.
//   프롬프트 [금지] 목록에는 그대로 남아 있어 억제력은 유지된다.
const STRIP_SKIP = new Set(["순위", "최고", "완벽", "1위", "반드시", "만족", "추천"]);
...
// ── 화자 도입 (방충망 업체 1인칭. 업체명 본문 미노출) ──
function buildAgentIntro(region, aptName) {
  return `안녕하세요. ${region} 방충망 업체입니다.\n오늘은 ${region} 방충망을 안내해 드리겠습니다.`;
}

// ── 정보블럭 선택 (cat 기준) ──────────
function pickInfoBlock(treatment) {
  const cat = treatment.cat;
  return SCREEN_INFO_BLOCKS[cat] || SCREEN_INFO_BLOCKS["교체"];
}
...
// ── 지역키 추론 (APT_DATA 실데이터 — region 매칭) ──
...
// ── aptName 선택 (useApt=true → region 매칭 시 단지 풀에서 선택) ──
...
function buildTitle(region, treatment, aptName, aptMeta) {
  const _t = buildIntentTitleOrNull(region, treatment, "screen");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment, aptName, aptMeta);
}

export default async function handleScreen(req, res) {
  try {
    const {
    ...
    // 방충망 = 출장/현장방문 업종. 발행코치에서 위치 5필드 채워 보내면 「찾아오시는 길」 삽입,
    ...
    const treatment =
      SCREEN_TREATMENTS.find((t) => t.id === program?.id) ||
      SCREEN_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `방충망 메뉴 매칭 실패: ${program?.name}` });
    }

    const kw = treatment.name;
    const { aptName, meta: aptMeta } = resolveAptName(treatment, region, bodyAptName);
    const effRegion = region; // 본문 지역축은 region 단독. 단지명은 제목·결합 힌트로만.
    const systemPrompt = buildSystemPrompt(effRegion, treatment, aptName, aptMeta);

    const PHOTO_ALT = {
      intro: "방충망 종류 안내", axis1: "교체 필요성 안내", axis2: "설치 위치·안전 안내",
      axis3: "진행 순서 안내", axis4: "관리·종류 비교 안내", closing: "방충망 상담 안내",
    };

    const writtenSections = new Set();
    const sections = [];

    for (const sec of SCREEN_FLOW) {
      ...
      body = body.replace(/(^|\n)\s*운영자\s*(\n|$)/g, "\n").trim();
      // closing 섹션이 화자 인사("안녕하세요 …방충망 업체입니다")를 재등장시키는 패턴 제거(intro 중복)
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*방충망\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }
      body += "\n\n[이미지: " + (PHOTO_ALT[sec.key] || "방충망 상담 안내") + "]";
      sections.push(body);
    }
    ...
    console.log(`[QC][screen] 메뉴(${kw}): kw ${kwCount}`);
    console.log(`[QC][screen] 글자수: ${content.length}`);
    console.log(`[QC][screen] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[screen] 오류:", err);
    return res.status(500).json({ error: err.message || "방충망 글 생성 오류" });
  }
}
