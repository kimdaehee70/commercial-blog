// ============================================================
// generateChinese.js — 중식(중화요리) 블로그 생성기 v1.0 (Chinese Engine 독립)
//
...
import {
  CHINESE_TREATMENTS, CHINESE_BLOCK_MAP,
  CHINESE_TITLE_MIDDLE, CHINESE_TITLE_SUFFIX,
  CHINESE_TITLE_SCENE, CHINESE_TITLE_SCENE_BY_CATEGORY,
} from "../../lib/chinese-data";
import {
  buildChinesePrompt, getChineseDirection,
} from "../../lib/chinese-prompts";
import {
  getChineseSections,
} from "../../lib/chinese-playConfig";
import {
...
const CHIN_FORBIDDEN_COMMERCIAL = [
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
// ============================================================
const CHIN_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("chinese");
  } catch (e) {
    return [
      ...CHINESE_BLOCK_MAP.medical,
      ...CHINESE_BLOCK_MAP.cafe,
      ...CHINESE_BLOCK_MAP.study,
      ...CHINESE_BLOCK_MAP.ad,
    ];
  }
})();
...
function buildChineseTitle(treatment, region, situation, purpose, seoData, mode, titleSuffixOpts) {
  ...
  if (mode === "commercial") {
    ...
    //   - 중식 commercial 제목은 purpose 미사용 → buildTitle에 purpose 빈값.
    //     resolvePurpose(null)=null → 패턴 분기 안 탐 → commercial 폴백 경로
    //     `${region} ${menu}${mid? " "+mid:""}${suf? "｜"+suf:""}` 산출.
    //   - 기존 `${region} ${menu} ${mid}｜${suf}`와 형태 동치(mid/suf 비어도 빈자국 정리).
    //   - SCENE/MIDDLE/SUFFIX는 data 소유 → 풀 주입. titlePatterns 미주입(중식 commercial 미사용).
    //   - 연속 발행 회피: state 모듈 보관(_titleState).
    const pools = {
      titlePatterns: [],   // 중식 commercial은 패턴 미사용(폴백 경로 사용)
      SCENE:        CHINESE_TITLE_SCENE,
      SCENE_BY_CAT: CHINESE_TITLE_SCENE_BY_CATEGORY,
      MIDDLE:       CHINESE_TITLE_MIDDLE,
      SUFFIX:       CHINESE_TITLE_SUFFIX,
    };
    const suffix = titleSuffixOpts
      ? resolveTitleSuffix(titleSuffixOpts)
      : DEFAULT_TITLE_SUFFIX;
    const out = buildTitle({
      region, menu, cat, situation: sit, purpose: "",   // commercial purpose 미사용
      mode: "commercial",
      pools,
      purposeTitleLabel: "",
      state: _titleState,
...
// ============================================================
function buildChineseHashtags(treatment, region, situation, purpose, mode) {
...
// ============================================================
function cleanChineseText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  const direction = getChineseDirection(treatment, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  let result = text;

  const removeList = [...CHIN_FORBIDDEN_BASE, ...CHIN_FORBIDDEN_AI, ...CHIN_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...CHIN_FORBIDDEN_COMMERCIAL);
  ...
  if (particleErrorCount > 0) {
    console.log(`[chinese][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[chinese][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[chinese][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[chinese][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (particleDupCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[chinese][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }
  ...
  if (stepG2Count > 0) console.log(`[chinese][QC] ★ STEP G2 잔여정정: 매장안내참고/이거의/한그릇가/메뉴명조합/전문점중식당들은/가게마다/방문시중복/맛조사/중식당전문점`);

  // ─────────────────────────────────────────────────────
  // ★ STEP H-josa — 앞선 치환 잔류 조사/이중표기 최종 청소 (결정론)
  //   근거(세션57 실측): STEP F-1(조사중복 제거)은 632줄에 있어, 그 뒤 STEP G2 등
  //     후속 치환이 새로 만든 조사 잔류(에서는은/들은은/때가은)를 못 잡음(순서 버그).
  //   해결: 전 치환의 맨 끝(여기)에서 잔류만 결정론 청소. 문장 삭제·의미 판정 없음.
  //   ※ 이전 STEP H 열거식(평가·비교 문장 제거)은 롤백:
  //      실측상 의미 파생을 못 막고(발산) 품질 하락(82→78) 확인 → 열거 확장 중단.
  //      의미 기반 추론 차단은 별도 계층(C축 재작성/GPT 재판정)에서 재설계.
  // ─────────────────────────────────────────────────────
  let stepHCount = 0;
  {
    const _before_H = result;
    const josaClean = [
      // 동일/이종 조사 2연 잔류 (F-1 이후 치환 산출물)
      [/에서는은(?=\s|[,.!?]|$)/g, "에서는 "],
      [/에서는는(?=\s|[,.!?]|$)/g, "에서는 "],
      [/([가-힣])은은(?=\s|[,.!?]|$)/g, "$1은 "],
      [/([가-힣])는은(?=\s|[,.!?]|$)/g, "$1는 "],
      [/([가-힣])은는(?=\s|[,.!?]|$)/g, "$1은 "],
      // '때가' 오결합 (시간명사 '때'에 주격 '가' 오부착 — GPT/치환 잔류)
      [/때가은은?(?=\s|[,.!?]|$)/g, "때 "],
      [/때가(?=\s)/g, "때 "],
      // 전문점/지역 치환 잔여 이중 업종 지칭 (앞단 축약, 메뉴명 보존)
      [/전문점에서는\s*(?:근처의|인근의|이\s*동네의|주변의)?\s*중식당에서는/g, "중식당에서는"],
      [/전문점의\s*중식당/g, "중식당"],
    ];
    josaClean.forEach(([re, to]) => {
      const b = result; result = result.replace(re, to);
      if (b !== result) stepHCount++;
    });
    if (_before_H !== result) result = normalizeWhitespace(result);
  }
  if (stepHCount > 0) console.log(`[chinese][QC] ★ STEP H-josa 조사잔류 청소: ${stepHCount}건 (에서는은/때가은/이중업종 → 정정)`);

  if (transFixCount > 0)     console.log(`[chinese][QC] ★ STEP G 번역체 정정: ${transFixCount}건 (그것의/그것을/그것이 → 메뉴)`);
  if (phraseDupCount > 0)    console.log(`[chinese][QC] ★ STEP G 어절중복 제거: ${phraseDupCount}건 (메뉴명 군더더기 인접반복)`);
  if (generalizeFixCount > 0) console.log(`[chinese][QC] ★ STEP G 일반화 안전화: ${generalizeFixCount}건 (일부 가게/대부분 → 매장에 따라)`);
  if (repeatCapCount > 0)    console.log(`[chinese][QC] ★ STEP G 과반복 제한: ${repeatCapCount}건 (3회째+ 축약 · 첫 2회·메뉴명 보존)`);

  // 외부 QC 집계용
  cleanChineseText.__lastParticleDup = particleDupCount;
  cleanChineseText.__lastSubjParticleFix = subjParticleFixCount;
  cleanChineseText.__lastEfficacyFix = efficacyFixCount;
  cleanChineseText.__lastStoreRefFix = storeRefFixCount;
  cleanChineseText.__lastMenuDup = menuDupCount;
  cleanChineseText.__lastSuperlativeFix = superlativeFixCount;
  cleanChineseText.__lastUnitFix = unitFixCount;
  cleanChineseText.__lastDupSentence = dupSentenceCount;
  cleanChineseText.__lastParticleErrors = particleErrorCount;
  cleanChineseText.__lastMenuMeta = menuMetaCount;
  cleanChineseText.__lastAdEvalFixed = adEvalFixed;
  cleanChineseText.__lastPlaceholderFix = placeholderFixCount;
  cleanChineseText.__lastTransFix = transFixCount;
  cleanChineseText.__lastPhraseDup = phraseDupCount;
  cleanChineseText.__lastGeneralizeFix = generalizeFixCount;
  cleanChineseText.__lastRepeatCap = repeatCapCount;
  cleanChineseText.__lastStepG2 = stepG2Count;
  cleanChineseText.__lastStepH = (typeof stepHCount !== "undefined") ? stepHCount : 0;
...
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  const adEvalFixed = (typeof cleanChineseText.__lastAdEvalFixed === "number")
    ? cleanChineseText.__lastAdEvalFixed : 0;

  // cleanChineseText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanChineseText.__lastParticleErrors === "number")
    ? cleanChineseText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanChineseText.__lastMenuMeta === "number")
    ? cleanChineseText.__lastMenuMeta : 0;
  ...
  console.log(`[chinese][QC] 메뉴블럭: ${hasMealBlock}`);
  console.log(`[chinese][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  console.log(`[chinese][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[chinese][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[chinese][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[chinese][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[chinese][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[chinese][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[chinese][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[chinese][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[chinese][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[chinese][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[chinese][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
    console.log(`[chinese][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[chinese][QC] 가격명시(commercial 위반): ${priceCount}건`);
  }
...
// 메인 핸들러
// ============================================================
export default async function handleChinese(req, res) {
  const {
  ...
  console.log(`[chinese] mode: ${validMode} | 상황: ${situation || "(미지정)"} | 목적: ${purpose || "(미지정)"}`);
  console.log(`[chinese][BUILD] chinese-v1.3-stepG2 (Chinese Engine 독립 · STEP G2 잔여정정 10룰 · 배포검증: 이 줄이 안 보이면 구버전 실행 중)`);

  // ── restaurant 조합 검증 ─────────────────────────────────
  const CHIN_IDS = CHINESE_TREATMENTS.map(t => t.id);
  const isRest = CHIN_IDS.includes(program.id) || program.industry === "chinese";
  if (!isRest) {
    console.error(`[chinese] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  const treatmentData = CHINESE_TREATMENTS.find(t => t.id === program.id)
    || CHINESE_TREATMENTS[0];
  ...
  console.log(`[chinese] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getChineseDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[chinese] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getChineseSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";

  for (const sec of SECTIONS) {
    const richPrompt = buildChinesePrompt(sec.key, treatmentData, region, {
    ...
    const userPrompt = `업종: chinese | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat} | 상황: ${situation} | 목적: ${purpose} | 모드: ${validMode}
${prevBlock}
---
...
${richPrompt}`;
  ...
  const altList = SECTIONS.slice(0, _altCount).map(sec => {
    const label = _CHIN_ALT_BY_KEY[sec.key] || "메뉴 사진";
    return `[이미지: ${label}]`;
  });
  console.log(`[chinese][ALT] mode=${validMode} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  let title = overrideTitle || buildChineseTitle(treatmentData, region, situation, purpose, seoData, validMode, _titleSuffixOpts);
  // 의료·카페 어휘가 제목에 새면 차단
  ...
  assembled += "\n\n" + buildChineseHashtags(treatmentData, region, situation, purpose, validMode);
  ...
  //         제목은 buildChineseTitle 단계에서 이미 menu 포함 보장됨
  {
    ...
    let cleanedBody = cleanChineseText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanChineseText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  console.log(`[chinese] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  // 경고
  if (qc.adPatternCount > 0)
    console.error(`[chinese] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[chinese] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[chinese] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[chinese] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[chinese] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[chinese] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[chinese] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
