// ============================================================
// generateKorean.js — 한식(한식) 블로그 생성기 v1.0 (Korean Engine 독립)
//
...
import {
  KOREAN_TREATMENTS, KOREAN_BLOCK_MAP,
  KOREAN_TITLE_MIDDLE, KOREAN_TITLE_SUFFIX,
  KOREAN_TITLE_SCENE, KOREAN_TITLE_SCENE_BY_CATEGORY,
  KOREAN_TITLE_MENU_CLASS, KOREAN_TITLE_PURPOSE_BY_CLASS,
  KOREAN_TITLE_PURPOSE_FALLBACK, KOREAN_TITLE_FORMS,
  pickRotatedSituations,
  KOREAN_TITLE_PURPOSE_TO_MASTER,
} from "../../lib/korean-data";
...
import {
  buildKoreanPrompt, getKoreanDirection,
} from "../../lib/korean-prompts";
import {
  getKoreanSections,
} from "../../lib/korean-playConfig";
import {
...
const KOR_FORBIDDEN_COMMERCIAL = [
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
// ============================================================
const KOR_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("korean");
  } catch (e) {
    return [
      ...KOREAN_BLOCK_MAP.medical,
      ...KOREAN_BLOCK_MAP.cafe,
      ...KOREAN_BLOCK_MAP.study,
      ...KOREAN_BLOCK_MAP.ad,
    ];
  }
})();
...
// ============================================================
// 직전 commercial 제목 토큰 — 연속 발행 시 회피용
//   _lastTitlePurpose: pickTitlePurpose(master 산출)와 buildKoreanTitle이 공유 — 유지.
//   FORM/MIDDLE 회피상태는 Spine buildTitle의 state로 이관(_titleState).
let _lastTitlePurpose = "";
let _titleState = {};

// ★ [v2.6] purpose 단일 SoT — 제목 purpose 추첨을 buildKoreanTitle 밖으로 분리.
...
function pickTitlePurpose(menu) {
  ...
  const menuClass = KOREAN_TITLE_MENU_CLASS[menu] || "soup";
  const purposePool = (KOREAN_TITLE_PURPOSE_BY_CLASS[menuClass]
                     || KOREAN_TITLE_PURPOSE_FALLBACK);
  ...
  const master = KOREAN_TITLE_PURPOSE_TO_MASTER[displayPurpose] || displayPurpose;
  return { displayPurpose, master, menuClass };
}

function buildKoreanTitle(treatment, region, situation, purpose, seoData, mode, titlePurpose, titleSuffix) {
  ...
  if (mode === "commercial") {
    // ── Platform Spine 위임 (Title Engine v2 — FORM 가중선택) ──
    //   한식=C형: KOREAN_TITLE_FORMS(가중 비복원) + {middle} → Spine pools.FORMS로 위임.
    //   purpose=titlePurpose(핸들러 pickTitlePurpose가 확정한 displayPurpose). master는 핸들러 보존.
    //   displayPurpose는 Registry 밖 자유라벨 → resolvePurpose 콜백으로 strict throw 우회.
    //   titlePurpose 미전달(하위호환) 시 menuClass 풀에서 자체 추첨 후 주입.
    const menuClass = KOREAN_TITLE_MENU_CLASS[menu] || "soup";
    let tPurpose = titlePurpose;
    if (!tPurpose) {
      const pickAvoid = (arr, avoid) => {
        if (!arr || !arr.length) return "";
        if (arr.length === 1) return arr[0];
    ...
    const r = buildTitle({
      region, menu, cat,
      situation: sit,
      purpose: tPurpose,                 // displayPurpose 자유라벨 — 아래 콜백으로 해석
      mode: "commercial",
      pools: {
        FORMS: KOREAN_TITLE_FORMS,       // 가중 비복원 + {middle} (titlePatterns보다 우선)
        SCENE: KOREAN_TITLE_SCENE,
        SCENE_BY_CAT: KOREAN_TITLE_SCENE_BY_CATEGORY,
        MIDDLE: KOREAN_TITLE_MIDDLE,
        SUFFIX: KOREAN_TITLE_SUFFIX,
      },
      ...
      purposeTitleLabel: tPurpose,
      state: _titleState,
      sceneProbability: 0.4,
      suffix: titleSuffix,   // [v77] 제목 끝 상호 표시 — Spine §7 Title Suffix hook. 미주입=OFF.
    });
    _titleState = r.state;
    return r.title;
  }
...
// ============================================================
function buildKoreanHashtags(treatment, region, situation, purpose, mode) {
...
// ============================================================
function cleanKoreanText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  const direction = getKoreanDirection(treatment, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  let result = text;

  const removeList = [...KOR_FORBIDDEN_BASE, ...KOR_FORBIDDEN_AI, ...KOR_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...KOR_FORBIDDEN_COMMERCIAL);
  ...
  if (particleErrorCount > 0) {
    console.log(`[korean][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[korean][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[korean][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[korean][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  ...
  //   원칙: 리터럴 to(캡처참조 0)만 사용 → josa3와 같은 $1 함정 회피.
  {
    ...
    cleanKoreanText.__lastBanFix = banFixCount;
  }
  ...
  if (particleDupCount > 0) {
    console.log(`[korean][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[korean][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[korean][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[korean][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[korean][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[korean][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[korean][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[korean][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }
  ...
  if (stepG2Count > 0) console.log(`[korean][QC] ★ STEP G2 잔여정정: 매장안내참고/이거의/한그릇가/메뉴명조합/전문점한식당들은/가게마다/방문시중복/맛조사/한식당전문점`);

  if (transFixCount > 0)     console.log(`[korean][QC] ★ STEP G 번역체 정정: ${transFixCount}건 (그것의/그것을/그것이 → 메뉴)`);
  if (phraseDupCount > 0)    console.log(`[korean][QC] ★ STEP G 어절중복 제거: ${phraseDupCount}건 (메뉴명 군더더기 인접반복)`);
  if (generalizeFixCount > 0) console.log(`[korean][QC] ★ STEP G 일반화 안전화: ${generalizeFixCount}건 (일부 가게/대부분 → 매장에 따라)`);
  if (repeatCapCount > 0)    console.log(`[korean][QC] ★ STEP G 과반복 제한: ${repeatCapCount}건 (3회째+ 축약 · 첫 2회·메뉴명 보존)`);

  // 외부 QC 집계용
  cleanKoreanText.__lastParticleDup = particleDupCount;
  cleanKoreanText.__lastSubjParticleFix = subjParticleFixCount;
  cleanKoreanText.__lastEfficacyFix = efficacyFixCount;
  cleanKoreanText.__lastStoreRefFix = storeRefFixCount;
  cleanKoreanText.__lastMenuDup = menuDupCount;
  cleanKoreanText.__lastSuperlativeFix = superlativeFixCount;
  cleanKoreanText.__lastUnitFix = unitFixCount;
  cleanKoreanText.__lastDupSentence = dupSentenceCount;
  cleanKoreanText.__lastParticleErrors = particleErrorCount;
  cleanKoreanText.__lastMenuMeta = menuMetaCount;
  cleanKoreanText.__lastAdEvalFixed = adEvalFixed;
  cleanKoreanText.__lastPlaceholderFix = placeholderFixCount;
  cleanKoreanText.__lastTransFix = transFixCount;
  cleanKoreanText.__lastPhraseDup = phraseDupCount;
  cleanKoreanText.__lastGeneralizeFix = generalizeFixCount;
  cleanKoreanText.__lastRepeatCap = repeatCapCount;
  cleanKoreanText.__lastStepG2 = stepG2Count;
...
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  const adEvalFixed = (typeof cleanKoreanText.__lastAdEvalFixed === "number")
    ? cleanKoreanText.__lastAdEvalFixed : 0;

  // cleanKoreanText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanKoreanText.__lastParticleErrors === "number")
    ? cleanKoreanText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanKoreanText.__lastMenuMeta === "number")
    ? cleanKoreanText.__lastMenuMeta : 0;
  ...
  console.log(`[korean][QC] 메뉴블럭: ${hasMealBlock}`);
  console.log(`[korean][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  console.log(`[korean][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[korean][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[korean][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[korean][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[korean][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[korean][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[korean][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[korean][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[korean][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[korean][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[korean][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
    console.log(`[korean][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[korean][QC] 가격명시(commercial 위반): ${priceCount}건`);
  }
...
// 메인 핸들러
// ============================================================
export default async function handleKorean(req, res) {
  const {
    ...
    mode = "commercial",
    // ★ [세션42] Visit Pilot 게이트 — index.js 게이트가 ON일 때만 카멜 visitPilot/visitInfo 주입.
    visitPilot = false,
    visitInfo = null,
  } = req.body;
  ...
  //   titleSuffixOn=사용자 토글(store_profiles.title_suffix_on) / storeName=실제 업체명(req.body.storeName, index.js가 hubStore.store_name 전달).
  ...
  const industry  = "korean";
  // [v2] personal은 명시적으로 요청된 경우에만. 그 외(미지정·commercial) = commercial.
  const validMode = (mode === "personal") ? "personal" : "commercial";

  // ★ [세션42] Visit Pilot 관측 로그 — 게이트 발동 여부 확인용(korean 실사용 엔진).
  if (visitPilot) {
    console.log(`[VISIT_PILOT] korean ON | visitInfo=${visitInfo ? "present" : "null"} | mode=${validMode}`);
  }
  ...
  console.log(`[korean] mode: ${validMode} | 상황: ${situation || "(미지정)"} | 목적: ${purpose || "(미지정)"}`);
  console.log(`[korean][BUILD] korean-v2.3.6-layered+t2.4 (본문 v2.3.6 FREEZE 유지(무수정) · ★제목엔진 v2.4: 방문목적 선두 가중폼(A지역+목적+메뉴40%·B목적선두20%·C기존형20%·D정보형15%·E메뉴선두5%) + 결별 purpose 폴백(soup/noodle/meat/rice) + FORM·purpose·middle 직전회피 · ★후처리 v2.4: 풍성해지다 어미활용 + 식감괜찮은,쉼표 어절깨짐 · ★menu-map 진단: id실패시 menu재매칭 방어 + 국밥무음폴백 경고로그(원인 index.js payload 점검은 다음세션) · commercial 제목분기만 수정 · 배포검증: +t2.4 안 보이면 제목/후처리 구버전 실행 중)`);

  // ── restaurant 조합 검증 ─────────────────────────────────
  const KOR_IDS = KOREAN_TREATMENTS.map(t => t.id);
  const isRest = KOR_IDS.includes(program.id) || program.industry === "korean";
  if (!isRest) {
    console.error(`[korean] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  // ★ [v2.4·치명버그 진단] 메뉴 매핑 오류 추적 (삼계탕/제육 선택 → 국밥 생성 케이스)
  //   배경: program.id로 treatment를 찾는데, 실패 시 KOREAN_TREATMENTS[0](국밥)으로
  ...
  const _idMatch   = KOREAN_TREATMENTS.find(t => t.id === program.id);
  ...
  const _menuMatch = !_idMatch && _menuKey
    ? KOREAN_TREATMENTS.find(t => t.menu === _menuKey || t.menuRef === _menuKey)
    : null;
  const treatmentData = _idMatch || _menuMatch || KOREAN_TREATMENTS[0];

  // 진단 로그 — 매핑 경로를 명시. 운영 로그에서 즉시 원인 판별 가능.
  if (!_idMatch) {
    console.error(
      `[korean][★MENU-MAP] id 매칭 실패! programId=${JSON.stringify(program?.id)} ` +
      ...
      `${(!_idMatch && !_menuMatch) ? " ← ⚠국밥[0] 무음폴백 발생(잘못된 글 생성 위험)" : " ← menu명으로 복구됨"}`
    );
    console.error(`[korean][★MENU-MAP] availableIds=${JSON.stringify(KOREAN_TREATMENTS.map(t => t.id))}`);
  }
  console.log(`[korean][treatment] 선택: ${treatmentData.id} / ${treatmentData.menu} (경로: ${_idMatch ? "id직매칭" : _menuMatch ? "menu재매칭" : "국밥폴백"})`);
  ...
  console.log(`[korean] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getKoreanDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[korean] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getKoreanSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";
  ...
  for (let _si = 0; _si < SECTIONS.length; _si++) {
    ...
    const richPrompt = buildKoreanPrompt(sec.key, treatmentData, region, {
      ...
      visitPilot, visitInfo,   // ★ [세션42] Visit Pilot 게이트 전달 (commercial만 소비)
    });
    const prevBlock = prevTextRaw
      ? `\n[지금까지 작성된 내용 — 표현 반복 금지]\n${prevTextRaw.slice(0, 1500)}\n[끝]\n`
      : "";

    const userPrompt = `업종: korean | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat} | 상황: ${situation} | 목적: ${purpose} | 모드: ${validMode}
${prevBlock}
---
...
${richPrompt}`;
  ...
  const altList = SECTIONS.slice(0, _altCount).map(sec => {
    const label = _KOR_ALT_BY_KEY[sec.key] || "메뉴 사진";
    return `[이미지: ${label}]`;
  });
  console.log(`[korean][ALT] mode=${validMode} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  let title = overrideTitle || buildKoreanTitle(treatmentData, region, situation, purpose, seoData, validMode, _titlePurpose ? _titlePurpose.displayPurpose : undefined, _titleSuffix);
  // 의료·카페 어휘가 제목에 새면 차단
  ...
  assembled += "\n\n" + buildKoreanHashtags(treatmentData, region, situation, purpose, validMode);
  ...
  //         제목은 buildKoreanTitle 단계에서 이미 menu 포함 보장됨
  {
    ...
    let cleanedBody = cleanKoreanText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanKoreanText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  console.log(`[korean] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  // 경고
  if (qc.adPatternCount > 0)
    console.error(`[korean] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[korean] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[korean] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[korean] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[korean] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[korean] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[korean] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
