// ============================================================
// pages/api/generateWaterproof.js — 방수공사(waterproof) 엔진 핸들러
// 정보형 + 출장 시공 안내형({region} 방수 시공 업체 1인칭).
// 후기형·내돈내산·고객사례·추천·보장·최저가·비용유도·수명약속 금지.
// engineBootstrap에서 register("waterproof", handleWaterproof)로 편입.
//
// 복제 베이스: generateDoor.js (섹션루프형). Runtime/FLOW/Handler 구조 무변경.
// door 대비 변경 2점:
//   ① 판단 축 교체 — symptom(발견) + part(원인군). door 의 (증상, 부품군)과 동형 계약.
//   ② 해시태그 어휘 — 방수 검색축(누수/방수/부위)으로 교체. dedup 는 door 동형 Set 유지.
//
...
import {
  WATERPROOF_TREATMENTS,
  WATERPROOF_INFO_BLOCKS,
  getInfoBlocks,
  formatSymptom,
  getPartNote,
} from "../../lib/waterproof-data";
import {
  buildSystemPrompt,
  buildUserPrompt,
  getImageAlts,
  getSectionAlt,
  FORBIDDEN,
} from "../../lib/waterproof-prompts";
import { WATERPROOF_FLOW, WATERPROOF_SECTION_PHOTO } from "../../lib/waterproof-playConfig";
...
//     ("보장" → 보장하다/보장재 / "완벽" / "최고" / "1위" / "반영구"(영구 부분일치 방지)
...
// ── 마무리 해시태그 ─────────────────────────────
//   ★ 메뉴명이 "방수"로 끝나 `#{rg}{k}` 와 `#{rg}방수` 가 겹칠 수 있다 → Set 일괄 정리.
...
//   → 메뉴 확장은 waterproof-data.js 단독 작업. 핸들러를 다시 열지 않는다.
function pickInfoBlock(t) {
  const B = (typeof getInfoBlocks === "function" ? getInfoBlocks(t.cat) : null)
    || WATERPROOF_INFO_BLOCKS;
  return B[t.infoKey] || B[t.cat] || WATERPROOF_INFO_BLOCKS.prebook;
}
...
//   ★ waterproof 는 siteBlock 미사용(useSite:false) → 단지명 분기 없음.
...
function buildTitle(region, treatment) {
  const _t = buildIntentTitleOrNull(region, treatment, "waterproof");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment);
}

export default async function handleWaterproof(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
      symptom,                                   // 접수된 발견(누수 흔적) — 미입력 시 무영향
      part,                                      // 원인군(membrane·crack·drain·joint·penetration·urethane·sheet)
    } = req.body;

    // 방수공사 = 출장 시공 업종 → 고정 사업장 위치블록 미노출(PATCH-07)
    const _locStore = {};
    ...
    const treatment =
      WATERPROOF_TREATMENTS.find((t) => t.id === program?.id) ||
      WATERPROOF_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `방수공사 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    for (const sec of WATERPROOF_FLOW) {
      ...
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*방수\s*시공\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }

      const slot = WATERPROOF_SECTION_PHOTO[sec.key];
    ...
    const rgKwCount = region
      ...
      : 0;
    console.log(`[QC][waterproof] 메뉴(${kw}): kw ${kwCount} / 지역+kw 결합 ${rgKwCount} (3 이하 정상)`);
    console.log(`[QC][waterproof] 글자수: ${content.length}`);
    console.log(`[QC][waterproof] 발견: ${sym || "미입력"} / 원인군: ${pt ? pt.label : "미입력"}`);
    console.log(`[QC][waterproof] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[waterproof] 오류:", err);
    return res.status(500).json({ error: err.message || "방수공사 글 생성 오류" });
  }
}
