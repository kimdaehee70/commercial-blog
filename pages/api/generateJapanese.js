// ============================================================
// generateJapanese.js — 일식(일본요리) 블로그 생성기 v1.0 (Japanese Engine 독립)
//
...
import {
  JAPANESE_TREATMENTS, JAPANESE_BLOCK_MAP, JAPANESE_CROSS_CAT_BLOCK,
  JAPANESE_TITLE_MIDDLE, JAPANESE_TITLE_SUFFIX,
  JAPANESE_TITLE_SCENE, JAPANESE_TITLE_SCENE_BY_CATEGORY,
} from "../../lib/japanese-data";
import {
  buildJapanesePrompt, getJapaneseDirection,
} from "../../lib/japanese-prompts";
import {
  getJapaneseSections,
} from "../../lib/japanese-playConfig";
import {
...
const JAPN_FORBIDDEN_COMMERCIAL = [
  "저는 ", "제가 ", "내가 ", "나는 ", "저도 ",
...
// ============================================================
const JAPN_CROSS_BLOCK = (function () {
  try {
    return getCrossBlocks("japanese");
  } catch (e) {
    return [
      ...JAPANESE_BLOCK_MAP.medical,
      ...JAPANESE_BLOCK_MAP.cafe,
      ...JAPANESE_BLOCK_MAP.study,
      ...JAPANESE_BLOCK_MAP.ad,
    ];
  }
})();
...
function buildJapaneseTitle(treatment, region, situation, purpose, seoData, mode, titleSuffixOpts) {
  const menu = treatment.menu || treatment.menuRef || "";
  const cat  = treatment.cat || "일식";
  const sit  = situation || "";
  const pur  = purpose   || "";
  ...
  if (mode === "commercial") {
    ...
    const pools = {
      titlePatterns: [],
      SCENE:        JAPANESE_TITLE_SCENE,
      SCENE_BY_CAT: JAPANESE_TITLE_SCENE_BY_CATEGORY,
      MIDDLE:       JAPANESE_TITLE_MIDDLE,
      SUFFIX:       JAPANESE_TITLE_SUFFIX,
    };
...
function buildJapaneseHashtags(treatment, region, situation, purpose, mode) {
  ...
  const catKw = (treatment.cat || "일식").replace(/\s/g, "");
...
// ============================================================
function cleanJapaneseText(text, treatment, region, situation, purpose, mode = "personal") {
  const menu = treatment.menu || treatment.menuRef || "";
  const direction = getJapaneseDirection(treatment, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  let result = text;

  const removeList = [...JAPN_FORBIDDEN_BASE, ...JAPN_FORBIDDEN_AI, ...JAPN_CROSS_BLOCK];
  if (mode === "commercial") removeList.push(...JAPN_FORBIDDEN_COMMERCIAL);
  // ★ 계열 교차차단 — 현재 메뉴의 cat 기준 "다른 계열 핵심 표현"만 제거 (taste 결 충돌 방지)
  //   JAPANESE_CROSS_CAT_BLOCK[cat] = 그 계열에서 차단할 타계열 묘사어. cat 미매칭 시 무동작.
  {
    const _cat = treatment.cat || treatment.catRef || "";
    const _ccb = (JAPANESE_CROSS_CAT_BLOCK && JAPANESE_CROSS_CAT_BLOCK[_cat]) || [];
  ...
  if (particleErrorCount > 0) {
    console.log(`[japanese][QC] 조사오류 교정: ${particleErrorCount}건 (메뉴+조사+장소 패턴 → "이 장소"로 자동 교정)`);
  }
  if (menuMetaCount > 0) {
    console.log(`[japanese][QC] 메뉴 fingerprint 차단: ${menuMetaCount}건 ("${menu}+조사+메뉴" → "이 한 그릇/이거/그것" ★ Phase 9.5 v4)`);
  }
  if (adEvalFixed > 0) {
    console.log(`[japanese][QC] 광고평가어 치환: ${adEvalFixed}건 (기대이상/만족/든든 류 → 행동·장면 표현)`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (priceStripCount > 0) {
    console.log(`[japanese][QC] ★ v1.2 가격 출력 제거: ${priceStripCount}건 (숫자 가격 → "부담 없는 가격")`);
  }
  // ─────────────────────────────────────────────────────
  ...
  if (particleDupCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 조사중복/이종2연 제거: ${particleDupCount}건 (은은/는은/은을/이메뉴은 등 → 단일화)`);
  }
  if (subjParticleFixCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 주격조사 자연화: ${subjParticleFixCount}건 (메뉴 잘/자주+서술 → 메뉴+이/가)`);
  }
  if (efficacyFixCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 효능표현 제거: ${efficacyFixCount}건 (숙취해소/건강 단정 → 행동·선택 표현)`);
  }
  if (storeRefFixCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 매장지칭 일반화: ${storeRefFixCount}건 (이 가게의/그 집의 → 메뉴명/이 메뉴는)`);
  }
  if (menuDupCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 메뉴명 중복 제거: ${menuDupCount}건 (○○ 떡볶이 떡볶이 → ○○ 떡볶이)`);
  }
  if (superlativeFixCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 과수식어 제거: ${superlativeFixCount}건 (완벽한/최고의/역대급 → 중립화)`);
  }
  if (unitFixCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 단위 정합: ${unitFixCount}건 (servingUnit=접시 → 그릇→접시)`);
  }
  if (dupSentenceCount > 0) {
    console.log(`[japanese][QC] ★ STEP F 동일문장 제거: ${dupSentenceCount}건 (완전일치 중복만, 유사도 미적용)`);
  }
  ...
  let generalizeFixCount = 0;
  {
    ...
    result = result
      ...
      .replace(/일부 일식당은/g, "매장에 따라")
      .replace(/일부 일식당에서는/g, "매장에 따라")
      .replace(/매장은 대부분 좌석이 넉넉하여/g, "매장에 따라 좌석 구성은 다를 수 있으며")
      .replace(/대부분 좌석이 넉넉하여/g, "좌석 구성은 매장에 따라 다를 수 있으며");
    generalizeFixCount = (before.match(/일부 가게에서는|일부 일식당은|일부 일식당에서는|매장은 대부분 좌석이 넉넉하여|대부분 좌석이 넉넉하여/g) || []).length;
  }
  ...
  let stepG2Count = 0;
  {
    ...
    result = result
      .replace(/[가-힣]{2,8}\s*전문점의\s*일식당들은/g, "일식당에서는")
      .replace(/일식당들은(?=\s)/g, "일식당에서는");
    ...
    result = result
      .replace(/일식당\s*전문점에서는/g, "일식당에서는")
      .replace(/일식\s*전문점에서는/g, "일식당에서는");

    if (before !== result) stepG2Count = 1;
  }
  if (stepG2Count > 0) console.log(`[japanese][QC] ★ STEP G2 잔여정정: 매장안내참고/이거의/한그릇가/메뉴명조합/전문점중식당들은/가게마다/방문시중복/맛조사/중식당전문점`);

  if (transFixCount > 0)     console.log(`[japanese][QC] ★ STEP G 번역체 정정: ${transFixCount}건 (그것의/그것을/그것이 → 메뉴)`);
  if (phraseDupCount > 0)    console.log(`[japanese][QC] ★ STEP G 어절중복 제거: ${phraseDupCount}건 (메뉴명 군더더기 인접반복)`);
  if (generalizeFixCount > 0) console.log(`[japanese][QC] ★ STEP G 일반화 안전화: ${generalizeFixCount}건 (일부 가게/대부분 → 매장에 따라)`);
  if (repeatCapCount > 0)    console.log(`[japanese][QC] ★ STEP G 과반복 제한: ${repeatCapCount}건 (3회째+ 축약 · 첫 2회·메뉴명 보존)`);

  // 외부 QC 집계용
  cleanJapaneseText.__lastParticleDup = particleDupCount;
  cleanJapaneseText.__lastSubjParticleFix = subjParticleFixCount;
  cleanJapaneseText.__lastEfficacyFix = efficacyFixCount;
  cleanJapaneseText.__lastStoreRefFix = storeRefFixCount;
  cleanJapaneseText.__lastMenuDup = menuDupCount;
  cleanJapaneseText.__lastSuperlativeFix = superlativeFixCount;
  cleanJapaneseText.__lastUnitFix = unitFixCount;
  cleanJapaneseText.__lastDupSentence = dupSentenceCount;
  cleanJapaneseText.__lastParticleErrors = particleErrorCount;
  cleanJapaneseText.__lastMenuMeta = menuMetaCount;
  cleanJapaneseText.__lastAdEvalFixed = adEvalFixed;
  cleanJapaneseText.__lastPlaceholderFix = placeholderFixCount;
  cleanJapaneseText.__lastTransFix = transFixCount;
  cleanJapaneseText.__lastPhraseDup = phraseDupCount;
  cleanJapaneseText.__lastGeneralizeFix = generalizeFixCount;
  cleanJapaneseText.__lastRepeatCap = repeatCapCount;
  cleanJapaneseText.__lastStepG2 = stepG2Count;
...
function runQC(text, treatment, region, situation, purpose, mode, fullKeyword) {
  ...
  //   누적 = cleanJapaneseText가 치환한 총 횟수
  // ─────────────────────────────────────────────────────
  ...
  const adEvalFixed = (typeof cleanJapaneseText.__lastAdEvalFixed === "number")
    ? cleanJapaneseText.__lastAdEvalFixed : 0;

  // cleanJapaneseText 마지막 호출 시 누적된 교정 횟수
  const particleFixed = (typeof cleanJapaneseText.__lastParticleErrors === "number")
    ? cleanJapaneseText.__lastParticleErrors : 0;
  ...
  const menuMetaFixed = (typeof cleanJapaneseText.__lastMenuMeta === "number")
    ? cleanJapaneseText.__lastMenuMeta : 0;
  ...
  console.log(`[japanese][QC] 메뉴블럭: ${hasMealBlock}`);
  console.log(`[japanese][QC] 운영정보: ${hasMealValue} (포함 ${mealValueCount}/9, 5개 이상 권장)`);
  console.log(`[japanese][QC] 지역+메뉴 결합 "${combo}": ${comboCount}회 (3회 이하 ★ Phase 9.5)`);
  console.log(`[japanese][QC] 메뉴명 "${menu}": ${menuCount}회 / 지역명 "${region}": ${regionCount}회`);
  console.log(`[japanese][QC] 조사오류 잔존(0 필수 ★ 이슈 #1): ${particleLeakCount}건 / 후처리 교정: ${particleFixed}건`);
  console.log(`[japanese][QC] 메뉴 fingerprint 잔존(0 필수 ★ v4 P1): ${menuMetaLeakCount}건 / 후처리 차단: ${menuMetaFixed}건`);
  console.log(`[japanese][QC] 광고평가어 잔존(0 권장): ${adEvalLeakCount}건 / 후처리 치환: ${adEvalFixed}건`);
  console.log(`[japanese][QC] 광고패턴(0 필수 ★ Phase 9.5): ${adPatternCount}`);
  console.log(`[japanese][QC] 추천유도(0 권장): ${recommendCount}`);
  console.log(`[japanese][QC] 의료 침투(0 필수): ${medicalLeak} / 카페 침투(0 필수): ${cafeLeak}`);
  console.log(`[japanese][QC] 🔍 fossil[꾸준한:${fossil_kkujun}|한결같:${fossil_hangyeol}|깔끔:${fossil_kkalkkeum}|뚝배기:${fossil_ttukbaegi}|김올라:${fossil_gim}|한술:${fossil_hansul}|모락:${fossil_morak}|입안가득:${fossil_ipan}] (★ v4.1 측정만)`);
  console.log(`[japanese][QC] 🔍 fossil2[촉촉:${fossil_chokchok}|마지막한술:${fossil_majimak}|첫입첫술:${fossil_cheossul}|이집의:${fossil_ijibui}] (★ v4.2 측정만)`);
  if (fullKeyword) console.log(`[japanese][QC] 완전체키워드(${fullKeyword}): ${fullKwCount}`);
  if (mode === "commercial") {
    console.log(`[japanese][QC] 1인칭(commercial 위반): ${firstPersonCount}건`);
    console.log(`[japanese][QC] 가격명시(commercial 위반): ${priceCount}건`);
  }
...
export default async function handleJapanese(req, res) {
  ...
  console.log(`[japanese] mode: ${validMode} | 상황: ${situation || "(미지정)"} | 목적: ${purpose || "(미지정)"}`);
  console.log(`[japanese][BUILD] japanese-v1.0-base (Japanese Engine 독립 · chinese v1.3 베이스 · cat 4계열 · 배포검증: 이 줄이 안 보이면 구버전 실행 중)`);

  // ── restaurant 조합 검증 ─────────────────────────────────
  const JAPN_IDS = JAPANESE_TREATMENTS.map(t => t.id);
  const isRest = JAPN_IDS.includes(program.id) || program.industry === "japanese";
  if (!isRest) {
    console.error(`[japanese] 잘못된 조합 진입 차단: ${name} / id=${program.id}`);
    return res.status(400).json({ error: `맛집 생성기에 잘못된 항목이 전달되었습니다: ${name}` });
  }

  // ── 데이터 로드 ─────────────────────────────────
  const treatmentData = JAPANESE_TREATMENTS.find(t => t.id === program.id)
    || JAPANESE_TREATMENTS[0];
  const cat  = treatmentData?.cat || "일식";
  ...
  console.log(`[japanese] 지역: ${region} | 카테고리: ${cat} | 메뉴: ${menu}`);

  // ── DIRECTION 동적 생성 (하이브리드 merge) ─────────────────
  const direction   = getJapaneseDirection(treatmentData, situation, purpose);
  const genericName = direction.genericName || "이 식당";
  const flowBias    = direction.flowBias || "";
  console.log(`[japanese] flowBias: ${flowBias || "(없음)"} | genericName: ${genericName}`);
...
- 미완성 문장·비문 금지`;
  ...
  const SECTIONS = getJapaneseSections(cat, flowBias, validMode);
  const sectionTexts = {};
  let prevTextRaw = "";

  for (const sec of SECTIONS) {
    const richPrompt = buildJapanesePrompt(sec.key, treatmentData, region, {
    ...
    const userPrompt = `업종: japanese | 지역: ${region} | 메뉴: ${menu} | 카테고리: ${cat} | 상황: ${situation} | 목적: ${purpose} | 모드: ${validMode}
${prevBlock}
---
...
${richPrompt}`;
  ...
  const altList = SECTIONS.slice(0, _altCount).map(sec => {
    const label = _JAPN_ALT_BY_KEY[sec.key] || "메뉴 사진";
    return `[이미지: ${label}]`;
  });
  console.log(`[japanese][ALT] mode=${validMode} keys=[${SECTIONS.map(s=>s.key).join(",")}] labels=[${altList.join(" ")}]`);

  // ── 제목 생성 ─────────────────────────
  let title = overrideTitle || buildJapaneseTitle(treatmentData, region, situation, purpose, seoData, validMode, _titleSuffixOpts);
  // 의료·카페 어휘가 제목에 새면 차단
  ...
  //         제목은 buildJapaneseTitle 단계에서 이미 menu 포함 보장됨
  {
    ...
    let cleanedBody = cleanJapaneseText(bodyPart, treatmentData, region, situation, purpose, validMode);
    if (validMode === "commercial") {
      cleanedBody = cleanJapaneseText(cleanedBody, treatmentData, region, situation, purpose, validMode);
    }
    assembled = titleLine + "\n\n" + cleanedBody.replace(/^\n+/, "");
  }
  ...
  console.log(`[japanese] 완료: ${charCount}자 / SEO ${seoScore}점 / mode=${validMode}`);

  // 경고
  if (qc.adPatternCount > 0)
    console.error(`[japanese] 🚨 광고 패턴 ${qc.adPatternCount}회 — Phase 9.5 안전핀 위반 (브랜드 톤)`);
  if (qc.medicalLeak > 0)
    console.error(`[japanese] 🚨 의료 어휘 침투 ${qc.medicalLeak}회`);
  if (qc.cafeLeak > 0)
    console.error(`[japanese] 🚨 카페 어휘 침투 ${qc.cafeLeak}회`);
  if (qc.comboCount > 3)
    console.warn(`[japanese] ⚠️ "${region} ${menu}" 결합 ${qc.comboCount}회 (3회 초과 — Phase 9.5 위반)`);
  if (validMode === "personal" && qc.fullKwCount < 2)
    console.warn(`[japanese] ⚠️ "${fullKeywordForQC}" 노출 ${qc.fullKwCount}회 — 키워드 밀도 부족`);
  // [Phase 9.5 A안] mealValueCount는 관찰용만 (강제 경고 제거 — 정보 SEO 회귀 차단)
  // → 콘솔 출력 L648에서 이미 노출됨
  if (validMode === "commercial") {
    if (qc.firstPersonCount > 0) console.warn(`[japanese] ⚠️ commercial 1인칭 ${qc.firstPersonCount}건 잔존`);
    if (qc.priceCount > 0)       console.warn(`[japanese] ⚠️ commercial 가격 ${qc.priceCount}건 잔존`);
  }
