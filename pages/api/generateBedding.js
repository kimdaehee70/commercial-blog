...
function buildTitle({ titleRegion, treatmentName, treatmentId }) {
  ...
  //   [세션55] 전국 공통 엔진 — 지역 미확보 시 {region} 토큰을 공백 없이 제거.
  const _rg = String(titleRegion || "").trim();
  let title = patterns[Math.floor(Math.random() * patterns.length)]
    .replace(/\{region\}\s*/g, _rg ? _rg + " " : "");
...
// [세션54][HOTFIX] 제목 지역 하드코딩 제거 — titleRegion = BEDDING_REGION_DEFAULT("용인") 고정이었다.
//   증상: 노원구 매장 글 제목이 "용인 혼수침구…" → 본문·주소·해시태그와 지역 충돌.
//   원칙: 해시태그와 동일 로직(deriveHashtagRegions) 재사용 → 첫 토큰(시 우선) 1개만 제목에 사용.
//   폴백: repRegion 미전송/파생 실패 시 기존 기본값 → 기존 동작 100% 유지(회귀 0).
//   ※ 제목 패턴·프롬프트·데이터 구조 무변경. 지역 산출 1개소만.
function deriveTitleRegion(repRegion) {
  const rg = deriveHashtagRegions(repRegion, "");
  return rg[0] || "";
}
...
export default async function handleBedding(req, res) {
  try {
    ...
    // [세션55] 전국 공통 엔진 전환 — BEDDING_REGION_DEFAULT("용인") 폴백 전면 제거.
    const region = String(userRegion || "").trim();
    // [세션55] 제목 지역 = 대표지역(repRegion) 단일 신뢰. 미확보 시 지역 없는 제목.
    const titleRegion = deriveTitleRegion(repRegion) || "";
