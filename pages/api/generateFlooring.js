// ============================================================
// pages/api/generateFlooring.js — 장판(flooring) 엔진 핸들러
// 정보형 + 시공 안내형({region} 장판 시공 업체 1인칭).
// 후기형·내돈내산·고객사례·추천·보장·최저가·비용유도 금지.
// engineBootstrap에서 register("flooring", handleFlooring)로 편입.
//
// 복제 베이스: generateDobae.js (섹션루프형). Runtime/FLOW/Handler 구조 무변경.
// 장판 추가 3점:
//   ① siteBlock 수신 — parseSite(req.body) → insertSiteBeforeHashtags 후단 1줄
//   ② 제목 전단 분기 — buildSiteTitleOrNull() → null이면 titleEngine → legacy
//   ③ 자재·두께 주입 — formatMaterial()/thickness, 미입력 시 빈 문자열(기존 흐름 유지)
//
...
import {
  ...
  getSectionAlt,
  FORBIDDEN,
} from "../../lib/flooring-prompts";
import { FLOORING_FLOW, FLOORING_SECTION_PHOTO } from "../../lib/flooring-playConfig";
...
// ── 마무리 해시태그 ─────────────────────────────
//   ★ 장판도 지역 3계층(시·구·동)이 태그로 함께 걸린다. 단지명은 태그에 쓰지 않는다
//     (매장·현장 식별 노출 최소화 — PHILOSOPHY 원칙1 정합).
function buildHashtags(region, kw, cat) {
  const rg = (region || "").replace(/\s+/g, "");
  const k = (kw || "").replace(/\s+/g, "");
  // [세션71] 자재축(마루) CAT 은 장판 토큰을 달지 않는다 — 검색군 혼입 방지.
  const isWood = String(cat || "").includes("마루");
  const tags = isWood
    ? [`#${rg}${k}`, `#${rg}마루`, `#${rg}마루시공`, `#${k}`, `#${rg}바닥재`, `#${rg}강마루`]
    : [
    `#${rg}${k}`,
    `#${rg}장판`, `#${rg}장판시공`,
    `#${k}`, `#${rg}바닥재`, `#${rg}장판교체`,
  ];
  // 중복 제거 — kw가 "전체장판"이면 `#${rg}${k}` 와 `#${rg}장판`은 다르지만,
  //   메뉴명이 "장판"으로 끝나 충돌 가능. Set으로 일괄 정리.
...
//   → 메뉴 확장은 flooring-data.js 단독 작업. 핸들러를 다시 열지 않는다.
function pickInfoBlock(t) {
  const B = (typeof getInfoBlocks === "function" ? getInfoBlocks(t.cat) : null)
    || FLOORING_INFO_BLOCKS;
  return B[t.infoKey] || B[t.cat] || FLOORING_INFO_BLOCKS.prebook;
}
...
function buildTitle(region, treatment, site) {
  ...
  if (_s) return _s;
  const _t = buildIntentTitleOrNull(region, treatment, "flooring");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment);
}

export default async function handleFlooring(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
      material,                                  // ③ 자재(브랜드) — 업종 소유 입력값
      thickness,                                 //    두께(T) — 장판 판단 축. 미입력 시 무영향
    } = req.body;

    // 장판 = 출장/현장방문 업종 → 고정 사업장 위치블록 미노출(PATCH-07)
    const _locStore = {};
    ...
    const treatment =
      FLOORING_TREATMENTS.find((t) => t.id === program?.id) ||
      FLOORING_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `장판 메뉴 매칭 실패: ${program?.name}` });
    }

    const kw = treatment.name;
    const mat = formatMaterial(material);
    const ctx = { site, material: mat, thickness: thickness || "" };
    const systemPrompt = buildSystemPrompt(region, treatment, ctx);

    const writtenSections = new Set();
    const sections = [];

    for (const sec of FLOORING_FLOW) {
      ...
      if (sec.key === "closing") {
        body = body.replace(/^안녕하세요[^.\n]*(장판|마루|바닥)\s*시공\s*업체입니다[.,]?\s*/g, "").trim();
        body = body.replace(/오늘은[^.\n]*안내해?\s*드리겠습니다[.,]?\s*/g, "").trim();
      }

      const slot = FLOORING_SECTION_PHOTO[sec.key];
    ...
    // 마무리 해시태그
    content += buildHashtags(region, kw, treatment.cat);
    content = content.replace(/\n{3,}/g, "\n\n").trim();
    ...
    const rgKwCount = region
      ...
      : 0;
    console.log(`[QC][flooring] 메뉴(${kw}): kw ${kwCount} / 지역+kw 결합 ${rgKwCount} (3 이하 정상)`);
    console.log(`[QC][flooring] 글자수: ${content.length}`);
    console.log(`[QC][flooring] 현장정보: ${site.siteName ? `${site.siteName} ${site.siteSize}` : "미입력"} / 자재: ${mat || "미입력"} / 두께: ${thickness || "미입력"}`);
    console.log(`[QC][flooring] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[flooring] 오류:", err);
    return res.status(500).json({ error: err.message || "장판 글 생성 오류" });
  }
}
