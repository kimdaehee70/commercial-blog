// ============================================================
// pages/api/generatePaint.js — 페인트공사(paint) 엔진 핸들러
// 정보형 + 출장 시공 안내형({region} 페인트 시공 업체 1인칭).
// 후기형·내돈내산·고객사례·추천·보장·최저가·비용유도·수명약속 금지.
// engineBootstrap에서 register("paint", handlePaint)로 편입.
//
// 복제 베이스: generateWaterproof.js (섹션루프형). Runtime/FLOW/Handler 구조 무변경.
// waterproof 대비 변경 1점:
//   ① 해시태그 어휘 — 페인트 검색축(도장/페인트/시공)으로 교체. dedup 는 동형 Set 유지.
...
import {
  ...
  getSectionAlt,
  FORBIDDEN,
} from "../../lib/paint-prompts";
import { PAINT_FLOW, PAINT_SECTION_PHOTO } from "../../lib/paint-playConfig";
...
//     ("보장" → 보장하다 / "완벽" / "최고" / "1위" / "평생"(평생교육 등 오삭제 방지)
//   프롬프트에는 금칙어를 나열하지 않는다(출력 복사 방지). 제거는 여기서만.
const STRIP_SKIP = new Set(["최고", "완벽", "1위", "보장", "평생"]);
...
// ── 마무리 해시태그 ─────────────────────────────
//   ★ 메뉴명이 "도장"으로 끝나 `#{rg}{k}` 와 `#{rg}도장` 이 겹칠 수 있다 → Set 일괄 정리.
...
//   → 메뉴 확장은 paint-data.js 단독 작업. 핸들러를 다시 열지 않는다.
function pickInfoBlock(t) {
  const B = (typeof getInfoBlocks === "function" ? getInfoBlocks(t.cat) : null)
    || PAINT_INFO_BLOCKS;
  return B[t.infoKey] || B[t.cat] || PAINT_INFO_BLOCKS.prebook;
}
...
//   ★ paint 는 siteBlock 미사용(useSite:false) → 단지명 분기 없음.
...
function buildTitle(region, treatment) {
  const _t = buildIntentTitleOrNull(region, treatment, "paint");
  if (_t) return _t;
  return buildTitleLegacy(region, treatment);
}

export default async function handlePaint(req, res) {
  try {
    const {
      program, userRegion, region: regionFallback,
      symptom,                                   // 접수된 발견(손상 흔적) — 미입력 시 무영향
      part,                                      // 원인군(film·putty·damp·substrate·rust·floor)
    } = req.body;

    // 페인트공사 = 출장 시공 업종 → 고정 사업장 위치블록 미노출(PATCH-07)
    const _locStore = {};
    ...
    const treatment =
      PAINT_TREATMENTS.find((t) => t.id === program?.id) ||
      PAINT_TREATMENTS.find((t) => t.name === program?.name);

    if (!treatment) {
      return res.status(400).json({ error: `페인트공사 메뉴 매칭 실패: ${program?.name}` });
    }
    ...
    const rgKwCount = region
      ...
      : 0;
    console.log(`[QC][paint] 메뉴(${kw}): kw ${kwCount} / 지역+kw 결합 ${rgKwCount} (3 이하 정상)`);
    console.log(`[QC][paint] 글자수: ${content.length}`);
    console.log(`[QC][paint] 발견: ${sym || "미입력"} / 원인군: ${pt ? pt.label : "미입력"}`);
    console.log(`[QC][paint] 문장잘림 의심: ${countBrokenSentences(content)}건 (0이어야 정상)`);
    ...
    console.error("[paint] 오류:", err);
    return res.status(500).json({ error: err.message || "페인트공사 글 생성 오류" });
  }
}
