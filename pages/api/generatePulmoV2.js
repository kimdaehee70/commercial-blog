// ╔══════════════════════════════════════════════════════════╗
// ║ pages/api/generatePulmoV2.js — 호흡기내과 V2 Purpose       ║
// ║ generateGastroV2 골격 복제 + 호흡기 치환.                  ║
// ║ self-contained — v1 없음(신규 업종, V2 단독).             ║
// ║ ★ 고유 축: decisionAxis 1필드로 4섹션 분기                 ║
// ║   exam(3종) = 검사 선택 기준 / disease(9종) = 치료 결정    ║
// ║ SOP v4.2 PATCH-07: locationBlock 후단 1줄.                ║
// ║ ⚠ 관측 전. FREEZE 아님.                                   ║
// ╚══════════════════════════════════════════════════════════╝

import { PULMO_TREATMENTS } from "../../lib/pulmo-data";
import {
  buildPulmoV2Prompt,
  PULMO_V2_SYSTEM_PROMPT,
  getPulmoV2ImageAlts,
  getPulmoV2Direction,
} from "../../lib/pulmo-v2-prompts";
import { PULMO_V2_FLOW_ENGINE } from "../../lib/pulmo-v2-playConfig";
...
// ============================================================
const PULMO_CAT_FOCUS = {
  "검사":            { note: "검사 선택 기준과 확인 범위가 축입니다. 질환 치료 설명으로 흐르지 않습니다." },
  "기도질환":        { note: "증상 양상 확인 → 폐기능·영상 → 흡입 약물 중심 접근 순의 판단 축입니다." },
  "감염질환":        { note: "원인 확인 → 영상·검사 → 약물 접근 → 경과 확인 순의 판단 축입니다. 전신 상태가 다음 단계를 가릅니다." },
  "만성폐질환":      { note: "폐기능 수치와 흡연력이 중심 축. 약물과 생활 관리가 함께 이어집니다." },
  "폐결절·수면호흡": { note: "이전 결과와의 변화·중증도 확인이 중심 축. 추적 간격과 관리 방향이 결정됩니다." },
};
...
function countViolations(text) {
  let n = 0;
  PULMO_V2_FORBIDDEN.forEach(w => { if (text.includes(w)) n++; });
...
function buildTitle(treatment, region) {
  ...
  const d    = getPulmoV2Direction(treatment.id);
...
function buildHashtags(name, region) {
  ...
  return "#" + reg + base + " #" + reg + "호흡기내과 #" + base + "정보 #호흡기진료안내";
}
...
export default async function handler(req, res) {
  ...
  const treatment =
    PULMO_TREATMENTS.find(t => t.id === program?.id) ||
    PULMO_TREATMENTS[0];
  ...
  const dir     = getPulmoV2Direction(tid);
  const axis    = dir.decisionAxis === "exam" ? "exam" : "disease";
  const catNote = (PULMO_CAT_FOCUS[treatment.cat] || { note: "확인 항목과 결정 기준을 중심으로 서술합니다." }).note;

  const finalTitle = buildTitle(treatment, region);
  let result = "# " + finalTitle + "\n\n";

  const imgAlts = getPulmoV2ImageAlts(treatment, region, activeKeyword);
  ...
  // ── 섹션 루프 ──
  for (const sec of PULMO_V2_FLOW_ENGINE.sections) {
    const basePrompt = buildPulmoV2Prompt(sec.key, treatment, region);
