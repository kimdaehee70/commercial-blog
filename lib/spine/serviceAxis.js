...
// ─────────────────────────────────────────────────────────
export const MENU_AXIS_INDUSTRY = new Set(["interior", "restaurant"]);
...
export function buildObservationCore(industry, region, cluster, treatmentId, treatmentName) {
  ...
  // ─────────────────────────────────────────────────────────
  // [RESTAURANT-CORE-MENU-AXIS-01] 음식점 — 생활권 + 메뉴.
  //
  //   근거(실측 n=423): region 이 「노원구 석계역」·「노원구 공릉동」 2단 복합으로 저장된다.
  //     그대로 쓰면 「노원구 석계역 통뼈닭발」 — 실제로 검색되지 않는 조합이 된다.
  //     음식점 검색 단위는 행정구가 아니라 생활권이다.
  //       노원구 석계역 + 통뼈닭발  →  석계역 통뼈닭발
  //       노원구 공릉동 + 순대국    →  공릉동 순대국
  //
  //   ★ region 마지막 토큰 정규화는 restaurant 에만 적용한다.
  //     병원군·interior·funeral 은 아래 공통 블록을 그대로 탄다(무수정).
  //   ★ 확장 7계열(chinese/korean/snack/japanese/western/chicken/meat)은 아직 OFF.
  //     검색시장 검증 후 한 계열씩 등재한다(ONE INDUSTRY AT A TIME).
  //   ※ region 입력 자체의 행정구역 오염(예: 「노원구 신내동」= 실제 중랑구)은
  //     별도 축 REGION-INPUT-CONTAMINATION-01. 여기서 교정하지 않는다.
  //   ※ tid 공백 = 메뉴 미선택 → 이 분기에 들어오지 않고 기존 폴백으로 간다.
  // ─────────────────────────────────────────────────────────
  if (ind === "restaurant" && tid && tnm) {
    const r = String(region || "").trim();
    const last = r ? (r.split(/\s+/).pop() || "") : "";
    return `${last} ${tnm}`.trim() || null;
  }
