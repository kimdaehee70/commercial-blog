...
// 마지막 사진은 기본적으로 CTA 구간에 붙는다 — 본문에서 소비하지 않는다.
// tailPhoto:false 를 넘기면 이 규칙을 끄고 전 역할을 본문에서 소비한다(세션118).
...
export function renderPhotoMarkers(text = "", roles = DEFAULT_PHOTO_ROLES, { tailPhoto = true } = {}) {
  const list = Array.isArray(roles) && roles.length ? roles : DEFAULT_PHOTO_ROLES;
  // 마지막 역할은 후단(CTA) 전용 → 본문 치환 대상에서 제외.
  // ★ 세션118 — tailPhoto:false 이면 후단 사진 없이 전 역할을 본문에서 소비한다.
  //   외관·입구가 신뢰 정보가 아닌 업종을 위한 경로. 기본값 true라 기존 엔진 동작 무변경.
  const bodyRoles = tailPhoto ? list.slice(0, Math.max(1, list.length - 1)) : list.slice();

  let i = 0;
  let out = String(text).replace(MARKER_RE, () => {
    const role = bodyRoles[i] || bodyRoles[bodyRoles.length - 1];
    i += 1;
    return `${CONSULT_ICONS.photo} ${role}`;
  });

  // ★ PHOTO-01 (세션115) — 모델이 [사진]을 덜 출력해도 요구 수량을 코드가 보장한다.
  //   프롬프트 지시는 보장이 아니다. 결정론적 항목의 최종 보장은 코드에서 한다.
  //   이미 출력된 마커는 건드리지 않는다. 부족분만 문단 경계에 보충한다.
  if (i < bodyRoles.length) {
    const r = fillPhotoMarkers(out, bodyRoles, i);
    out = r.text;
    i = r.used;
  }

  return { text: out, used: i, bodyRoles, tailRole: tailPhoto ? list[list.length - 1] : "" };
}

/* ── 부족분 보충 ────────────────────────────────
   삽입 지점: 문단(빈 줄) 경계. 첫 문단 직후·마지막 문단 직전은 제외한다.
     · 첫 문단 직후 — 도입부가 끊기면 검색자가 이탈한다
     · 마지막 문단 직전 — 후단 블록(CTA·📍·📷tail)과 붙어 사진이 연속 출력된다
   가용 경계가 부족하면 넣을 수 있는 만큼만 넣는다(문단 한가운데 삽입 금지).
   ──────────────────────────────────────────── */
function fillPhotoMarkers(text, bodyRoles, usedAlready) {
  const paras = String(text).split(/\n{2,}/);
  const need = bodyRoles.length - usedAlready;

  // 경계 index = 그 문단 "뒤"에 삽입. 0(첫 문단 뒤)과 마지막 경계 제외.
  const slots = [];
  for (let p = 1; p < paras.length - 2; p++) {
    if (paras[p].includes(CONSULT_ICONS.photo)) continue;      // 이미 사진이 붙은 문단 뒤는 건너뛴다
    if (paras[p + 1] && paras[p + 1].startsWith(CONSULT_ICONS.photo)) continue; // 연속 방지
    slots.push(p);
  }
  if (!slots.length) return { text, used: usedAlready };

  // 균등 배분 — 앞뒤로 몰리지 않게 슬롯을 등간격으로 고른다.
  const take = Math.min(need, slots.length);
  const picked = [];
  for (let k = 0; k < take; k++) {
    const at = Math.floor(((k + 1) * slots.length) / (take + 1));
    const s = slots[at];
    picked.push(typeof s === "number" ? s : slots[k]);
  }

  const uniq = [...new Set(picked)].sort((a, b) => a - b);

  let n = usedAlready;
  // 뒤에서부터 삽입해야 앞선 index가 밀리지 않는다.
  for (let k = uniq.length - 1; k >= 0; k--) {
    const role = bodyRoles[usedAlready + k];
    if (!role) continue;
    paras.splice(uniq[k] + 1, 0, `${CONSULT_ICONS.photo} ${role}`);
    n += 1;
  }
...
/* ── 해시태그 블록 (세션118) ──
   메타성 tail 전용. 본문(body)에는 절대 넣지 않는다.
   ※ storeNameInBody:false 원칙은 본문 대상이며, tail 식별 태그는 별개 축이다.
   이 파일은 업종 지식을 갖지 않는다 — 태그 산출은 엔진이 하고 여기서는 조립만 한다. */
export function buildHashtagBlock(tags = []) {
  const list = (Array.isArray(tags) ? tags : [tags])
    .map((t) => String(t || "").replace(/\s+/g, "").replace(/^#+/, ""))
    .filter(Boolean);
  if (!list.length) return "";
  return [...new Set(list)].map((t) => "#" + t).join(" ");
}

/* ═════════════════════════════════════════════
   4. 후단 조립 — 순서 고정
   [본문] → 📞문의 문장 → 방문정보(📍🕒🚗🚇) → 📷tail 사진(선택) → ℹ️고지 → #해시태그
   ═════════════════════════════════════════════ */
export function appendTail(body = "", { region, ctaLine, notices = [], tailPhotoRole, visit, hashtags = [] } = {}) {
...
export function renderConsultPost(body = "", opts = {}) {
  const { photoRoles, region, ctaLine, notices, visit, hashtags, tailPhoto = true } = opts;
  const p = renderPhotoMarkers(body, photoRoles, { tailPhoto });
