...
function _buildDecisionAssets(treatment) {
...
③ 시설: 빈소 규모·안치실·접객실(식당)·편의시설을 "조문객 몇 명 규모인지"와 연결해 서술.
④ 이용 안내: 조문 시간대·동선·유족 대기 등 실제 이용 시 확인할 항목.
⑤ 장례지도사 연결: 시설만으로 결정되지 않는 부분(빈소 규모 판단·절차 진행·화장장 조율)을 담당 장례지도사가 맡는다는 점으로 자연 연결.
...
- 본문 = 판단 기준. "조문객이 많다면 주차와 식당 규모를 함께 확인하는 것이 좋습니다" 처럼 '왜 확인하는가'.
- 하단 블록 = 확인된 사실(주소·주차 대수·빈소 수·식당 좌석). 숫자와 시설 목록은 블록이 담당한다.
...
[상품 연결 — 마무리 직전 1문장만]
- 상담 안내 직전에 "장례 형태와 조문 규모에 따라 적합한 상품 구성이 달라질 수 있어 상담 과정에서 함께 안내드립니다" 수준 1문장으로 하단 상품 블록과 연결한다.
- 상품명·가격·구성 비교는 본문에서 절대 반복하지 않는다(하단 블록 소관).
${hasFacts
...
- 시설 항목을 설명하는 대신, 조건 → 확인 항목 형태로 쓴다:
  · 조문객이 적으면 작은 빈소 운영 여부를 먼저 확인한다.
  · 조문객이 많으면 주차 대수와 식당 좌석 회전을 먼저 확인한다.
  · 화장이 이른 시각이면 화장장까지의 이동 거리를 먼저 확인한다.
  · 원거리 유족이 있으면 대기 공간과 조문 시간대를 확인한다.
- ★ 마무리는 상담 안내가 아니라 판단 기준으로 닫는다.
  (예: 장례식장은 규모보다 현재 장례 형태와 예상 조문객 수에 맞는지를 먼저 확인하는 것이 중요합니다.)`,
...
export function buildPrompt({ treatment, region, storeName = "{storeName}", hallName = "", hallFacts = null }) {
  ...
  const decisionAssets = _buildDecisionAssets(treatment); // [세션59] cat별 판단자산
...
[지역] ${region}${hallLine}
[주제] ${treatment?.name || ""}
[화자] ${hallName ? `${hallName} ` : ""}장례지도사 (안내자 톤)
...
[작성 지침]
- 도입부: "안녕하세요. ${hallName ? `${hallName} ` : ""}장례지도사입니다."로 시작. 상조회사 상호명은 쓰지 말 것. 유가족의 막막함에 차분히 공감하되 감성 과장 금지.${hallRule}
