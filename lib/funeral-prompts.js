...
④ 일반 확인 정보: 위 [일반 정보층]에 제공된 문장을 하나도 빠뜨리지 않고 독립 단락으로 쓴다. ③의 값을 다시 꺼내지 않는다. 이 단락은 시설 사실과 무관한 일반 서술이다.` : `③ 시설: [확인된 시설 정보]에 실제로 등록된 항목만 서술한다. 등록되지 않은 항목은 언급조차 하지 않는다. 항목이 적으면 이 단락을 짧게 쓰거나 ④와 합쳐도 된다.
...
- ④(일반 확인 정보) = 제공된 문장 수가 곧 분량이다. 백분율 목표는 없다. 문장을 늘려 분량을 맞추지 않는다.
...
export function buildPrompt({ treatment, region, storeName = "{storeName}", hallName = "", hallFacts = null }) {
  ...
  const flow = intent === "hall" ? _flowHall(!!factsBlock, treatment, hallFacts) : _flowBereaved(treatment);
