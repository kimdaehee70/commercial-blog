...
export function buildPrompt({ treatment, region, storeName = "{storeName}", hallName = "", hallFacts = null, _collect = null }) {
  ...
  // [S4-DET-01] 이번 요청의 S4 원문을 후처리로 전달한다. 프롬프트에는 넣지 않는다(composer 에서 제거).
  //   ★ _collect 는 호출측이 만든 요청 로컬 객체다. 모듈/전역 상태로 승격 금지(serverless 요청 간 오염).
  if (_collect) _collect.practical = topicPlan ? topicPlan.s4.map((p) => p.text) : [];
