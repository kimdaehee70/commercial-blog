...
[일반 정보층 — 사실층과 분리해서 쓴다]
- 장례 일반 지식(장례 절차, 조문 예절, 준비 순서 등)은 써도 된다. 글이 사실 나열만으로 끝나지 않게 하는 층이다.
- 단, 주어를 이 장례식장으로 쓰지 않는다. "일반적으로 ~합니다" 형태로 쓰고, 이 시설의 사실인 것처럼 연결하지 않는다.
  · 허용: "일반적으로 빈소는 안치 이후에 정해집니다."
  · 금지: "경희의료원 장례식장에서는 빈소를 안치 이후에 정합니다." (등록되지 않은 사실)
- 일반 정보와 [확인된 시설 정보]를 한 문장 안에서 인과로 묶지 않는다.
