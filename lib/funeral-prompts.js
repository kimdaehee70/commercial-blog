...
② 위치·접근: ${hasFacts ? `[확인된 시설 정보]의 주소 값을 그대로 서술하는 데서 끝낸다. 교통수단·소요시간·접근 난이도·편리함·방문 용이성은 확인된 값이 아니다. 쓰지 않는다.` : `어디에 있고 어떻게 가는지. 주차 확인이 필요한 이유와 함께.`}
...
⑤ 장례지도사 연결: ${hasFacts ? `담당 장례지도사가 가족을 지원한다는 점만 한 문장으로 연결한다. 이 장례식장이 어떤 절차·순서·진행 방식을 따르는지는 확인된 값이 아니므로 서술하지 않는다. 일반 장례 지식을 이 시설의 사실로 연결하지 않는다.` : `시설 정보만으로 결정되지 않는 부분(빈소 규모 판단·절차 진행)을 담당 장례지도사가 맡는다는 점으로 자연 연결.`} 화장장 관련 내용은 [확인된 시설 정보]에 화장장 항목이 있을 때만 쓴다.
⑥ 상담: ${hasFacts ? `상담·문의·맞춤 안내를 권하는 단락을 만들지 않는다. 확인된 값의 서술이 끝나면 그대로 글을 닫는다.` : `한 단계만 안내하고 짧게 닫는다.`}
