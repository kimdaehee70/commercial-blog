...
export const FUNERAL_TREATMENTS = [
  // ── A. 장례비용 ──────────────────────────────────────────────
  {
    ...
    cat: "장례비용",
    emoji: "💳",
    // [FUNERAL-OTHER-MENU-EXPOSURE-01 · INTENT FIX / A안]
    //   기존 4패턴(가족장/3일장/무빈소/화장 비용)은 제목만 Intent 특화였고
    //   BODY 는 DIRECTION 1세트 기준의 장례비용 총론만 생성한다(제목은 GPT 호출 이후
    //   pickTitle 로 사후 랜덤 선택 — 선택된 Intent 가 프롬프트에 전달되지 않는 구조).
    //   → 실측: "무빈소 장례 비용" 제목에 소형·중형·대형 빈소 설명이 나옴(Intent 충돌).
    //   총론 BODY 가 실제로 생성하는 범위 안의 제목만 남긴다. 신규 Intent 생성 0.
    //   ★ "가족장"은 funeral_familycost 전용 — 여기서 중복 사용 금지.
    //   ★ HOLD(별도 treatment 승격 후보, 지금 구현 금지):
    //       funeral_3day_cost / funeral_noparlor_cost / funeral_cremation_cost
    titlePatterns: [
      "{region} 장례비용 무엇에 따라 달라지나요",
      "{region} 장례비용 항목별 안내",
      "조문 규모별 {region} 장례비용 정리",
    ],
  ...
  // ── B. 장례절차 ──────────────────────────────────────────────
  {
    ...
    cat: "장례절차",
    emoji: "📋",
    // [FUNERAL-OTHER-MENU-EXPOSURE-02 · INTENT FIX]
    //   "가족장 진행 순서 안내" 제거. DIRECTION 은 절차 총론 1세트뿐이라
    //   가족장 전용 절차(빈소 축소·조문 최소화 등)를 BODY 가 생성하지 못한다.
    //   제목은 GPT 호출 이후 pickTitle 로 사후 랜덤 선택되므로 구조적 충돌.
    //   ★ HOLD(지금 구현 금지): funeral_family_procedure — 가족장 전용 절차 treatment.
    titlePatterns: [
      "사망 후 장례 절차 정리",
  ...
  },
  {
    ...
    cat: "장례절차",
    emoji: "🕯️",
    // [FUNERAL-OTHER-MENU-EXPOSURE-03] BODY 미지원 Intent 제거.
    //   지역 토큰형·화장예약 단독형은 DIRECTION.effect 4요소를 지원하지 않는다.
    titlePatterns: [
      "사망 후 가장 먼저 해야 할 일",
    ],
...
export const FUNERAL_FAQ_DATA = [
  ...
  { q: "장례식장을 옮길 수 있나요?", a: "안치 이후에는 이송 절차와 추가 비용이 발생합니다. 이송 시 화장 예약과 발인 일정도 다시 조율해야 합니다.", intent: "hall" },
