...
function photoBlock(specId) {
  // ★ 세션118 사진축 — tail 사진 폐지. n장 전부 본문에서 소비한다.
  //   (기존: 마지막 1장을 후단 CTA 구간에 부착 → 외관·입구 제거로 해당 자리 자체가 없어졌다)
  const inBody = Math.max(1, getPhotoCount(specId));
...
/* ─────────────────────────────────────────────
   8-B. 제목 꼬리 반향 제거 (세션104)
   ★ 증상: 제목 "꿈 해몽, 무엇을 물어봐야 하나" → 본문 첫 줄 "무엇을 물어봐야 하나"
     모델이 제목 꼬리를 소제목처럼 한 번 더 쓴다.
     checkShamanTitle의 BODY_COPY는 '본문이 제목으로 시작'만 잡아서 이 형태를 놓친다.
   ★ 원칙: 최종 보장은 코드(세션102). 프롬프트 지시로 막지 않는다.
   꼬리 목록은 제목 풀에서 직접 산출한다 — 풀이 바뀌면 자동으로 따라온다.
   ───────────────────────────────────────────── */
const TITLE_TAILS = (() => {
  const tpl = [
    ...Object.values(SHAMAN_TITLE_POOLS || {}).flat(),
    ...Object.values(SHAMAN_TITLE_PATTERNS || {}).flat(),
  ];
  const set = new Set();
  for (const t of tpl) {
    const tail = String(t)
      .replace(/\{situation\}|\{specialty\}|\{region\}/g, "")
      .replace(/^[\s,·—\-–]+/, "")
      .trim();
    if (tail.length >= 6) set.add(tail);
  }
  return [...set];
})();
...
/* 본문 첫 줄이 제목 꼬리와 동일하면 그 줄만 제거. 그 외에는 원문 무변경. */
export function stripEchoedTitleTail(raw = "") {
  const lines = String(raw).split("\n");
  let i = 0;
  while (i < lines.length && !lines[i].trim()) i++;
  if (i >= lines.length) return raw;

  const head = normalizeHeadLine(lines[i]);
  if (!head || head.length > 40) return raw;
  if (!TITLE_TAILS.some((t) => head === normalizeHeadLine(t))) return raw;
...
/* ═════════════════════════════════════════════
   9. 원샷 헬퍼 (핸들러에서 사용)
   순서 고정: 꼬리 반향 제거 → FORBIDDEN 검사 → 사진 마커 치환 → 후단(위치·문의·사진·고지·해시태그) 부착
   ※ 후단을 먼저 붙이면 고지문이 금지어 검사 대상이 된다.
   ═════════════════════════════════════════════ */
/* ── 상황 사진 라벨 (세션118) ──
   situation.scenes(전 35건 보유, 각 3개)를 그대로 쓴다. 신규 데이터 0개.
   목적: 사용자가 "무슨 사진을 올려야 하는지" 바로 알게 한다.
   situationId가 없는 경로(Engine B 분야 소개글)는 빈 문자열 → 상황 사진 자체가 생략된다.
   ※ 없는 정보를 만들지 않는다 — scenes가 없으면 라벨도 만들지 않는다. */
export function situationPhotoRole(situationId) {
  const s = situationId ? getSituation(situationId) : null;
  const sc = (s && Array.isArray(s.scenes) ? s.scenes : []).filter(Boolean);
  if (!sc.length) return "";
  return `상황 사진 — ${sc.join(" · ")}`;
}

/* ── 해시태그 (세션118) ──
   구조: 생활권+서비스 / 서비스 일반 / 상담예약 / 업체명 1개
   · 신규 상수 0개 — SHAMAN_TITLE_SERVICES 재사용
   · 업체명은 tail 태그 1개까지만. 본문 노출은 storeNameInBody:false 로 계속 차단된다.
   · 빈 값(지역·업체명 미입력)은 해당 태그만 자동 생략된다. */
export function buildShamanHashtags(region = "", storeName = "") {
  const squash = (v) => String(v || "").replace(/\s+/g, "");
  const r = squash(region);
  const out = [];
  if (r) SHAMAN_TITLE_SERVICES.forEach((sv) => out.push(r + squash(sv)));
  out.push("신점상담", "상담예약");
  const sn = squash(storeName);
  if (sn) out.push(sn);
  return out;
}

export function finalizeShamanPost(rawInput, opts = {}) {
  const raw = stripEchoedTitleTail(rawInput);
  ...
  const visit = normalizeVisitInfo(opts.visitInfo, opts.storeFields, {
    // ★ 세션114 — 📍 상담소 위치는 주소축(업체 실주소). 검색축(region=생활권)은 폴백으로만 남긴다.
    address: opts.address || opts.region || "",
  });

  // ★ 세션118 사진축 — [상황 사진] + [상담 공간] + [상담 준비] = 기본 3장, tail 사진 없음.
  //   상황 사진이 없는 경로(분야 소개글)는 상담 2장으로 자연 축소된다.
  const sitRole = situationPhotoRole(opts.situationId);
  const photoRoles = [sitRole, ...SHAMAN_META.photoRoles]
    .filter(Boolean)
    .slice(0, getPhotoCount(specId));
  ...
  const r = renderConsultPost(raw, {
    ...
    visit,
    hashtags: buildShamanHashtags(
      opts.region || "",
      (opts.storeFields && opts.storeFields.storeName) || ""
    ),
  });
