...
function _personalConcern(name, region, pains, surgeryGuide, nonSurgeryEmphasis, aiSmellGuide, kwDensityGuide, grammarGuide) {
  return `
${region} 정형외과 ${name} 진료 과정의 첫 번째 섹션입니다. 일반적인 진료 흐름을 3인칭 설명 시점으로 안내합니다.
...
- 어미는 설명체: "~합니다 / ~되는 경우가 있습니다 / ~확인할 수 있습니다 / ~나타나기도 합니다 / ~진행됩니다"
...
- 상황 묘사로 시작 (장면형 도입 OK, 단 1인칭 금지):
  ...
  ✅ "퇴근 무렵 계단을 오를 때 무릎 통증이 반복되는 경우가 있습니다"
  ✅ "출근 중 허리 통증이 반복적으로 나타나기도 합니다"
...
- ⚠️ 모든 문장은 서술어로 끝맺음 (예: "~선택지로." "~것이." "~데." "~사례로." 처럼 중도 종료 절대 금지)
- 분량: 200~300자
- 말투: ~합니다 / ~됩니다 / ~나타나기도 합니다 / ~확인할 수 있습니다 (일반 진료과정 설명체)
`.trim();
}
...
- 어미: ~합니다 / ~진행됩니다 / ~확인할 수 있습니다 / ~나타나기도 합니다
- "솔직/추천/꼭" 등 광고 어휘 금지
...
- "검토/확인" 반복 지양 — 정보 종류를 다양화(무엇을·왜·어떻게 활용) 하여 서술
...
- ⚠️ 모든 문장은 서술어로 끝맺음 (예: "~선택지로." "~것이." "~데." "~사례로." 처럼 중도 종료 절대 금지)
- 분량: 200~300자
- 말투: ~합니다 / ~확인할 수 있습니다 / ~검토하는 과정이 진행됩니다 (일반 진료과정 설명체)
`.trim();
}
...
[톤 — 진료과정 설명형]
- 1인칭 시점 절대 금지 ("저는/제가" 등 금지)
- 어미: ~합니다 / ~확인할 수 있습니다 / ~안내됩니다 / ~설명됩니다
- "솔직/추천/꼭" 등 광고 어휘 금지
- 의사 직접 인용은 허용 (사실 기록), 단 단정 표현 금지

[주제] ${region} 정형외과 ${name} 상담 단계 기록
[조건]
- 많이 궁금해하는 질문 1개를 일반화하여 제시 (비용·횟수·회복 기간 중 하나)
  예: "많은 분들이 '${name}은 몇 회 정도 진행되나요?'라는 점을 궁금해합니다" 형태
  ❌ "원장님, 실비 적용되나요?" (실비 직접 언급 지양)
- 일반적인 안내 내용 1회 (특정 환자 아님, 단정 표현 금지):
  ✅ "진료 횟수는 개인 상태에 따라 달라질 수 있습니다"
...
- 검사는 "어떤 검사를·왜 하는지·결과를 치료계획에 어떻게 활용하는지" 흐름으로 다양화 ("검토/확인" 반복 금지)
- 분량: 250~350자
- 말투: ~합니다 / ~안내됩니다 / ~확인할 수 있습니다 (일반 진료과정 설명체)
`.trim();
}
...
function _personalReason(name, region, compareWith, isDisc, isSurgery, aiSmellGuide, kwDensityGuide, grammarGuide = "") {
  ...
  const surgNote = isSurgery
    ...
    : `\n- 비수술 진료 선택 배경을 사실 위주로 기록 (회복 과정·부담은 개인 상태·진단 결과에 따라 다를 수 있다는 표현)`;
...
[톤 — 진료과정 설명형]
- 1인칭 시점 절대 금지 ("저는/제가" 등 금지)
- 어미: ~합니다 / ~검토합니다 / ~결정하는 과정이 진행됩니다 / ~확인할 수 있습니다
- 광고 어휘 금지: "솔직/추천/꼭"
...
- ⚠️ 모든 문장은 서술어로 끝맺음 (예: "~선택지로." "~것이." "~데." "~사례로." 처럼 중도 종료 절대 금지)
- 분량: 200~300자
- 말투: ~합니다 / ~검토합니다 / ~결정하는 과정이 진행됩니다 (일반 진료과정 설명체)
`.trim();
}
...
function _personalResult(name, region, operationNotes, isSurgery, isProcedure, isManual, isShoulder, isFoot, isKnee, aiSmellGuide, kwDensityGuide, grammarGuide = "") {
  ...
  if (isSurgery) {
    timelineFormat = "치료 직후 / 초기 경과 / 중기 경과 / 이후 경과 확인 단계";
    specificNote   = "\n- 일상 복귀 시점은 일반적 경과로 기술 (단정 금지)";
  } else if (isProcedure) {
    timelineFormat = "치료 직후 / 초기 경과 / 회차 진행 경과 / 이후 경과 확인 단계";
    specificNote   = "\n- 회차별 변화는 관찰형으로 (예: \"통증 변화를 단계별로 살펴봅니다\")";
  } else if (isManual) {
    timelineFormat = "초기 회차 / 진행 경과 / 이후 경과 / 경과 확인 단계";
...
[톤 — 진료과정 설명형]
- 1인칭 시점 절대 금지 ("저는/제가" 등 금지)
- 어미: ~합니다 / ~살펴봅니다 / ~확인할 수 있습니다 / ~점차 ~됩니다 / ~진행됩니다
- 효과 단정 절대 금지 ("통증 10→5 줄어듦" 같은 단정 수치 금지)
  → 대신 "통증 변화를 살펴봅니다", "점진적 회복 단계로 진행됩니다" 정도
...
[조건]
- ${timelineFormat} 형식으로 단계별 변화를 일반화하여 설명${specificNote}
- 구체적 차수/주차/개월차 단정 금지: "경과 확인은 개인 상태에 따라 수일~수개월에 걸쳐 진행될 수 있습니다"로 일반화
- 참고 정보: ${operationNotes}
- 변화 표현은 관찰형:
  ${symptomExamples}
- 검사·진료 단계는 사실만 기록:
  ✅ "이학적 검사를 통해 가동범위 회복 정도를 단계별로 확인합니다"
  ✅ "회복 경과는 개인차가 있으며 일반적으로 ~로 진행됩니다"
  ❌ "팔이 자연스럽게 올라가요" / "잠을 푹 자게 됐어요" (1인칭 체험 금지)
- 분량: 300~400자
- 말투: ~합니다 / ~살펴봅니다 / ~경과를 확인합니다 (일반 진료과정 설명체)
`.trim();
}
