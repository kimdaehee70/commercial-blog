...
export function StoreInfoForm({ hubStore, setHubStore, saveStore, createStore, storeSaving, industryLabel, hubLoading, isOwner, initialPick, initialSpecialty, onGoIndustryCenter, onCoachVideo, authUserId, editRef, INDUSTRY_CONFIG, lex }) {
  ...
  // [INDUSTRY-SELECT-UI-NOSYNC-01] initialPick 값의존 복원.
  //   navView 전환(blog→store)으로 폼 인스턴스가 교체되면 pending 이 소비되지 못해 첫 클릭만 실패한다.
  //   부모 확정값(industryCenterSel)을 따라오게 해 인스턴스 교체·유지 양쪽을 모두 덮는다.
  //   미확정 계정 한정 — 확정계정 편집(editIdent)에는 개입하지 않는다.
  useEffect(() => {
    if (!industryConfirmed && initialPick && initialPick !== pickIndustry) setPickIndustry(initialPick);
  ...
  const VISIT_INFO_FIELDS_FUNERAL = [
    // [S149] ① 상품정보 · ② 서비스정보 UI 제거.
    //   근거: 서비스 5필드(24시간 상담/출동·365접수·긴급·지도사)는 업체 간 반복률이 높아 차별화 Facts가 아님.
    //         상품정보는 상품 편집기 자체가 입력 부담 대비 소비 근거 약함.
    //   ★ 화면 제거일 뿐 DB 삭제 아님 — onSaveVisit payload가 기존 visit_info 를 spread 후 병합하므로
    //     consult24/dispatch24/receive365/urgent/director/funeralProducts 저장값은 그대로 남는다.
    //   ★ FUNERAL_SERVICE_DEFAULTS / productEditor 렌더 분기는 무수정 존치 —
    //     VISIT_INFO_KEYS 에 키가 없어 진입하지 않는다(사문화, 무영향). 되돌릴 때 이 배열만 복원.
    // ③ 상담정보 — 유지. 대표전화는 업체 고유 Facts이며 공공 장례식장 DB로 대체되지 않는다.
    ...
    // ⑤ 출동지역 UI 제거 [S149] — 입력 부담 대비 차별화 Facts 아님.
    //   ★ 화면 제거일 뿐 DB 삭제 아님(payload 병합 규칙 동일). 되돌릴 땐 이 배열만 복원.
  ];
  ...
  //   ★ ctpv/sigungu 는 입력 필드가 아니라 자동완성 선택으로만 채워지는 식별자다(화면 미노출).
  const FUNERAL_HALL_BLANK = {
    name: "", ctpv: "", sigungu: "",
    address: "", parking: "", parkingFee: "", restaurant: "",
  ...
  // ── [01E-UI] 장례식장 공공데이터 자동완성 ────────────────────────────
  //   ★ [S149 방향 전환] 선택 → address/parking/halls/mortuary 자동입력 → visit_info 저장 구조 폐기.
  //     근거: funeral_halls_public(1080건)이 서버 정본인데 같은 Facts를 사용자 업체정보에
  //     복제 저장하면 이중 출처가 된다. Facts 는 생성 시점에 서버가 런타임 조립한다
  //     (FUNERAL-PUBLIC-RUNTIME-INJECT-01, pages/api/generate.js).
  //   ★ 여기 남는 것은 검색·선택으로 '공식 장례식장명'을 확정하는 기능뿐이다.
  //     name 만 공식명으로 교체 — 소비단이 hallName 완전일치 1건으로 찾으므로 표기가 곧 식별자다.
  //   ★ 나머지 필드(주차요금·식당·편의시설·화장장·메모 등 공공데이터에 없는 항목)는 사용자 입력 유지.

  // 편집기 내 name input 은 한 번에 하나만 포커스된다 → 단일 상태로 충분(idx로 소유권 구분).
  const [hallAcIdx, setHallAcIdx] = useState(-1);
  const [hallAcItems, setHallAcItems] = useState([]);
  const hallAcTimer = useRef(null);
  const hallAcSeq = useRef(0);

  const closeHallAc = () => { setHallAcIdx(-1); setHallAcItems([]); };

  const onHallNameInput = (idx, val) => {
    // [FUNERAL-HALL-IDENTITY-MERGE-01] 사용자 타이핑은 항상 그대로 반영.
    //   ★ 동시에 이전 선택의 지역 식별자를 무효화한다 — 이름만 바뀌고 ctpv/sigungu 가
    //     남으면 "다른 시설명 + 옛 지역"으로 조회되어 오정보가 된다. 식별자는 선택으로만 생긴다.
    setVisitInfo(prev => ({
      ...prev,
      funeralHalls: (prev.funeralHalls || []).map((h, i) =>
        i === idx ? { ...h, name: val, ctpv: "", sigungu: "" } : h
      ),
    }));
    if (hallAcTimer.current) clearTimeout(hallAcTimer.current);
    const q = String(val || "").trim();
    if (q.length < 2) { closeHallAc(); return; }   // API와 동일 기준(2자)
    ...
    hallAcTimer.current = setTimeout(async () => {
      try {
        ...
        if (j && j.ok && Array.isArray(j.items) && j.items.length) {
          setHallAcItems(j.items); setHallAcIdx(idx);
        } else { closeHallAc(); }
      } catch { closeHallAc(); }                   // 실패 시 조용히 닫음 — 수동 입력 경로 무영향
    }, 300);
  };

  //   ★ 공식명만 확정한다. public Facts(주소·주차·빈소·안치)는 복제 저장하지 않는다.
  //   ★ [FUNERAL-HALL-IDENTITY-MERGE-01] ctpv/sigungu 는 Facts 가 아니라 '식별자'라서 저장한다.
  //     근거: 시설명은 유일하지 않다(T2 실측 — 중복 27종/66행). 이름만 남기면 생성 시점에
  //     어느 지역 시설을 골랐는지가 소실되어 다른 지역 Facts 가 붙을 수 있다.
  //     ctpv/sigungu 는 이 후보를 내려준 같은 테이블의 값이므로 조회 시 정규화가 필요 없다.
  const pickHallCandidate = (idx, it) => {
    setVisitInfo(prev => ({
      ...prev,
      funeralHalls: (prev.funeralHalls || []).map((h, i) =>
        i === idx
          ? { ...h, name: it.name, ctpv: it.ctpv || "", sigungu: it.sigungu || "" }
          : h
      ),
    }));
    hallAcSeq.current++;                           // 진행 중 요청 무효화
    closeHallAc();
  };
  ...
  const onApplyIdent = async () => {
    ...
    if (_industryChanged) {
      try {
        // [#223] 토큰 인증 — 서버는 body auth_user_id 무시, Bearer user.id만 사용
        const { data: _isess } = await supabase.auth.getSession();
        const _itoken = _isess?.session?.access_token;
        const ir = await fetch("/api/store-industry", {
          method: "POST",
          headers: { ...(_itoken ? { Authorization: `Bearer ${_itoken}` } : {}), "Content-Type": "application/json" },
      ...
      <div style={{ background: "linear-gradient(180deg,#fbf5ff,#fff)", borderRadius: 14,
        ...
        <div>
          ...
          <input value={form.sub_region} onChange={e => { set("sub_region", e.target.value); setSubRegionConfirm(false); }}
            placeholder={isPro ? "생활권 3~5개 입력 (쉼표로 구분) · 구 단위 권장 · 예: 노원구, 성북구, 중랑구" : "생활권 3~5개 입력 (쉼표로 구분) · 예: 공릉동, 태릉입구역, 하계동"} style={strategyInputStyle} />
          ...
          {subRegionConfirm && (
            <div style={{ marginTop: 12, background: "#fff8e1", border: "1.5px solid #ffe082",
              ...
              <div style={{ fontSize: 12, color: "#7a6a3a", lineHeight: 1.65 }}>
                {isPro ? "보통은 3~5개의 생활권(구 단위)을 함께 설정합니다. 많을수록 검색 노출이 넓어집니다." : "보통은 3~5개의 생활권을 함께 설정합니다. 많을수록 검색 노출이 넓어집니다."}<br />
        ...
        <div role="button" tabIndex={0}
          ...
          <span style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 2, textAlign: "left" }}>
            ...
            <span style={{ fontSize: 11.5, color: "#9457b8", fontWeight: 600 }}>
              {isFuneral
                ? "대표전화 · 주로 진행하는 장례식장 — 채우면 글에 자동 반영됩니다"
                : "채우면 글에 자동 반영됩니다"}
            </span>
          </span>
                  ...
                  <div key={f.key} style={{ gridColumn: "1 / -1" }}>
                    {(visitInfo.funeralHalls || []).map((h, idx) => (
                      <div key={idx} style={{ border: "1px solid #ece3f5", borderRadius: 10, padding: 10, marginBottom: 8, background: "#faf7fd" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 8px" }}>
                          {/* [01E-UI] 자동완성 — wrapper 는 드롭다운 기준점(position:relative) 용도로만 신설 */}
                          <div style={{ gridColumn: "1 / -1", position: "relative" }}>
                            <input value={h.name || ""} onChange={e => onHallNameInput(idx, e.target.value)}
                              onBlur={() => setTimeout(closeHallAc, 150)}
                              autoComplete="off"
                              placeholder="장례식장명 (예: 을지대병원 장례식장)" style={{ ...inputStyle, width: "100%" }} />
                            {hallAcIdx === idx && hallAcItems.length > 0 && (
                              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 30,
                                marginTop: 2, background: "#fff", border: "1.5px solid #e0d0f0", borderRadius: 8,
                                boxShadow: "0 6px 18px rgba(80,40,120,.13)", overflow: "hidden" }}>
                                {hallAcItems.map((it, k) => (
                                  <div key={k}
                                    onMouseDown={e => { e.preventDefault(); pickHallCandidate(idx, it); }}
                                    style={{ padding: "7px 10px", fontSize: 11.5, cursor: "pointer",
                                      borderTop: k ? "1px solid #f2ecf8" : "none" }}>
                                    <span style={{ fontWeight: 700, color: "#4a3560" }}>{it.name}</span>
                                    <span style={{ color: "#a99bbb", marginLeft: 6 }}>
                                      {it.ctpv}{it.sigungu ? " " + it.sigungu : ""}
                                    </span>
                                  </div>
                                ))}
                                <div style={{ padding: "5px 10px", fontSize: 10, color: "#b0a3c0", background: "#faf7fd" }}>
                                  ↳ 선택하면 공식 명칭으로 확정됩니다
                                </div>
                              </div>
                            )}
