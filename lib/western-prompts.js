// =============================================
// lib/western-prompts.js
// 양식(레스토랑) 프롬프트 빌더 v1.0 — Western Engine 독립
//
// 기반: restaurant-prompts.js 구조 동형 (Restaurant 계열 엔진)
//   · 함수명 western화, import = ./western-data.js
//   · commercial(메뉴 정보형) 기본. personal 빌더는 보존(restaurant 동형)
//   · 중식·일식 전용 예시는 양식 톤으로 중립화 (§3① 정정)
...
import { buildDirection, getPurposeProfile } from './western-data.js';

// ============================================================
// 0. DIRECTION 헬퍼 — 데이터 조합으로 동적 생성
// ============================================================
export function getWesternDirection(treatment, situation, purpose) {
  if (!treatment) {
...
메뉴명("${menu}") + 조사 사용 시 문장이 끊기지 않게 자연스럽게:
  ✅ "${menu}을/를 먹으러", "${menu} 한 접시", "${menu}이 유명한"
...
예시 방향:
  ...
  · 부담감(가격 X): "부담 없이 한 접시" / "가볍게 한 끼" (숫자 가격 절대 금지)
...
// 1. 메인 빌더 (mode 분기)
// ============================================================
export function buildWesternPrompt(section, treatment, region, options = {}) {
  const { mode = 'personal', situation = '', purpose = '' } = options;
  if (mode === 'purpose') {
    return buildPurposeWesternPrompt(section, treatment, region, options);
  }
  if (mode === 'commercial') {
    return buildCommercialWesternPrompt(section, treatment, region, options);
  }
  return buildPersonalWesternPrompt(section, treatment, region, options);
}

// ── personal 모드 ──
function buildPersonalWesternPrompt(section, treatment, region, options = {}) {
...
- ❌ 메뉴 표기는 '${menu}'만 사용. 동의어·상위어(파스타·스테이크·면 등) 임의 생성 금지 — 본문 전체 '${menu}'로 통일.
...
${direction.extraDetail ? `- 추가 디테일: ${direction.extraDetail}` : ''}`;
  ...
  switch (section) {
    ...
    default: throw new Error(`[western-prompts] 알 수 없는 섹션: ${section}`);
  }
}

function _personalVisit(genericName, region, menu, situation, purpose, common) {
  return `
당신은 ${region} 일대를 자주 다니는 일반인입니다. ${region}에서 ${menu} 한 끼 먹은 경험을 1인칭 블로그 후기로 작성합니다.
...
- 소스·후추·치즈가루 더하는 같은 본인의 행동 1~2회 포함
...
function buildCommercialWesternPrompt(section, treatment, region, options = {}) {
  const { situation = '', purpose = '' } = options;
  const { name, cat = '양식', menu = treatment.menuRef || '', compareWith = '' } = treatment;
  const direction = getWesternDirection(treatment, situation, purpose);
  const genericName = direction.genericName || name || '이 식당';
  const unit = direction.servingUnit || '한 그릇';        // ★ 메뉴별 단위 (피자=한 판, 파스타=한 접시 등)
...
[조건]
- 이 메뉴를 찾게 되는 상황을 메뉴 기준으로 (예: "추운 날 뜨거운 국물이 당길 때 찾기 좋은 메뉴")
...
- ❌ 이 메뉴에 없는 재료 임의 추가 금지 (예: 고기 단품에 국물·내장을 끼워넣지 말 것)
...
- 예: "크림 소스가 진한 편", "굽기에 따라 호불호 갈리는 편", "소스를 따로 받으면 면이 덜 불어 식감이 유지되는 편"
...
[조건]
- 곁들임·추가 메뉴·조합을 정보형으로 (예: 피클·수프, 메인에 곁들이는 빵·샐러드 조합 ${sidedishes || ''})
...
- 호불호 기준을 구체적으로 (예: "진한 국물을 선호하면 잘 맞고, 담백한 걸 찾으면 다른 선택")
...
// ============================================================
// 2.5 [Pilot] purpose 모드 — 목적 중심 4섹션 빌더
//   축: PURPOSE → PLACE EXPERIENCE → MENU(수단) → STORE INFO
//   최상위 원칙: 주인공 = '검색자의 방문 목적'. 메뉴는 목적 해결 수단.
//   화법: 검색자 상황에서 출발. 조언형·광고·매장명 노출 금지. scene 강화.
//   섹션 key(4): purposeIntro · placeExperience · menuAsMeans · storeInfo
// ============================================================
function buildPurposeWesternPrompt(section, treatment, region, options = {}) {
  const { situation = '', purpose = '' } = options;
  const { cat = '양식', menu = treatment.menuRef || '', nearbyHint = '' } = treatment;
  // [지역 일치 게이트] nearbyHint가 현재 region과 무관하면(타지역 지명 누출 방지) 폐기.
  //   region 문자열이 힌트에 포함될 때만 통과. 부작용 0(빈값=미주입).
  const safeNearbyHint = (nearbyHint && region && nearbyHint.includes(region)) ? nearbyHint : '';
  const direction = getWesternDirection(treatment, situation, purpose);
  const genericName = direction.genericName || '이 레스토랑';

  const prof = getPurposeProfile(purpose);
  // 폴백: 목적 미매칭 시 최소 골격만 (메뉴중심 회귀 아님 — 일반 목적 톤 유지)
  const purposeLabel = (prof && prof.titleLabel) || purpose || '방문';
  const openings = (prof && prof.openingSituations) || [
    `${region}에서 양식 레스토랑을 찾고 있다면`,
  ];
  const fitReason = (prof && prof.fitReason) || '양식 레스토랑은 자리 흐름이 여유로워 목적에 맞춰 시간을 보내기 좋다';
  const placeExp = (prof && prof.placeExperience) || [
    '좌석 간격과 조명이 목적에 맞는 분위기',
    '테이블 구성이 인원과 상황에 맞는 배치',
  ];
  const menuMeans = (prof && prof.menuAsMeans) || {
    framing: '메뉴는 목적에 맞는 구성으로 고르면 흐름이 자연스럽다',
    picks: ['목적에 맞춰 나눠 먹기 좋은 구성'],
  };
  const sceneTail = (prof && prof.sceneTail) || '목적에 맞춰 자리가 채워지는 흐름';

  // 최상위 화법 규칙 (목적 중심 공통)
  const purposeFrameGuide = `
[목적 중심 화법 — 최상위 규칙 ★★ 위반 시 전면 실패]
- 주인공은 '${purposeLabel}'라는 방문 목적. 메뉴 설명글·후기 아님.
- 글은 반드시 '검색자의 상황'에서 출발한다. 메뉴("파스타는~"·"스테이크는~")로 시작 금지.
- 메뉴는 목적을 푸는 '수단'. 메뉴 자체 소개·맛 단정이 아니라 '이 목적엔 이렇게 고르면 좋다'는 선택 이유로만 등장.
- ❌ 광고 단정 금지: "최고", "원조", "찐맛집", "꼭 가보세요", "강추", "인생"
- ❌ 독자 조언·권유 남발 금지: "~하시는 분들께 추천" 류. 상황 제시("~라면")까지만 허용.
- ❌ 매장명 본문 직접 노출 금지 — 지역·골목·일대·공간 지시어로 자연 치환('${genericName}'/'여기'/'이곳').
- ❌ 지역명(${region}) 반복 금지: 글 전체 3회 이하. 매 문단 첫머리 반복 금지 → "이 동네/근처/일대"로 치환.
- ❌ 효능·의학 단정 금지, 의료·카페·학습 어휘 금지.
- ❌ 메뉴 표기는 '${menu || cat}'만 사용. 동의어(파스타·스테이크·면 등) 임의 생성 금지.
- ❌ "${menu || cat} 지역"·"${menu || cat} 동네" 같은 결합 금지.
- ✅ '${purposeLabel}' 목적을 가장 먼저 해결하는 흐름으로 전개.
- ✅ 공간 경험(분위기·좌석·조명·간격·창가/예약석)을 목적과 연결해 구체 장면으로.
- ✅ 문단 2~4줄. 읽기 쉬운 안내·묘사체.`;

  const common = [purposeFrameGuide, getAiSmellGuide(), getKwDensityGuide(genericName, region, menu)].join('\n');

  const openingHint = openings.map(o => `  · "${o}"`).join('\n');
  const placeHint = placeExp.map(p => `  · ${p}`).join('\n');
  const picksHint = (menuMeans.picks || []).map(p => `  · ${p}`).join('\n');

  const sectionGuides = {
    // ① 목적·상황 도입 — 검색자 상황에서 출발
    purposeIntro: `
[섹션 주제] '${purposeLabel}' 목적으로 이곳을 찾는 상황 도입
[조건]
- 반드시 검색자의 상황에서 시작. 아래 도입 방향 중 하나의 결로 첫 문장을 연다(그대로 복제 금지, 자연 변주):
${openingHint}
- 그다음 왜 양식 레스토랑이 '${purposeLabel}'에 맞는지 다리를 놓는다: ${fitReason}
- ❌ 메뉴("파스타는~")로 시작 금지. ❌ 방문 후기 1인칭 서사 아님.
- 목적: ${purposeLabel} / 상황: ${situation || '(일반)'}
- 분량: 250~350자`,

    // ② 공간 경험 — 목적을 푸는 자리 (핵심)
    placeExperience: `
[섹션 주제] '${purposeLabel}'에 맞는 공간 경험 (★이 글의 핵심 섹션)
[조건]
- 이 자리가 왜 '${purposeLabel}' 목적에 맞는지 공간 경험으로 구체화. 아래 요소 중 3개 이상을 장면으로 풀어낸다:
${placeHint}
- 분위기·좌석·조명·테이블 간격·창가/예약석·인원 적합성을 '${purposeLabel}' 목적과 연결.
- ${sceneTail}
- ❌ 메뉴 설명으로 새지 말 것 — 이 섹션 주인공은 '공간'.
- ❌ "분위기 맛집"·"감성 가득" 추상 표현 금지 → 구체적 자리·배치·간격으로.
- 분량: 350~460자`,

    // ③ 메뉴(목적 해결 수단)
    menuAsMeans: `
[섹션 주제] '${purposeLabel}' 목적을 푸는 메뉴 구성 (수단)
[조건]
- 메뉴는 목적 해결 수단. 메뉴 자체 소개·맛 단정이 아니라 '${purposeLabel}'엔 이렇게 고르면 좋다는 선택 이유로.
- 구성 방향: ${menuMeans.framing}
- 아래 구성 예 중 1~2개를 '${purposeLabel}' 목적과 엮어 자연스럽게(나열 금지, 흐름으로):
${picksHint}
- ❌ 메뉴명 단순 나열·맛 단정("맛있다") 금지. ❌ 가격 숫자 금지.
- 메뉴명은 목적을 설명하는 근거로만 등장. 메뉴가 주인공이 되지 않게.
- 분량: 280~380자`,

    // ④ 업체 정보 (보조 한정)
    storeInfo: `
[섹션 주제] '${purposeLabel}' 달성에 필요한 업체 정보 (보조)
[조건]
- 위치·좌석 구성·예약/단체 가능·포장 여부 등 '${purposeLabel}' 목적에 필요한 정보까지만.
- ⚠ 매장이 주인공이 되면 후기형 회귀 — 어디까지나 목적 보조 정보로 한정.
- ❌ 매장명 본문 직접 노출 금지 (지역·골목·일대·공간 지시어로).
- ${safeNearbyHint ? `참고 위치 결: ${safeNearbyHint}` : ''}
- 분량: 200~300자`,
  };

  const guide = sectionGuides[section]
    || `[섹션 주제] '${purposeLabel}' 목적 안내\n- 목적 중심. 메뉴는 수단. 광고 단정·매장명 노출 금지.\n- 분량: 200~300자`;

  return `
${region} ${purposeLabel} 레스토랑 — 목적 중심 글의 [${section}] 섹션만 작성.

${guide}
${common}

---
이 섹션만 작성. 검색자의 목적 해결이 최우선. 메뉴는 목적을 푸는 수단.
메뉴 설명글로 회귀 금지 · 광고 단정 금지 · 매장명 본문 노출 금지.
문단은 2~4줄로 유지.
`.trim();
}
...
// ============================================================
export function getWesternImageAlts(treatment, region, options = {}) {
  const { situation = '', purpose = '' } = options;
  const menu = treatment.menu || treatment.menuRef || '';
  const cat = treatment.cat || '양식';
  const r = region || '';

  // Restaurant 5슬롯 SoT 순서: 외관 / 실내·좌석 / 대표 메뉴 / 메뉴판 / 분위기·디테일
  //   슬롯 순서·개수는 photoPolicyRegistry 소유. 엔진은 ALT 문구만 업종화.
  return [
    `${r} ${menu} 가게 외관 사진`,
    `${r} ${menu} 실내·좌석 배치`,
    `${r} ${menu} 대표 메뉴 한 접시`,
    `${r} ${menu} 메뉴판 사진`,
    `${r} ${menu} 매장 분위기·디테일${situation ? ' (' + situation + ')' : ''}${purpose ? ' · ' + purpose : ''}`,
  ].filter(s => s.trim());
}
