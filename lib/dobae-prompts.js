...
// [S217 DOBAE-INTENT-BODY-ALIGNMENT-01] Intent focus layer.
//   Scene stays as-is. Intent decides WHAT the scene must explain toward.
const INTENT_FOCUS = {
  cost:    { label: "비용 결정요인", axis: "벽 상태·철거량·평탄도·밑작업량·천장 포함 여부·보양 난이도·자재 선택이 비용을 어떻게 바꾸는지", close: "지금 보이는 이 상태가 왜 비용 차이로 이어지는지" },
  trouble: { label: "자주 생기는 문제", axis: "실제로 자주 나오는 문제와 그 원인, 어디까지가 정상이고 어떤 상태가 재확인 대상인지", close: "이 문제를 미리 줄이려면 무엇을 보고 무엇을 정하는지" },
  compare: { label: "범위 결정 기준", axis: "전체와 부분, 천장 포함 여부, 부속 공간까지 갈지를 무엇을 보고 정하는지", close: "이 현장이라면 범위를 어디까지로 정하는지" },
};
function intentBlock(ctx) {
  const it = ctx && ctx.intent ? INTENT_FOCUS[ctx.intent] : null;
  if (!it) return "";
  return [
    ``,
    `[Intent 중심축 — 이 글이 답해야 하는 것: ${it.label}]`,
    `- ⭕ 이 섹션의 장면은 그대로 갑니다. 다만 장면 하나하나가 다음으로 연결되어야 합니다: ${it.axis}`,
    `- ❌ 공정 순서만 나열하고 끝내지 마세요. 본 것 → 그래서 ${it.label}이 어떻게 갈리는지까지 이어 씁니다.`,
    `- ⭕ 섹션을 닫을 때는 ${it.close}로 닫습니다.`,
    `- 분량 배분: ${it.label} 축 60~70% / 현장 작업 흐름 30~40%.`,
    ``,
  ].join("\n");
}
export function buildUserPrompt(region, treatment, secKey, ctx) {
  ...
  const B = base(rg, kw) + intentBlock(ctx);
