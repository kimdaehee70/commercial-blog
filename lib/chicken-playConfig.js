// ============================================================
// lib/chicken-playConfig.js — 치킨 FLOW_ENGINE 구조 (Chicken Engine 독립, Japanese 복사 베이스)
...
// generateChicken.js의 getImageAlts와 짝을 이룸
// ─────────────────────────────────────────────────────────
export const CHICKEN_PHOTO_SPOTS = {
  visit:   null,  // 사진 없음 (도입 텍스트)
  arrive:  '외관·간판·입구 동선',
  order:   '메뉴판·상차림 세팅·반찬 구성',
  taste:   '음식 클로즈업 (튀김옷·속살·소스 표면 디테일)',
  scene:   '테이블 분위기·창가·옆자리 흐름·식사 중 풍경',
  revisit: '식당 마무리 컷 (계산대·출구·간판)',
};

// ─────────────────────────────────────────────────────────
// CHICKEN_FLOW_ENGINE — 6섹션 구조
// 섹션 키 6개: visit · arrive · order · taste · scene · revisit
// generateChicken.js의 writtenSections·prompt 빌더와 반드시 일치
// ─────────────────────────────────────────────────────────
export const CHICKEN_FLOW_ENGINE = {
  industry: 'chicken',
  ...
  sections: [
    ...
    },
    {
      ...
      label: '맛·식감',
      order: 4,
      description: '튀김옷·속살·소스 등 핵심 맛 묘사. 첫 입 → 식감 → 온도 → 향. 정보 나열 금지, 한 입의 체감 중심',
      required: true,
      minLength: 300,
      maxLength: 400,
      photo: '음식 클로즈업 (튀김옷·속살·소스 표면 디테일)',
    },
    {
      key: 'scene',
      label: '장면·분위기',
    ...
    },
    {
      ...
      label: '재방문·추천',
      order: 6,
      description: '재방문 의사 + 어떤 상황·목적에 추천하는지 (점심/혼밥/가족모임 등)',
      required: true,
      minLength: 200,
      maxLength: 250,
  ...
  // 차단 키워드 (chicken-data.js의 CHICKEN_BLOCK_MAP과 일치)
  blockKeywords: [
    // 의료 침투 방지
    ...
    // ★ 한식 narrative 침투 차단 (Chicken 독립 ecosystem — Naver §3 전략2)
  ...
  // 필수 키워드 (최소 1개 포함) — 치킨 결 (계열 무관 공통)
  requiredKeywords: [
    '소스', '맛', '식감', '한 그릇', '한 입', '한 점',
    '치킨', '메뉴', '요리', '재방문',
  ],
  ...
  // generateChicken.js의 injectMealValue()에서 사용
  ...
  // 맛집 전용 추가 블럭 (generateChicken.js에서 주입)
  extraBlocks: [
    'WAITING_BLOCKS',     // 웨이팅 정보
    'SIDEDISH_BLOCKS',    // 반찬·기본찬 구성
    'BEST_TIME',          // 방문 추천 시간대
    'MENU_COMBINATION',   // 메뉴 조합 추천 (치킨+사이드+음료 등)
...
// ============================================================
// [v2 / 2026-06-25] CHICKEN_COMMERCIAL_FLOW_ENGINE — 8섹션 메뉴 중심 정보형
// ⚠ 기존 CHICKEN_FLOW_ENGINE(6섹션 personal)은 무수정 보존 — 롤백 안전
...
// ============================================================
export const CHICKEN_COMMERCIAL_FLOW_ENGINE = {
  industry: 'chicken',
  mode: 'commercial',
  ...
  sections: [
    ...
    },
    {
      ...
      label: '메뉴장면(소비상황)',
      order: 2,
      description: '이 메뉴가 어울리는 소비 상황 — 추운 날 찾기 좋은 / 든든한 한 끼로 / 혼밥으로 등. "갔다·먹었다" 1인칭 서사 금지. 메뉴와 상황의 결합만',
      required: true,
      minLength: 180,
      maxLength: 260,
    ...
    },
    {
      ...
      label: '메뉴구성',
      order: 3,
      description: '한 마리/한 접시의 구성 요소 — 들어가는 재료·비율·양 가늠·기본 구성. 관찰 가능한 사실 위주 (예: 부위 구성·소스 양·순살 여부)',
      required: true,
      minLength: 280,
      maxLength: 380,
    ...
    },
    {
      ...
      label: '맛특징(관찰형)',
      order: 4,
      description: "관찰 가능한 맛 특징만. '맛있다' 금지 → '양념이 진한 편', '튀김옷이 두꺼운 편', '간이 센 편/슴슴한 편' 등 선택에 도움되는 기술. 1인칭 체험 금지",
      required: true,
      minLength: 300,
      maxLength: 400,
      photo: '음식 표면·단면 디테일',
    },
    {
      ...
      label: '곁들임',
      order: 5,
      description: '함께 먹으면 좋은 구성 — 곁들임·추가 메뉴·조합(치킨무·콜라·사이드). 정보형. 강요·광고 표현 금지',
      required: true,
      minLength: 180,
      maxLength: 260,
      photo: '곁들임·반찬·조합 컷',
    ...
    },
    {
      ...
      label: '추천상황',
      order: 7,
      description: '어떤 상황·목적에 이 메뉴가 맞는지 — 점심/혼밥/가족모임/가벼운 식사 등. 3인칭 정보형. "꼭 가보세요" 류 광고 종결 금지',
      required: true,
      minLength: 180,
      maxLength: 260,
      photo: null,
  ...
  blockKeywords: [
    ...CHICKEN_FLOW_ENGINE.blockKeywords,
  ],
...
//   - taste: 맛 묘사 비중↑ (점심 등 "국물 한 술" 중심 상황)
//   - scene: 분위기 묘사 비중↑ (비오는날·데이트·가족모임 등)
//   - arrive: 도착·1인석·동선 비중↑ (혼밥·야식 등)
// generateChicken.js에서 section.minLength·maxLength 보정에 사용
// ─────────────────────────────────────────────────────────
...
// ★ 치킨 4계열 분리 정의 — taste 묘사 결을 계열별로 고정 (계열 교차 표현 차단)
//   fried(순수튀김옷) / seasoned(소스결) / oven(구운결) / special(구성형)
//   ⚠ taste 결 충돌 방지가 4계열 분리의 핵심 목적 (CHICKEN_CROSS_CAT_BLOCK과 짝)
// ─────────────────────────────────────────────────────────
export const CHICKEN_CATEGORY_OVERRIDES = {
  'fried': {
    // 후라이드 — 순수 튀김옷·겉바속촉 중심. 소스 코팅·구운결 표현 금지
    taste: {
      description: '튀김옷의 바삭함과 부서지는 결, 속살의 육즙·온도, 소금·후추 간만의 담백함 한 입 체감. 소스 코팅·구운결 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 후라이드 구성(한 마리/순살) + 곁들임(치킨무·소금·소스) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
  'seasoned': {
    // 양념·간장·마늘·허니·고추 — 소스 코팅·결 중심. 순수튀김 담백·구운결 표현 금지
    taste: {
      description: '튀김 위에 입혀진 소스의 코팅·윤기, 매콤달콤/짭조름/달콤 등 양념의 결, 손끝에 묻는 소스 한 입 체감. 순수튀김 담백·구운결 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 양념 종류 구성(양념/간장/마늘/허니/고추) + 곁들임(치킨무·콜라) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
  'oven': {
    // 오븐·로스트 — 구운결·기름기 적음 중심. 튀김옷·소스 코팅 표현 금지
    taste: {
      description: '오븐에 구워 겉은 노릇하고 속은 촉촉한 살의 결, 기름에 튀기지 않은 담백함, 허브·소금 간의 풍미 한 입 체감. 튀김옷·소스 코팅 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 오븐/로스트 구성 + 곁들임(치킨무·머스터드) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
  'special': {
    // 순살·반반·닭강정·윙봉세트 — 구성·부위 조합 중심. 조리법 단정 표현 최소
    taste: {
      description: '한입 크기·부위 구성의 먹는 재미(순살/윙·봉/반반 비교), 손에 들고 먹는 편함, 두 가지 맛 번갈아 집는 한 입 체감. 조리법 단정 표현 최소',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 구성 선택(순살/반반/닭강정/윙봉세트) + 곁들임(치킨무·콜라) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
...
// ─────────────────────────────────────────────────────────
// 섹션 머지 헬퍼 — generateChicken.js에서 호출
...
export function getChickenSections(category, flowBias, mode) {
  ...
  if (mode === 'commercial') {
    return CHICKEN_COMMERCIAL_FLOW_ENGINE.sections.map(sec => ({ ...sec }));
  }

  // personal 모드 (기존 동작 — 무변경)
  const base = CHICKEN_FLOW_ENGINE.sections;
  const catOvr = CHICKEN_CATEGORY_OVERRIDES[category] || {};
...
// generateChicken.js의 getImageAlts에서 활용
// ─────────────────────────────────────────────────────────
export function getPhotoSpotForSection(sectionKey) {
  return CHICKEN_PHOTO_SPOTS[sectionKey] || null;
}
