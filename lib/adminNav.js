// lib/adminNav.js
// v1.2 [OPS-RECENT-PUBLISH-TOP-NAV-01] 상단에 '최근발행'(/admin/publish) 1줄 추가. 상단 6 → 7.
// - 목적은 발행관리 화면의 독립 진입점 확보. 화면 정본은 여전히 관측 좌측 레일 '수동관측(발행)'.
// - publish.js 가 current="/admin/publish" 를 넘기므로 그 화면에서 상단 활성은 '최근발행'이고
//   상단 '관측'은 비활성이 된다 — 선장 판정으로 정상 UX. 활성 판정 규칙은 손대지 않는다.
// - observationLayout.js · publish.js 무접촉. 라우트·페이지 신규 0.
//
// v1.1 [OBS-LEGACY-OBSERVATIONS-RETIRE-01] 관측 업무영역 통합 (DEC-022). 상단 8 → 6.
// - '발행'(/admin/publish) · '자동관측'(/admin/auto-observe) 상단에서 제거.
//   두 화면은 상단 '관측' 아래 좌측 레일(lib/observationLayout.js)이 소유한다.
// - '관측' href: /admin/observations → /admin/auto-observe (자동관측 = 기본 화면, 2026-09-10 정정).
//   /admin/observations 는 플래그 퇴역(DEC-007) — 접속 시 /admin/auto-observe 로 이동.
// - 라우트·페이지 삭제 0. 비운 자리는 채우지 않는다.
//
// v1.0 [PSEO-ADMIN-USABILITY-01] 상단 고정 교체 1:1. 총수 8개 유지.
// - 'pSEO'(/admin/pseo) : ADMIN_MORE(데이터) → ADMIN_NAV.
//   라벨은 'pSEO 관제'가 아니라 'pSEO'다 — 상단은 업무 영역, 좌측(PSEO_SUBNAV)이 세부 기능.
//   '관제'는 lib/pseoLayout.js 의 좌측 레일이 소유한다.
//   pSEO 공개상태는 결제 자격을 따라 자동으로 바뀌므로 매일 보는 화면이 됐다.
// - '시스템'(/admin/system) : ADMIN_NAV → ADMIN_MORE(점검).
//   라우트·페이지는 삭제하지 않는다(세션76 §5 — 진입점만 재배치).
// - 상단은 9개로 늘리지 않는다. 좋은 폭에서 줄바꿈이 생기면 콘솔 헤더가 깨진다.
//
...
export const ADMIN_NAV = [
  ...
  // OPS-RECENT-PUBLISH-TOP-NAV-01: 발행관리 독립 진입점.
  //   화면 정본은 관측 좌측 레일 '수동관측(발행)'이며 이 항목은 진입 경로만 추가한다.
  //   ※ publish.js 가 current="/admin/publish" 를 넘기므로 이 화면에서 상단 활성은 '최근발행'이고
  //     상단 '관측'은 비활성이 된다 — 선장 판정으로 정상 UX. 활성 규칙 수정 금지.
  { href: '/admin/publish',         label: '최근발행' },
  // OBS-LEGACY-OBSERVATIONS-RETIRE-01 (DEC-022): 관측 업무영역 대표.
  //   좌측 레일 = 자동관측 /admin/auto-observe · 수동관측(발행) /admin/publish (observationLayout.js).
  //   href 는 observationLayout.js OBS_ROOT 와 같아야 한다(활성 판정).
  { href: '/admin/auto-observe',    label: '관측' },
  { href: '/admin/status-board',    label: '상태판' },
  // PSEO-ADMIN-USABILITY-01: '시스템'과 1:1 교체. 상단 총수 8개 유지.
  { href: '/admin/pseo',            label: 'pSEO' },
];
...
export const ADMIN_MORE = [
  ...
  { group: '데이터', items: [
    ...
    // PSEO-ADMIN-USABILITY-01: pSEO는 ADMIN_NAV(상단 고정)으로 이동. 세부 기능은 좌측 레일.
  ...
  { group: '점검', items: [
    ...
    // PSEO-ADMIN-USABILITY-01: 상단에서 이동. 라우트·북마크 전부 그대로 동작.
