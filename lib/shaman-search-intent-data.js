...
// ★ 세션114 — head 단독 하한. 기존 TITLE_MINLEN(12)은 head+○○운 꼬리 합산 기준이었다.
//   꼬리 폐지 후 그대로 쓰면 '집이 안 나가요'(8) 같은 검색자 언어가 68% 탈락한다.
//   최종 제목은 뒤에 ' - {생활권} 신점 상담'(11자+)이 붙으므로 총길이 하한은 이미 충족된다.
export const HEAD_MINLEN = 6;
...
export const SHAMAN_SEARCH_INTENT = {
  ...
  biz_slow: {
    ...
    solution: ['손님이 다시 늘까', '장사가 언제쯤 풀릴까', '손님이 줄었는데 다시 늘까'],
  ...
  biz_contract: {
    ...
    solution: ['이번 계약은 될까', '이번 계약은 무사히 될까'],
  ...
  biz_partner: {
    ...
    solution: ['동업 그만두는 게 나을까', '동업 계속해도 될까'],
  ...
  biz_start: {
    ...
    cause: ['장사를 시작해도 잘할 수 있을까', '사업하면 안 되는 사주'],
  },
  biz_tangled: {
    name: '하는 일마다 꼬일 때',
    version: 2,
    reality: ['되는 일이 없어요', '하는 일마다 꼬여요', '왜 이렇게 안 풀리지'],
    solution: ['잘되던 일까지 왜 갑자기 꼬일까', '언제쯤 풀릴까'],
  ...
  doc_home: {
    ...
    solution: ['이 집으로 정해도 될까', '이사 날짜 언제가 좋을까', '이사해도 괜찮을까'],
  ...
  study_noStudy: {
    ...
    cause: ['공부해도 성적이 왜 안 오르지', '왜 집중을 못 할까'],
  },
  study_exam: {
    name: '시험만 보면 떨어질 때',
    version: 2,
    reality: ['시험 계속 떨어져요', '공무원 시험 계속 떨어짐', '공부는 했는데 안 붙어요', '또 떨어졌어요'],
    solution: ['시험 언제쯤 붙을까', '이번 시험은 붙을까', '이번엔 시험에 붙을 수 있을까'],
  ...
  study_job: {
    ...
    solution: ['언제쯤 취업될까', '면접은 붙을까', '취업이 언제쯤 될까'],
  ...
  spirit_tired: {
    ...
    cause: ['쉬어도 왜 계속 피곤할까', '집 때문인가 사람 때문인가'],
  },
  spirit_anxious: {
    name: '이유 없이 불안할 때',
    version: 2,
  ...
  spirit_child: {
    ...
    solution: ['아이가 언제쯤 안 아플까', '아이가 편해질까'],
  ...
  anc_repeat: {
    ...
    solution: ['집안에 안 좋은 일이 왜 계속 생기지', '집안일이 언제쯤 잦아들까'],
  ...
  anc_family: {
    ...
    solution: ['가족에게 왜 자꾸 안 좋은 일이 생기지', '한 사람만 계속 아픈 이유가 뭘까'],
    fortune: ['가정운', '건강운 보는곳'],
  ...
  anc_move: {
    ...
    solution: ['이사 오고 꼬인 일이 언제 풀릴까', '이사 후에 뭘 해야 할까'],
  ...
  anc_rite: {
    ...
    solution: ['제사를 지내야 할까', '제사를 안 지내도 괜찮을까'],
    fortune: ['집안운', '조상 사주', '가정운 보는곳'],
    consult: ['집안운 상담', '신점 상담', '사주 상담'],
    cause: ['성묘를 계속 미뤘는데 괜찮을까', '제사 안 지내서 그런가'],
  },
  ...
  mar_late: {
    ...
    solution: ['좋은 인연 만날 수 있을까', '언제쯤 결혼할까', '나는 왜 결혼이 늦어질까'],
  ...
  mar_back: {
    ...
    solution: ['다시 이어질까', '이제 그만 잊는 게 나을까'],
...
/* 제목 1개 조립
   ★ 세션114 Title Policy V2 — 공통 강제 ○○운 꼬리 폐지.
     최종 구조: [구체적인 현실 문제 + 궁금증] - [생활권] 신점 상담
     지역·서비스 축은 shaman-data.js applyRegionToTitle 이 담당한다(SoT).
     여기서는 검색자 언어 head 만 반환한다.
     · '결혼운/시험운'류는 강제 삽입이 아니라 head 후보로만 살아 있는다.
     · fortuneLabel / TAIL_FORMS 는 삭제하지 않는다 — 후보 복구 시 재사용. */
function buildTitle(row, situationId, maxLen = TITLE_MAXLEN) {
  const head = one(row[pickMode()]);
  if (!head) return null;
  const t = head.trim();
  if (t.length > maxLen || t.length < HEAD_MINLEN) return null;
  return t;
}

/* 완성 제목 n개 — shaman-data.js situationTitlePatterns 가 그대로 사용 */
export function intentTitlesFor(situationId, n = 2, maxLen = TITLE_MAXLEN) {
  const row = SHAMAN_SEARCH_INTENT[situationId];
  if (!row) return [];
  const out = [];
  let guard = 0;
  while (out.length < n && guard++ < 60) {
    const t = buildTitle(row, situationId, maxLen);
