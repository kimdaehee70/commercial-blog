...
export const INDUSTRY_CATALOG = [
  ...
  {
    id: "urology", name: "비뇨기과", category: "건강·의료", status: "review", enabled: true, verified: true, version: "v2",
  ...
  {
    id: "eye", name: "안과", category: "건강·의료", status: "review", enabled: true, verified: true, version: "v2",
  ...
  },
  {
    ...
    // [세션216 2026-08-25] 배지 승격 ⬜ v2-pilot/review/enabled:false → 🟦 v2/live/enabled:true.
    //   근거: 실발행 「다산 주방 리모델링, 공간이 답답하게 느껴진다면 원인 점검」 네이버 블로그탭 관련도순 1위(2주 경과 유지).
    //   flooring·door(세션71) · tile·cleaning(세션70) 동일 판정 기준.
    //   ★ 배지·게이트만 변경. 엔진 4파일 FREEZE 유지·무수정.
    id: "interior", name: "인테리어", category: "건설·시공", status: "live", enabled: true, verified: true, version: "v2", hasPhysicalStore: false,
  ...
  },
  {
    ...
    // [세션146] closed:true — FILM INTENT 10 cat / 33종 서브메뉴 전수 CLOSE. 표시 전용(enabled·status 무영향).
    // [세션216 2026-08-25] 배지 승격 ⬜ v2-pilot/dev/enabled:false → 🟦 v2/live/enabled:true.
    //   근거: 실발행 「마석 현관문필름 필름으로 되는 경우와 교체가 필요한 경우」 네이버 통합검색 블로그영역 최상단(2주 유지).
    //   door(세션71) 동일 판정 기준.
    //   ★ SOP PATCH-08: enabled:true → 일반 사용자에게도 「선택하기」 개방. 온보딩 화이트리스트 동반 확인 필요.
    //   ★ 배지·게이트만 변경. 엔진 4파일 FREEZE 유지·무수정.
    id: "film", name: "인테리어필름", category: "건설·시공", status: "live", enabled: true, verified: true, version: "v2", closed: true, hasPhysicalStore: false,
  ...
  },
  {
    ...
    id: "window", name: "창호시공", category: "건설·시공", status: "live", enabled: true, verified: true, version: "v2", hasPhysicalStore: false,
  ...
  // ── 실버케어 ────────────────────────────────────────────────
  {
    id: "nursinghome", name: "요양원", category: "실버케어", status: "review", enabled: true, verified: true, version: "v1-new",
    icon: "🏡", summary: "장기요양등급 · 입소자격 · 입소절차 · 비용구조(본인부담금) · 비급여 · 요양원 vs 요양병원 · 치매 돌봄 · 시설 환경 확인 · 면회·외출·외박 · 요양원 선택기준",
    description: "요양원(노인요양시설) 입소자격·장기요양등급·비용구조·선택기준을 보호자 관점 정보형으로 안내합니다.",  // [NURSINGHOME-OPEN 2026-09-22] enabled:true · 관측 병행(review)
  },
  {
