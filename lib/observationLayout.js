...
// ─────────────────────────────────────────────────────────────
// [OPS-RECENT-PUBLISH-TOP-NAV-01 · FIX-A] 상단 current 조건 전달 (2026-09-21)
//   상단 바에 '최근발행'(/admin/publish) 독립 진입점이 생겼다(adminNav.js v1.2).
//   독립 메뉴로 들어왔는데 상단이 '관측' 활성이면 "관측 안에 있다"로 읽혀 진입점 분리가 무의미해진다.
//   → current === '/admin/publish' 일 때만 상단에 '/admin/publish' 를 넘긴다.
//     그 외 관측 계열(auto-observe 등)은 기존 OBS_ROOT 고정 동작 그대로.
//   좌측 레일은 무접촉 — Rail 은 계속 원본 current 를 받는다(수동관측(발행) 활성 유지).
//   AdminLayout · AdminNav · publish.js 무수정. 활성 판정은 여전히 문자열 완전일치.
//
...
//   → FIX-A 에서 '/admin/publish' 1건만 예외. 그 외 하드코딩 원칙 유지.
...
export function ObservationAdminLayout({ current, children, fluid = false }) {
  // OPS-RECENT-PUBLISH-TOP-NAV-01 FIX-A: 상단 활성만 결정한다. 좌측 레일은 원본 current 사용.
  const topCurrent = current === '/admin/publish' ? '/admin/publish' : OBS_ROOT;

  if (fluid) {
    return (
      <AdminLayout current={topCurrent} theme="dark" fluid>
  ...
  return (
    <AdminLayout current={topCurrent} theme="dark">
      <div style={P.row}>
