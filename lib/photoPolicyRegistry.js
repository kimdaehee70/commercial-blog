...
// 요양원(nursinghome) 5슬롯 — NURSINGHOME-PHOTO-POLICY-01 (표시 전용)
//   근거: generateNursinghome.js injectImageSlots 가 getImageAlts 5개 마커를 강제 삽입.
//   목적: 발행 전 체크리스트의 병원 폴백 문구(고민·상담·시술전·시술후·결과) 업종 오염 차단.
//   ★ 라벨은 안내용. 본문 ALT·삽입 위치는 nursinghome-prompts.js / generateNursinghome.js 소유(FREEZE 무접촉).
export const NURSINGHOME_PHOTO_SLOTS = {
  baseCount: 5,
  slots: [
    { key: "exterior", label: "외관",         desc: "건물 전경·입구" },
    { key: "room",     label: "생활실",       desc: "어르신 생활 공간" },
    { key: "dining",   label: "식사공간",     desc: "식당·식사 공간" },
    { key: "lounge",   label: "공용거실",     desc: "공용 거실·휴게 공간" },
    { key: "outdoor",  label: "옥외산책공간", desc: "옥외 산책·정원 공간" },
  ],
};
...
export function getPhotoPolicy(industry) {
  ...
  if (industry === "nursinghome") return NURSINGHOME_PHOTO_SLOTS;
