...
export function regionTitleSuffix(region) {
  const r = String(region || "").trim();
  return r ? `${SHAMAN_TITLE_SEP}${r} ${SHAMAN_TITLE_SERVICE}` : "";
}

export function applyRegionToTitle(title, region) {
  const t = String(title || "").trim();
  const r = String(region || "").trim();
  if (!t || !r) return t;
  if (t.includes(SHAMAN_TITLE_SERVICE)) return t;   // 서비스축 중복 방지
  // 앞부분에 이미 지역이 있으면 지역을 다시 넣지 않는다(중복 삽입 방어)
  return t.includes(r)
    ? `${t}${SHAMAN_TITLE_SEP}${SHAMAN_TITLE_SERVICE}`
    : `${t}${regionTitleSuffix(region)}`;
}

/* 제목 후보 풀 — axis(situation/specialty)에 따라 채워진 완성형 배열 반환
   ★ region 지정 시 지역·서비스 꼬리 길이만큼 앞부분 예산을 줄여서 생성한다. */
export function shamanTitleCandidates({ axis, situationId, specialtyId, specId, region } = {}) {
  const budget = SHAMAN_TITLE_MAXLEN - regionTitleSuffix(region).length;
  if (axis === "specialty") {
...
export function resolveShamanTitle(modelTitle, ctx = {}) {
  ...
  const suffixLen = regionTitleSuffix(region).length;
  const budget = SHAMAN_TITLE_MAXLEN - suffixLen;
  const minB   = Math.max(6, SHAMAN_TITLE_MINLEN - suffixLen);
  const ctxB   = { ...ctx, maxLen: budget, minLen: minB };

  const fails = checkShamanTitle(modelTitle, ctxB);
  if (!fails.length) {
    return { title: applyRegionToTitle(String(modelTitle).trim(), region), source: "model", fails: [] };
  }
  ...
  if (cands.length) {
    ...
    return { title: applyRegionToTitle(cands[pickIdx], region), source: "pattern", fails };
  }

  // 풀까지 비면 상황문 자체를 제목으로 쓴다(검색어는 최소한 살린다)
  const anchor = titleAnchors(ctx)[0] || "";
  return { title: applyRegionToTitle(anchor, region), source: "fallback", fails };
}
