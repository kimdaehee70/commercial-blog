...
export const SHAMAN_META = {
  ...
  label: "상담·역학",
  group: "professional",
  // ★ 세션118 사진축 — 전 분야 3장 통일. SPECIALTIES[].photoCount 개별 지정은 전량 제거했다.
  //   구조: [상황 사진(situation.scenes 기반)] + [상담 공간] + [상담 준비]
  //   외관·건물 입구 제거 — 점집은 외관이 신뢰 정보가 아니다(가정집·혼재 환경). tail 사진 0장.
  photoCount: 3,
  photoRoles: ["상담 공간", "상담 준비"],   // 상황 사진은 엔진이 앞에 붙인다(situation 단위)
...
/* Title Engine V2 — Search Intent (세션104)
   제목의 머리를 SITUATION.name("…할 때")에서 실검색 문장으로 교체한다.
   name·본문·프롬프트·Situation 흐름은 무접촉. 교체 지점은 아래 2함수뿐. */
import {
  hasIntent,
  intentTitlesFor,
  allIntentPhrases,
} from "./shaman-search-intent-data";
...
/* Engine A 제목 후보
   ★ V3(세션104): 꼬리를 SHAMAN_TITLE_POOLS에서 가져오지 않는다.
     Search Intent DB가 '현실 → 운' 전체를 완성해서 반환한다.
       손님이 없어요 · 사업운
       결혼이 늦어져요 · 결혼운이 궁금할 때
       재물운 · 돈이 안 모여요
     이유: '무엇을 물어봐야 하나' 류는 검색되지 않고 업종 의도를 흐린다.
     부수효과: titleNeedsQuestions()가 꺼져 본문 질문 나열이 완화된다(의도된 것).
     DB 미등록 situation은 기존 풀 그대로 사용한다. */
export function situationTitlePatterns(specId, situationId, maxLen) {
  const pool = SHAMAN_TITLE_POOLS[getTitleAxis(specId, situationId)] || [];
  if (!situationId || !hasIntent(situationId)) return pool;

  // ★ 세션114 — maxLen 미지정 시 기존 동작 그대로(프롬프트 경로 무변경).
  const titles = maxLen
    ? intentTitlesFor(situationId, Math.max(pool.length, 3), maxLen)
    : intentTitlesFor(situationId, Math.max(pool.length, 3));
  return titles.length ? titles : pool;
}
...
/* ★ 세션114 — Title Policy V2 Pilot (무속 시범)
   구조: [현실 Search Intent] · [○○운 질문] + [생활권 신점 상담]
     예) 돌아가신 분 꿈 · 조상 때문일까? 구의동 신점 상담
   · Search Intent 앞부분은 훼손하지 않는다. 지역은 뒤 검색축으로만 붙인다.
   · region 은 opts.region 그대로 = A축 산출 생활권 단독. 대표지역 재결합 금지.
   · 지역 확정은 이 파일 한 지점에서만 한다(SoT). 프롬프트·모델에 맡기지 않는다. */
export const SHAMAN_TITLE_SERVICE = "신점 상담";
export const SHAMAN_TITLE_SEP = " - ";   // 앞=콘텐츠 제목 / 뒤=지역 서비스 시각 분리

/* ★ 세션115 — 후방축 3종 풀
   실측 근거: 업계 디렉터리가 "점집"을 상위 탐색 카테고리로, "신점"을 하위 방식어로 쓴다.
   "용한점집"은 업체의 자기평가가 아니라 검색자가 실제로 치는 말이므로 검색축에서만 사용한다.
     · 점집        = 대중 탐색축
     · 신점 상담   = 상담 방식축
     · 용한점집    = 강한 탐색 의도축
   ※ 본문·프롬프트에는 확산시키지 않는다. 광고형 표현 억제 원칙 유지. */
export const SHAMAN_TITLE_SERVICES = ["점집", "신점 상담", "용한점집"];

// 중복 검사는 긴 문자열부터. "용한점집"은 "점집"을 부분문자열로 포함한다.
const SERVICE_BY_LEN = [...SHAMAN_TITLE_SERVICES].sort((a, b) => b.length - a.length);

export function pickTitleService() {
  return SHAMAN_TITLE_SERVICES[Math.floor(Math.random() * SHAMAN_TITLE_SERVICES.length)];
}

// 이미 어떤 서비스어가 제목에 들어 있는지 — 없으면 null
function includedService(t) {
  return SERVICE_BY_LEN.find((s) => String(t).includes(s)) || null;
}

export function regionTitleSuffix(region, service) {
  const r = String(region || "").trim();
  const sv = service || SHAMAN_TITLE_SERVICE;
  return r ? `${SHAMAN_TITLE_SEP}${r} ${sv}` : "";
}

export function applyRegionToTitle(title, region, service) {
  const t = String(title || "").trim();
  const r = String(region || "").trim();
  const sv = service || SHAMAN_TITLE_SERVICE;
  if (!t || !r) return t;
  if (includedService(t)) return t;                 // 서비스축 중복 방지(풀 전체 대상)
  // 앞부분에 이미 지역이 있으면 지역을 다시 넣지 않는다(중복 삽입 방어)
  return t.includes(r)
    ? `${t}${SHAMAN_TITLE_SEP}${sv}`
    : `${t}${regionTitleSuffix(region, sv)}`;
}

/* 제목 후보 풀 — axis(situation/specialty)에 따라 채워진 완성형 배열 반환
   ★ region 지정 시 지역·서비스 꼬리 길이만큼 앞부분 예산을 줄여서 생성한다. */
export function shamanTitleCandidates({ axis, situationId, specialtyId, specId, region, service } = {}) {
  const budget = SHAMAN_TITLE_MAXLEN - regionTitleSuffix(region, service).length;
  if (axis === "specialty") {
  ...
  if (!s) return [];
  const pool = situationTitlePatterns(s.spec || specId, situationId, budget) || [];
...
function titleAnchors({ axis, situationId, specialtyId, specId } = {}) {
  ...
  // ★ V2: Search Intent 문장과 그 토큰도 검색의도 앵커로 인정한다.
  //   이것이 없으면 Intent 기반 제목이 전부 NO_ANCHOR로 탈락해 name 제목으로 되돌아간다.
  const intents = allIntentPhrases(situationId);
  const iWords = intents.flatMap((p) =>
    String(p).split(/[\s,·—-]+/).filter((w) => w.length >= 2)
  );
  return [s.name, ...words, ...intents, ...iWords];
}
...
export function checkShamanTitle(title, ctx = {}) {
  ...
  if (!t) return ["EMPTY"];
  // ★ 세션114 — ctx.maxLen/ctx.minLen 지정 시 그 값을 상하한으로 쓴다.
  //   후보 검사는 지역·서비스 꼬리가 붙기 전(맨 head)에 이루어지므로 하한도 그만큼 낮춘다.
  const maxLen = ctx.maxLen || SHAMAN_TITLE_MAXLEN;
  const minLen = ctx.minLen || SHAMAN_TITLE_MINLEN;
  if (t.length < minLen) fails.push("TOO_SHORT");
  if (t.length > maxLen) fails.push("TOO_LONG");
...
export function resolveShamanTitle(modelTitle, ctx = {}) {
  // ★ 세션114 Title Policy V2 Pilot — 지역·서비스 꼬리 예산을 뺀 값이 앞부분 상한.
  //   최종 제목(꼬리 포함)은 SHAMAN_TITLE_MAXLEN 을 넘지 않는다.
  // ★ 세션115 — 후방축이 가변이 되었으므로 서비스어를 "먼저" 뽑고 그 값으로 예산을 계산한다.
  //   순서가 뒤집히면 선택된 서비스어 길이만큼 상한이 어긋난다.
  const region = ctx.region || "";
  const service = ctx.service || pickTitleService();
  const suffixLen = regionTitleSuffix(region, service).length;
  const budget = SHAMAN_TITLE_MAXLEN - suffixLen;
  const minB   = Math.max(6, SHAMAN_TITLE_MINLEN - suffixLen);
  const ctxB   = { ...ctx, service, maxLen: budget, minLen: minB };

  const fails = checkShamanTitle(modelTitle, ctxB);
  if (!fails.length) {
    return { title: applyRegionToTitle(String(modelTitle).trim(), region, service), source: "model", service, fails: [] };
  }

  const cands = shamanTitleCandidates(ctxB).filter(
    (c) => !checkShamanTitle(c, { ...ctxB, bodyFirst: null }).length
  );
  if (cands.length) {
    const pickIdx = Math.floor(Math.random() * cands.length);
    return { title: applyRegionToTitle(cands[pickIdx], region, service), source: "pattern", service, fails };
  }

  // 풀까지 비면 상황문 자체를 제목으로 쓴다(검색어는 최소한 살린다)
  const anchor = titleAnchors(ctx)[0] || "";
  return { title: applyRegionToTitle(anchor, region, service), source: "fallback", service, fails };
}
