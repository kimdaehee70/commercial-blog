...
// Phase F-3 — Supabase echo (silent fail) 추가, SoT는 파일 유지
//
// 올바른 비율:
//   구조    40% — 흐름만 (1~2개)
//   디테일  40% — 운영 방식 (최소 5개)
...
// Phase F-3: 추출 이벤트별 row 미러링 (SoT는 파일, mirror만)
// 실패해도 절대 throw 하지 않음. mergePatterns 흐름에 영향 X.
async function echoToSupabase({ extracted, keyword, totalScore }) {
  try {
    ...
    const { error } = await supabase.from("pattern_extractions_clinic").insert(row);
    if (error) {
      console.log(`[extractPattern] Supabase echo skip: ${error.message}`);
    } else {
      console.log(`[extractPattern] ✓ Supabase echo: kw="${keyword}" det=${row.details.length}`);
    }
  } catch (e) {
    console.log(`[extractPattern] Supabase echo skip: ${e.message}`);
  }
}

// mergePatterns — patternDB.js 의존 제거, 직접 JSON 파일 저장
function mergePatterns(extracted, keyword, totalScore) {
  try {
    ...
    // Phase F-3: Supabase echo (silent fail) — SoT는 파일, mirror만
    echoToSupabase({ extracted, keyword, totalScore });
...
【출력 — JSON만, 다른 텍스트 없음】
{
  ...
  try {
    ...
    const db = mergePatterns(extracted, keyword, diagResult?.totalScore);
