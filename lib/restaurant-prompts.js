...
const FACT_PORTION = { solo: '한 사람이 한 끼로 먹는 양', shared: '여럿이 나눠 먹는 양' };
const FACT_PACE    = { quick: '간단히 먹는 식사', slow: '천천히 먹는 식사', long: '자리를 오래 두고 먹는 메뉴' };
const FACT_TIMING  = { meal: '끼니 때 전반', morning: '아침·해장 쪽', evening: '저녁·술자리 쪽' };
...
function _commercialFacts(menu, direction = {}) {
  ...
  return {
    ...
    companion,
    soloFit: !companion ? 'unknown' : (/혼자|1인|혼밥/.test(companion) ? 'yes' : 'no'),
    purposeCompanion: pc && pc !== companion ? pc : '',   // [J1] 목적의 동행 = 프레이밍 전용
  };
}

// [FIX-B §4] 독자 판단 흐름 — 섹션별 '판단 단계' 고정  ([FIX-C] getOpeningRule 공유를 위해 모듈 상수로 이동 · 값 무변경)
...
function _readerFlow(section, F) {
  ...
  if (section === 'recommendSituation' && !F.timing && !F.pace) {
    return '검색자의 목적에 맞춰 알아두면 좋은 이용 판단(시간대·식사 페이스는 전제하지 않는다)';
  }
  return READER_FLOW[section] || '앞 섹션의 판단을 이어받는 다음 판단';
}
...
export function getOpeningRule(section, treatment, options = {}) {
  ...
  const companionOpeners = F.soloFit === 'yes'
    ? '"혼자라면 …" · "둘이라면 …" · "여럿이 모인다면 …" · "가족과 함께라면 …"'
    : F.soloFit === 'no'
      ? '"둘이라면 …" · "여럿이 모인다면 …" · "가족과 함께라면 …"'
      : '';
...
${companionOpeners ? `  · 동행: ${companionOpeners}\n` : ''}  · 목적: "처음 찾는다면 …" · "회식 자리를 알아본다면 …" · "간단히 한 끼 해결하려면 …" · "한잔 곁들이려면 …"${timingRule}
...
function buildCommercialRestaurantPrompt(section, treatment, region, options = {}) {
  ...
  const purposeFrame = direction.purposeFrame || '';
  // [FIX-C 보충 R1] PURPOSE는 상황 프레이밍(purposeFrame) 전용. decisionPoint·recommendSituation은
  //   PURPOSE_OVERRIDES 덮어쓰기를 쓰지 않고 MENU_BASE_DIRECTION 원값만 사용(메뉴 FACT 생성·변경 차단).
  const decisionPoint = _baseDir.decisionPoint || '';
  const recommendSit = _baseDir.recommendSituation || '';
  ...
  const bestCompanion = _F.companion;          // base bestCompanion (FACT)
...
- ❌ [v3] 만족 단정 금지: "푸짐하다/양이 많다/가성비 최고/배부르다/또 가고 싶다/추천한다" 등 결론 단정
  → 대신 판단 재료로: "1인분 기준 양", "한 자리 채우기 알맞은 편", "여럿이 나눠 먹기 좋은 양"${bestCompanion ? `(동행 프레이밍은 이 메뉴 결 「${bestCompanion}」 따름)` : ''}
...
- ★ '${menu}'의 메뉴 일반 맛 결(매장마다 다름): ${tasteRef || '(기본)'}. 이 결을 벗어난 재료 임의 추가 금지.
  ⚠ 참고 값에 단정 표현("잡내 없이", "잘 배어", "별도 양념 없이도")이 있어도 그대로 옮기지 말 것
    → 단정 부분은 버리고, 검색자의 판단 문장("~라면 ~가 맞는지 따져 보게 된다")의 근거로만 쓴다.
...
- ★ [FIX-A D1 → FIX-B 보조] 사실 경계 위반의 대표 예(전체는 [사실 경계] 원칙 적용):
  ❌ "주차 공간이 마련되어 있다" / "함께 나오는 반찬" / "매장의 대표 메뉴" → ⭕ ${_parkingProvided ? '"포장 여부는 매장마다 달라 미리 물어보는 편이 낫다"' : '"차로 간다면 주차가 되는지 미리 확인해 두면 편하다"'}
...
- ★★ [재료 3연속 나열 절대 금지 — 실측 회귀 패턴] "뽀얗고 진한 국물 + 부드러운 머릿고기 + 쫄깃한 순대"처럼
  ...
  const common = [getFactBoundary(_provided), infoFrameGuide, getCommercialStyleGuide(), getKwDensityGuide(genericName, region, menu, 'commercial'), visitGuide]
    .filter(Boolean).join('\n');
...
- ★★ 섹션 전체 주어 규칙(필수): '${menu}'를 주어로 한 설명을 나열하지 말 것.
  ...
  · 시작: '검색자의 상황'으로 연다 — ${bestCompanion ? `이 메뉴의 동행 결(「${bestCompanion}」)에 맞는 상황으로.` : '검색자의 목적에 맞는 상황으로(동행 전제 없이).'} ${companionLock}
...
- ❌ 메뉴 정체성·계열 설명으로 시작 금지. 검색자의 끼니·자리 상황으로 연다.
  ${_sf(
    `예) "이번 주말 부모님과 식사할 곳을 찾고 계신가요?"
      ...
      "회식 장소를 알아보고 계신가요?"`,
    `예) "오늘 저녁 친구들과 한잔할 자리를 찾고 계신가요?"
      "퇴근 후 가볍게 모일 곳을 알아보고 계신가요?"
      "회식·모임 장소를 찾고 계신가요?"`,
    `⚠ 동행·시간대 자료 없음 — 혼자·여럿·끼니 때를 특정하지 않는 상황 질문으로 연다.`)}
...
- 앞 섹션의 상황을 이어받아, 그 상황에 ${menu}가 왜 맞는지를 메뉴 일반 특성 1~2개로 짚는다.
  ${_sf(
    `예) "속을 든든히 채워야 하는 날이라면, 밥을 말아 한 번에 끝낼 수 있다는 점이 먼저 걸린다"`,
    `예) "한잔 곁들이며 이야기가 길어지는 자리라면, 가운데 두고 오래 집어 먹을 수 있다는 점이 먼저 걸린다"`,
    `⚠ 동행 결 자료 없음 — 혼자·여럿 어느 쪽도 전제하지 않고, 검색자의 목적에서 맞는 이유를 짚는다.`)}
- 메뉴 일반 맛 결(매장마다 다름): ${tasteRef || '(기본)'} — 근거로만 1문장 이내, 단정 표현은 완화
- ★ 방문 상황 결 참고: ${purposeFrame || '(일반)'}${bestCompanion ? ` / 함께 가기 좋은 동행: ${bestCompanion}` : ''}
...
- ★★ 주문을 정할 때 종합하는 축: ① 양(얼마나 먹을지) ② 취향(진한/담백·간) ③ 식사 목적(끼니/해장/안주) ④ 동행.
  ${bestCompanion
    ? `★ ④ 동행은 이 메뉴의 실제 결(「${bestCompanion}」)로만 잡는다 — '혼자/둘/여럿' 균등 나열 금지.`
    : `★ ④ 동행: 이 메뉴의 동행 결 자료 없음 → 이 축으로 판단 문장을 만들지 않는다.`}
  ${_sf(
...
${(portionFeel || bestCompanion)
  ? [portionFeel && `  · 양의 규모: ${portionFeel}`, bestCompanion && `  · 대표 동행: ${bestCompanion}`].filter(Boolean).join('\n')
...
- ★★ [이유 조합] 축 하나에 '왜 그렇게 따지는가'의 이유를 1개 붙인다.
  · 이유 축(택1): 예산(${direction.priceFeel || '가격 가늠'})${portionFeel ? ` · 양(${portionFeel})` : ''}${paceFeel ? ` · 식사 성격(${paceFeel})` : ''}.
...
- ★★ 앞 섹션 판단 기준의 '결과'. 그 상황이면 보통 어떻게 주문하는지 검색자 기준으로.
  ${_sf(
    ...
    `⚠ 동행 결 자료 없음 — 인원별(혼자/둘/여럿) 주문 방식을 전제하지 말고 검색자의 목적 기준으로만.`)}
...
- ★★ 공깃밥·사이드를 더할지 말지를 상황 기준으로 판단하게 한다.
  ${_sf(
    ...
    `⚠ 동행 결 자료 없음 — 인원을 전제하지 말고 '더할지 말지' 판단 기준만.`)}
- [FIX-C] 반찬·소스·곁들임 구성은 매장마다 다르고 이 글은 모른다 → 반찬·소스·쌈 채소 품목명을 들지 않는다.
  → "나온다 / 제공된다 / 함께 나오는 / 추가로 준비된다" 금지. 판단은 '공깃밥·사이드를 더할지 말지' 수준까지만.
...
- "어떤 상황·목적이면 ${menu}가 잘 맞고, 어떤 경우엔 다른 선택이 나은지" 검색자 기준으로 안내
  ${_sf(
    ...
    `예) "둘이 한 잔 곁들일 자리면 ${menu}가 잘 맞고, 빠르게 끼니만 채울 자리면 다른 선택이 나을 수 있다"(← 이 메뉴 동행 결: ${bestCompanion})`,
    `⚠ 동행 결 자료 없음 — 혼밥·모임 적합 여부를 정하지 않는다. 목적·입맛 기준으로만 맞는 경우/다른 선택을 나눈다.`)}
- ★ 선택 기준 참고: ${decisionPoint || '(메뉴 기본 결로 판단)'}
- ★★ 판단 재료(단정 금지 — 독자가 스스로 가늠하게):
${(portionFeel || paceFeel)
  ? [portionFeel && `  · 양의 규모: ${portionFeel}`, paceFeel && `  · 식사 성격: ${paceFeel}`].filter(Boolean).join('\n')
  : '  · (양·식사 성격 자료 없음 — 이 두 축으로 판단 문장을 만들지 않는다)'}
  → "양이 푸짐/가성비 최고" 단정이 아니라, ${bestCompanion ? `이 메뉴 동행 결(${bestCompanion})에 맞춰 ` : ''}"이 자리에 알맞은지" 판단 정보로${_sf('', ' (혼밥 프레이밍 금지)', ' (동행 결 자료 없음 — 혼밥 적합·부적합을 정하지 않음)')}
- 입맛 호불호 기준 1~2줄 (진한 선호 / 담백 선호 등)
- ★★ [선택지 — A/B/C] 검색자가 고를 수 있는 선택을 '대부분 유지, 일부만 변경'의 결로.
  · A(그대로): ${menu} ${unit}으로 간다 — 기본.
  · B(추가): 유지하되 상황 때문에 하나 더한다 (${_sf('혼자면 공깃밥, 여럿이면 사이드/한 가지 추가', '둘이면 사이드 하나, 여럿이면 한두 가지 추가', '상황에 맞춰 한 가지를 더한다')}).
...
${visitTiming
  ? `- ★ 이 메뉴를 떠올리는 때의 성격(일반론 — ★이 매장의 영업시간 아님): ${visitTiming}`
  : `- ★ 이 메뉴의 시간대 자료가 없다 → 이용 시간대(아침·점심·저녁·밤·끼니 때)를 정하거나 추정하는 문장을 쓰지 않는다.`}
${paceFeel
  ? `- ★ 식사 성격(메뉴 일반): ${paceFeel} — 식사 경과·완료 장면(비운다·일어선다)으로 바꿔 쓰지 않는다.`
  : `- ★ 이 메뉴의 식사 페이스 자료가 없다 → 빠르다/오래 앉는다를 정하지 않는다.`}
  → ❌ "이곳은 ~까지 운영" / "이 동네에서는 새벽까지 먹을 수 있다" / "이른 아침부터 찾는 사람이 많다" 처럼 매장 영업시간·방문량으로 바꿔 쓰기 금지
- ★ 포장·웨이팅·예약은 매장 사실을 단정하지 않는다 — 필요하면 "방문 전 확인" 수준으로만
- ★ 맞는 방문 상황 참고: ${recommendSit || '(메뉴 기본 결로 판단)'}
  (${bestCompanion ? `이 메뉴 동행 결 「${bestCompanion}」에 맞는 상황으로 — ` : ''}${_sf('혼밥/해장/가족 외식/직장인 점심 등', '한 잔 곁들이는 자리/회식/모임/늦은 저녁 등, 혼밥 프레이밍 지양', '검색자의 목적 기준으로만 — 동행·시간대 전제 금지')})
...
- ⭕ ${_parkingProvided ? '' : '"차로 간다면 주차가 되는지 방문 전에 확인해 두면 편하다" / '}"포장 여부는 매장마다 달라 미리 물어보는 편이 낫다"
[조건]
- 방문 목적에 따라 '무엇을 먼저 확인하면 좋은지'만 안내 — ${_sf('혼밥이면 1인 좌석, ', '', '')}모임이면 단체석, ${_parkingProvided ? '차량이면 아래 주차 안내 참고' : '차량이면 주차'}, 집에서 먹으려면 포장 (모두 확인 관점)
${_soloFit === 'no' ? `- ❌ 이 메뉴의 대표 동행 결은 「${bestCompanion}」이다. 1인 좌석을 대표 확인 항목으로 내세우지 말 것.` : ''}
