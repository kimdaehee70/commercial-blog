...
- 참고 값은 메뉴의 일반 특성이다. 매장의 사실로 옮기지 않는다.
  ❌ "매장에서 제공하는 새우젓으로 간을 맞춘다" → ⭕ "새우젓으로 간을 맞춰 먹는 메뉴입니다"
...
const FACT_LABEL_FORM = `  ⚠ 위 라벨은 "~라면 ~를 따져 보게 됩니다" 같은 판단 문장의 근거로만 쓴다.
    ❌ "~로 나온다 / 준비된다 / 제공된다 / 대기 없이 / 많이 찾는다"처럼 조리·제공·방문량 사실로 바꿔 쓰지 않는다.`;
...
const READER_FLOW = {
  ...
  menuScene:          '이 상황에 이 메뉴가 맞는 이유 — 메뉴 일반 특성 1~2개를 상황의 근거로',
  menuComposition:    '주문 전에 무엇을 기준으로 고르는가(판단 축)',
  tasteFeature:       '보통 어떤 식으로 주문하는가(인원·목적별 주문 방식)',
  pairing:            '곁들여 더 주문할지 판단하는 기준',
  decision:           '이 선택이 맞는 경우와 다른 선택이 나은 경우',
  recommendSituation: '언제·어떤 페이스로 찾는 메뉴인가(일반적인 이용 시간대·식사 페이스)',
  storeFeature:       '방문 전에 확인해 두면 좋은 항목',
};
...
- ★★★ 주인공은 '${menu}'(메뉴)가 아니라 '검색자의 상황·목적'이다.
  사람은 재료가 궁금해서가 아니라 자기 상황(혼밥·가족식사·추운 날·배고픈 날)을 해결하려고 검색한다.
...
- ★ [FIX-A D1 → FIX-B 보조] 사실 경계 위반의 대표 예(전체는 [사실 경계] 원칙 적용):
  ❌ "주차 공간이 마련되어 있다" / "함께 나오는 반찬" / "매장의 대표 메뉴" → ⭕ ${_parkingProvided ? '"포장 여부는 매장마다 달라 미리 물어보는 편이 낫습니다"' : '"차로 간다면 주차가 되는지 미리 확인해 두면 편합니다"'}
...
- ★★ 섹션 전체 주어 규칙(필수): '${menu}'를 주어로 한 설명을 나열하지 말 것.
  ...
  · 끝: "이런 상황이면 ${menu}가 선택지에 오릅니다" 식으로 상황↔메뉴를 다시 묶는다. (빈도·인기 주장 금지 — "찾는 사람이 많다" ✗)
...
- 흐름: 상황 질문 → 공감 한 줄 → "이런 상황이면 ${menu} 같은 메뉴가 떠오릅니다" 다리 1줄 (빈도·인기 주장 금지)
...
- 앞 섹션의 상황을 이어받아, 그 상황에 ${menu}가 왜 맞는지를 메뉴 일반 특성 1~2개로 짚는다.
  ${_sf(
    `예) "속을 든든히 채워야 하는 날이라면, 밥을 말아 한 번에 끝낼 수 있다는 점이 먼저 걸립니다"`,
    `예) "한잔 곁들이며 이야기가 길어지는 자리라면, 가운데 두고 오래 집어 먹을 수 있다는 점이 먼저 걸립니다"`,
...
- ★★ 주문을 정할 때 종합하는 축: ① 양(얼마나 먹을지) ② 취향(진한/담백·간) ③ 식사 목적(검색자가 정한 목적 기준) ④ 동행.
  ${bestCompanion
    ? `★ ④ 동행은 이 메뉴의 실제 결(「${bestCompanion}」)로만 잡는다 — '혼자/둘/여럿' 균등 나열 금지.`
    : `★ ④ 동행: 이 메뉴의 동행 결 자료 없음 → 이 축으로 판단 문장을 만들지 않는다.`}
  ${_sf(
    `예) "혼자 든든히 한 끼면 양과 속 편함을 먼저 보고, 여럿이 나눌 자리면 나눠 먹기 좋은지를 먼저 따지게 됩니다"`,
    `예) "둘이 한 잔 곁들일 자리면 안주로 알맞은지를 먼저 보고, 여럿이 모이면 나눠 먹기 좋은 양인지를 따지게 됩니다"(혼밥 예시 금지)`,
    '')}
...
- ★★ [판단 흐름 — 한 줄로 뭉개지 말 것] 아래 4비트를 문장으로 펼친다:
  ...
  예) "속이 부대끼는 날이면 담백한 쪽으로, 개운하게 풀고 싶은 날이면 얼큰한 쪽으로 마음이 기웁니다."
...
- ★★ 앞 섹션 판단 기준의 '결과'. 그 상황이면 보통 어떻게 주문하는지 검색자 기준으로.
  ${_sf(
    `예) "혼자라면 ${menu} ${unit}으로 끝내면 되고, 둘이라면 한 가지를 더해 나눠 먹는 식으로 고르면 됩니다"`,
    `예) "둘이라면 ${menu} ${unit}에 한 가지를 더해 나눠 먹고, 여럿이라면 몇 가지를 골라 가운데 두고 덜어 먹는 식으로 고르면 됩니다"`,
    `⚠ 동행 결 자료 없음 — 인원별(혼자/둘/여럿) 주문 방식을 전제하지 말고 검색자의 목적 기준으로만.`)}
- ❌ 빈도·판매량 주장 금지("가장 많이 나간다", "대부분 시킨다").
- ★ 먹는 방식 참고(메뉴 일반·현재형): [${_eatPool}] — "보통 ~해서 먹습니다"로만. 특정 손님의 동작·반응 서술 금지.
- ★ 맛 결(메뉴 일반: ${tasteRef || '기본'})은 "왜 그 주문 방식이 맞는지" 근거 1문장으로만. 맛을 독립 묘사하지 말 것.
  예) ⭕ "진한 편이라 밥과 함께 시키면 한 끼로 마무리하기 편합니다"
...
- ★★ 공깃밥·사이드를 더할지 말지를 상황 기준으로 판단하게 한다.
  ${_sf(
    `예) "혼자 한 끼로 끝낼 생각이라면 공깃밥을 더할지 정도만 보면 되고, 여럿이라면 사이드를 한둘 더할지 따져 보게 됩니다"`,
    `예) "둘이라면 사이드 하나를 더할지, 여럿이라면 한두 가지를 가운데 둘지 따져 보게 됩니다"`,
...
- "어떤 상황·목적이면 ${menu}가 잘 맞고, 어떤 경우엔 다른 선택이 나은지" 검색자 기준으로 안내
  ${_sf(
    `예) "혼자 한 끼를 해결하려면 ${menu}가 무난하고, 여럿이 이것저것 나눠 먹을 자리면 다른 선택이 나을 수 있습니다"`,
    `예) "둘이 한 잔 곁들일 자리면 ${menu}가 잘 맞고, 빠르게 끼니만 채울 자리면 다른 선택이 나을 수 있습니다"(← 이 메뉴 동행 결: ${bestCompanion})`,
...
- ★ 방문 상황: ${bestCompanion ? `이 메뉴 동행 결 「${bestCompanion}」에 맞는 상황으로 — ` : ''}${_sf('혼자 먹는 한 끼·일행과의 식사 등', '위 동행 결에 맞는 자리 — 혼밥 프레이밍 지양', '검색자의 목적 기준으로만 — 동행·시간대 전제 금지')}
...
- ⭕ ${_parkingProvided ? '' : '"차로 간다면 주차가 되는지 방문 전에 확인해 두면 편합니다" / '}"포장 여부는 매장마다 달라 미리 물어보는 편이 낫습니다"
[조건]
