...
→ 평가 단정형·완성 선언형·감상 종결형으로 문장을 맺지 않는다(좋다·적합·제격·만족·완성·특별 계열 전부 해당).
...
const FACT_COMPANION = { solo: '혼자', pair: '둘 이상', group: '여럿' };
...
const FACT_TASTE = {
  brothRich: '진한 국물 계열', brothClear: '맑은 국물 계열', spicy: '매콤한 계열', mild: '담백한 계열',
  chewy: '쫄깃한 식감', tender: '부드러운 식감', crisp: '바삭한 식감', savory: '고소한 결',
};
...
function getAxisContract(F, opts = {}) {
  ...
  (opts.eat ? known : unknown).push('먹는 방식');
  (opts.taste ? known : unknown).push('맛 결');
  // commercial FACT 입력 제외(J3) → 상시 UNKNOWN
  unknown.push('곁들임·소스·사이드·추가 구성', '좌석·시설');
  return `
[★★★ 자료 축 계약 — 사실 경계와 같은 최상위 · 위반 시 전면 실패]
- 자료 있는 축: ${known.join(' · ') || '(없음)'}
- 자료 없는 축: ${unknown.join(' · ')}
- 자료 없는 축은 '맞다/안 맞다를 정하지 않는다'로 끝나지 않는다. 그 축에 대해 아래 여섯 가지를 모두 생성하지 않는다:
  ① 품목명 ② 수량·인분·가짓수 ③ 제공·구성 사실 ④ 고객의 행동 ⑤ 먹는 방식 ⑥ 그 축을 전제한 조건절
- 자료 없는 축은 판단 흐름에서 뺀다. 완곡어·일반론·"보통 ~합니다"로 돌려 쓰는 것도 같은 위반이다.
- 자료 있는 축도 주어진 라벨의 범위까지만 쓴다. 라벨보다 구체적인 값을 만들지 않는다.`;
}
...
- ❌ '맛있다/맛있어요' 단정 금지
- ❌ [v3] 만족 단정 금지: 양·가성비·포만·재방문·추천 유형의 결론 단정
  → 대신 판단 재료로: ${portionFeel ? `주어진 양 라벨(「${portionFeel}」) 범위 안에서만` : '양 자료 없음 — 양·인분·나눠 먹기를 쓰지 않는다'}${bestCompanion ? ` / 동행은 라벨(「${bestCompanion}」) 범위 안에서만` : ' / 동행 자료 없음 — 동행 전제 금지'}
...
- ★★ [재료 3연속 나열 절대 금지 — 실측 회귀 패턴] 재료·식감 형용사를 셋 이상 잇는 서술은 백과사전 회귀다.
  ...
  // [STEP6-FIX-E X-8] 구성(부위·양념·소스·곁들임)·동행을 전제하는 항목 제거. 미판별(plate)은 UNKNOWN → 미주입.
  const _EAT_ACTIONS = {
    handheld_bone:     '손으로 집어 먹는다 / 뼈를 발라 가며 먹는다',
    handheld_boneless: '젓가락으로 한 점씩 집어 먹는다',
    grill:             '불판에 올려 익혀 먹는다 / 익힘 정도를 보며 뒤집는다',
    soup:              '국물을 떠 먹는다 / 건더기를 건져 먹는다',
    plate:             '',
  };
  const _eatPool = _EAT_ACTIONS[_eatStyle] || '';
  // [STEP6-FIX-E U-1] 자료 축 계약 — 섹션 조립 전 최상위로 주입
  const axisContract = getAxisContract(_F, { eat: !!_eatPool, taste: !!tasteRef });
...
[★★ 동행 제약 — 위반 시 실패]
- 이 메뉴의 동행 라벨: 「${bestCompanion}」. 본문 전체의 동행 묘사는 이 라벨 범위 안에서만 생성한다.
  라벨보다 구체적인 인원·자리 성격(몇 인·무슨 모임)을 만들지 않는다.
${_sf(
...
${tasteRef
  ? `- 메뉴 일반 맛 결(라벨 · 매장마다 다름): ${tasteRef} — 근거로만 1문장 이내. 라벨 밖의 맛·재료·구성을 덧붙이지 않는다.`
  : `- 맛 결 자료 없음 — 맛·식감·재료를 서술하지 않는다.`}
- ★ 방문 상황 결 참고: ${purposeFrame || '(일반)'}${bestCompanion ? ` / 동행 라벨: ${bestCompanion}(이 범위까지만)` : ' / 동행 자료 없음 — 동행 전제 금지'}
...
- ★★ 주문을 정할 때 종합하는 축(자료 있는 것만 쓴다): ${[portionFeel && '① 양', tasteRef && '② 취향(주어진 맛 결 범위 안)', '③ 식사 목적(검색자가 정한 목적)', bestCompanion && '④ 동행'].filter(Boolean).join(' · ')}
  ${bestCompanion
    ? `★ ④ 동행은 라벨(「${bestCompanion}」) 범위로만 잡는다 — 인원 균등 나열 금지.`
...
${_eatPool
  ? `- ★ 먹는 방식 참고(메뉴 일반·현재형): [${_eatPool}] — "보통 ~해서 먹습니다"로만. 목록에 없는 동작·구성(부위·양념·소스·곁들임)을 덧붙이지 않는다.`
  : `- ★ 먹는 방식 자료 없음 — 먹는 동작·구성을 서술하지 않는다.`}
- ❌ 특정 손님의 동작·반응 서술 금지.
${tasteRef
  ? `- ★ 맛 결(라벨: ${tasteRef})은 "왜 그 주문 방식이 맞는지" 근거 1문장으로만. 라벨 밖의 맛·재료·구성 금지.`
  : `- ★ 맛 결 자료 없음 — 맛·식감·재료를 서술하지 않는다.`}
...
- 쓸 수 있는 것은 두 가지뿐이다:
  ...
  ② 자료 있는 축${portionFeel ? `(양 라벨: ${portionFeel})` : '(양 자료 없음 — 양으로 판단 문장을 만들지 않는다)'}과 검색자의 목적
...
- "어떤 상황·목적이면 ${menu}가 잘 맞고, 어떤 경우엔 다른 선택이 나은지" 검색자 기준으로 안내
  ${_sf(
    ...
    `· 동행 자료 없음 — 1인·모임 적합 여부를 정하지 않는다. 목적·입맛 기준으로만 2갈래를 나눈다.`)}
...
- ❌ 회고 감상형(선택 결과 단정·만족·재방문) 금지 — 아직 아무도 방문하지 않았다.
- 분량: 300~400자`,
...
- ★ 방문 상황: ${bestCompanion ? `동행 라벨 「${bestCompanion}」 범위 안에서만 — ` : ''}${_sf('라벨 범위 안의 자리', '라벨 범위 안의 자리 — 1인 프레이밍 지양', '검색자의 목적 기준으로만 — 동행·시간대 전제 금지')}
