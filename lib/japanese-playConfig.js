// ============================================================
// lib/japanese-playConfig.js — 일식 FLOW_ENGINE 구조 (Japanese Engine 독립)
...
// ─────────────────────────────────────────────────────────
export const JAPANESE_PHOTO_SPOTS = {
...
export const JAPANESE_FLOW_ENGINE = {
  ...
  // 차단 키워드 (japanese-data.js의 JAPANESE_BLOCK_MAP과 일치)
  blockKeywords: [
    // 의료 침투 방지
    ...
    // ★ 한식 narrative 침투 차단 (Japanese 독립 ecosystem — Naver §3 전략2)
  ...
  extraBlocks: [
    ...
    'BEST_TIME',          // 방문 추천 시간대
    'MENU_COMBINATION',   // 메뉴 조합 추천 (라멘+가라아게+밥 등)
...
// ============================================================
// [v2 / 2026-06-25] JAPANESE_COMMERCIAL_FLOW_ENGINE — 8섹션 메뉴 중심 정보형
// ⚠ 기존 JAPANESE_FLOW_ENGINE(6섹션 personal)은 무수정 보존 — 롤백 안전
...
// ============================================================
export const JAPANESE_COMMERCIAL_FLOW_ENGINE = {
  industry: 'japanese',
  mode: 'commercial',
  ...
  sections: [
    ...
    },
    {
      ...
      label: '메뉴구성',
      order: 3,
      description: '한 그릇/한 접시의 구성 요소 — 들어가는 재료·비율·양 가늠·기본 구성. 관찰 가능한 사실 위주 (예: 차슈 비중·생선 부위 구성)',
      required: true,
      minLength: 280,
      maxLength: 380,
    ...
    },
    {
      ...
      label: '맛특징(관찰형)',
      order: 4,
      description: "관찰 가능한 맛 특징만. '맛있다' 금지 → '육수가 진한 편', '튀김옷이 두꺼운 편', '간이 센 편/슴슴한 편' 등 선택에 도움되는 기술. 1인칭 체험 금지",
      required: true,
      minLength: 300,
      maxLength: 400,
      photo: '음식 표면·단면 디테일',
    },
    {
      ...
      label: '곁들임',
      order: 5,
      description: '함께 먹으면 좋은 구성 — 곁들임·추가 메뉴·조합(가라아게·미소시루·사이드). 정보형. 강요·광고 표현 금지',
      required: true,
      minLength: 180,
      maxLength: 260,
      photo: '곁들임·반찬·조합 컷',
  ...
  blockKeywords: [
    ...JAPANESE_FLOW_ENGINE.blockKeywords,
  ],
...
// ★ 일식 4계열 분리 정의 — taste 묘사 결을 계열별로 고정 (계열 교차 표현 차단)
//   스시(신선도·결) / 면(육수·면) / 튀김(튀김옷·바삭) / 덮밥(밥+토핑)
//   ⚠ taste 결 충돌 방지가 4계열 분리의 핵심 목적 (JAPANESE_CROSS_CAT_BLOCK과 짝)
// ─────────────────────────────────────────────────────────
export const JAPANESE_CATEGORY_OVERRIDES = {
  '스시': {
    // 스시 — 신선도·결 중심. 국물 끓임·튀김옷 표현 금지
    taste: {
      description: '생선 한 점의 결·두께·온도, 샤리의 풀림, 간장·고추냉이 곁들임의 한 입 체감. 국물 끓임·튀김옷 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 초밥/회 구성 + 곁들임(간장·고추냉이·절임 생강) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
  '면': {
    // 면 — 육수·면 식감·온도 중심. 회 신선도·튀김옷 표현 금지
    taste: {
      description: '육수의 농도·온도, 면의 쫄깃함·굵기, 토핑(차슈·유부·계란)의 한 입 체감. 회 신선도·튀김옷 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 면/육수/토핑 구성 + 곁들임(차슈·유부·파·츠유) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
  '튀김': {
    // 튀김 — 튀김옷·바삭함·소스 중심. 회 결·국물 끓임 표현 금지
    taste: {
      description: '튀김옷의 두께·바삭함, 속살의 촉촉함, 소스(돈카츠 소스·텐츠유·레몬)와의 조합 한 입 체감. 회 결·국물 끓임 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 튀김 구성 + 곁들임(양배추·텐츠유·레몬·된장국) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
  '덮밥': {
    // 덮밥 — 밥+토핑 조합·소스 중심. 회 결·면 식감 표현 금지
    taste: {
      description: '밥에 스며든 소스의 간, 토핑(소고기·돈카츠·계란·카레)과 밥의 조합 한 입 체감. 회 결·면 식감 표현 금지',
      minLength: 300,
      maxLength: 400,
    },
    order: {
      description: '메뉴 고르기 + 덮밥 토핑/소스 구성 + 곁들임(단무지·된장국·후쿠진즈케) + 양 가늠 (숫자 가격 X)',
      minLength: 260,
      maxLength: 360,
    },
  },
...
export function getJapaneseSections(category, flowBias, mode) {
  ...
  if (mode === 'commercial') {
    return JAPANESE_COMMERCIAL_FLOW_ENGINE.sections.map(sec => ({ ...sec }));
  }

  // personal 모드 (기존 동작 — 무변경)
  const base = JAPANESE_FLOW_ENGINE.sections;
  const catOvr = JAPANESE_CATEGORY_OVERRIDES[category] || {};
...
export function getPhotoSpotForSection(sectionKey) {
  return JAPANESE_PHOTO_SPOTS[sectionKey] || null;
}
