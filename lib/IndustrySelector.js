...
//   🟩 초록 네모 = done:true — 검토 완료/CLOSE(선장 표시축). 최우선 판정.
//     ★ 세션146 — 색 변경 🟦→🟩. 2순위(verified+v2, 🟦)와 색 충돌 해소.
//     ★ 세션118 — done은 엔진 버전·검증 상태와 독립된 "작업 완료" 표시 전용 축이다.
//       version:"v2"를 가짜로 넣어 색을 맞추지 않기 위해 별도 필드로 분리했다.
function statusMark(status, verified, version, done) {
  if (done)                              return { bg: "#16a34a", bd: "#15803d", op: 1, radius: 3 };  // 🟩 검토 완료/CLOSE
                                  ...
                                  <div role="button" tabIndex={0}
                                    ...
                                    <span style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, overflow: "hidden", flex: 1, flexWrap: "nowrap" }}>
                                      <span style={{ width: 10, height: 10, flexShrink: 0,
                                        borderRadius: statusMark(sub.status, sub.verified, sub.version, sub.done).radius,
                                        background: statusMark(sub.status, sub.verified, sub.version, sub.done).bg,
                                        border: `2px solid ${statusMark(sub.status, sub.verified, sub.version, sub.done).bd}`,
                                        opacity: statusMark(sub.status, sub.verified, sub.version, sub.done).op }} />
                        ...
                        <div role="button" tabIndex={0}
                          ...
                          <span style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, overflow: "hidden", flex: 1, flexWrap: "nowrap" }}>
                            <span style={{
                              ...
                              borderRadius: statusMark(it.status, it.verified, it.version, it.done).radius,
                              background: statusMark(it.status, it.verified, it.version, it.done).bg,
                              border: `2px solid ${statusMark(it.status, it.verified, it.version, it.done).bd}`,
                              opacity: statusMark(it.status, it.verified, it.version, it.done).op,
                            }} />
