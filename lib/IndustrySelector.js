...
function statusMark(status, verified, version) {
      ...
      {activeGroup ? (
        <div style={grid}>
          {activeGroup.items.map(it => (
            <div key={it.key} role="button" tabIndex={0} style={card(value === it.key)}
                                  ...
                                  <div role="button" tabIndex={0}
                                    ...
                                    <span style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, overflow: "hidden", flex: 1, flexWrap: "nowrap" }}>
                                      <span style={{ width: 10, height: 10, flexShrink: 0,
                                        borderRadius: statusMark(sub.status, sub.verified, sub.version).radius,
                                        background: statusMark(sub.status, sub.verified, sub.version).bg,
                                        border: `2px solid ${statusMark(sub.status, sub.verified, sub.version).bd}`,
                                        opacity: statusMark(sub.status, sub.verified, sub.version).op }} />
                        ...
                        <div role="button" tabIndex={0}
                          ...
                          <span style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, overflow: "hidden", flex: 1, flexWrap: "nowrap" }}>
                            <span style={{
                              ...
                              borderRadius: statusMark(it.status, it.verified, it.version).radius,
                              background: statusMark(it.status, it.verified, it.version).bg,
                              border: `2px solid ${statusMark(it.status, it.verified, it.version).bd}`,
                              opacity: statusMark(it.status, it.verified, it.version).op,
                            }} />
