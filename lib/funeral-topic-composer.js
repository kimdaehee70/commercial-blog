...
export function composeTopicPlan(hallFacts) {
  ...
  const plan = {
    ...
    themeIds,
    region,
    // [BODY-PRACTICE-02] 공급량 축소 6/5 → 3/2. 자산 무수정.
    //   「사실의 수가 곧 분량」 계약(비중 ②) 때문에 최대 11 fact 가 BODY 를 점유했다.
    //   법령을 지우는 게 아니라 공간 점유를 낮춰 장례 진행층이 들어설 자리를 만든다.
    s2: { key: s2.key, facts: s2.facts.slice(0, 3) },
    s3: s3 ? { key: s3.key, linkType: s3.linkType, facts: s3.facts.slice(0, 2) } : null,
...
//     [실용 안내층] = 프롬프트에 없다. S4-DET-01 이후 후처리 결정론 삽입 (LLM 무접촉)
//     [일반 정보층] = 문장을 그대로 쓴다     (LLM 이 재작성 금지)
//     섞이면 LLM 이 일반 정보 문장까지 다시 쓴다.
// ════════════════════════════════════════════════════════════════════
...
- \u2605 열거를 조건으로 바꾸지 않는다. 사실이 대상을 여러 개 나열하면 나열로 쓰고,
  ...
  const s2 = _renderTopicBlock("[주제 정보층 — 첫 문장을 읽은 사람이 아래 사실 하나를 이미 알게 되어야 한다. 사실을 예고하는 문장으로 시작하지 않는다]", plan.s2.facts, _TOPIC_CONTRACT);
  const s3 = plan.s3
    ? `\n${_renderTopicBlock("[보조 주제 — 위 주제 다음에 이어 쓴다]", plan.s3.facts)}`
    : "";
  // [S4-DET-01] 실용 안내층(S4) 블록 삭제 — LLM 에 공급하지 않는다.
  //   ★ 명령 추가가 아니라 재료 회수다. 4회 시도(어미 제거·예외조항 삭제·허용목록 축소·원문 보존 명문화)
  //     끝에 PASS 했던 계약이 재회귀했다(2026-08-15). 프롬프트로는 재작성을 막지 못한다.
  //     문장을 보여주지 않으면 재작성 경로 자체가 없어진다. 원문은 generateFuneral 후처리에서
  //     SafeDrop 이후 결정론적으로 삽입한다(가공·재정렬 0).
  //   ★ 함께 삭제된 것: 「그대로 옮겨 쓴다」 계약 2줄 · 「순서 배열만」 · 「새로 만들지 않는다」 ·
  //     「하나도 빼지 않는다」. 계약이 지키던 대상이 프롬프트에서 사라졌으므로 계약도 불필요하다.
...
- 이 글의 재료는 아래 세 블록이 전부다. 블록 밖의 사실·절차·서류·기관·기한을 쓰지 않는다.
  단, 장례 진행의 통상적인 순서와 장례지도사의 통상 업무 서술은 이 금지의 대상이 아니다.
  특정 시설·특정 사례·특정 금액을 말하지 않는 일반 서술에 한한다.
...
② 주제: [주제 정보층]의 사실을 서술한다. ①의 값을 다시 꺼내지 않는다.${plan.s3 ? `
③ 보조 주제: [보조 주제]의 사실을 ②에 이어 서술한다. ②와 같은 사실을 반복하지 않는다.` : ``}
${plan.s3 ? `④` : `③`} 장례 진행: 장례가 시작된 뒤 실제로 지나가는 시간 순서를 따라 충분히 설명한다.
   안치·빈소, 입관 준비, 발인 일정, 화장 예약과 이동, 장지·안치 등 해당하는 과정이 이어지는 순서에 따라 쓴다.
   각 과정에서 가족이 결정하는 일과 장례지도사가 일정·예약·준비사항 등을 확인하고 다음 절차를 맞추는 일을 함께 설명한다.
   \u2605 여러 장례 과정을 한두 문장으로 압축하지 않는다. 이 글에서 가장 길게 쓰는 구간이다.
   \u2605 실제로 수행한 장례·유가족·고인·금액을 만들지 않는다. 통상 업무의 서술이다.
   \u2605 이 시설에 없는 시설·서비스를 있다고 쓰지 않는다.
   \u2605 장례 진행 서술에 이 장례식장의 이름을 주어로 쓰지 않는다. 통상적인 장례 절차의 서술이지
     특정 장례식장이 제공하는 서비스의 서술이 아니다.
${plan.s3 ? `⑤` : `④`} 실용 안내: [일반 정보층]에 제공된 문장을 쓴다.
...
- ②${plan.s3 ? `③` : ``}(주제) = 제공된 사실의 수가 곧 분량이다. 사실을 늘려 쓰지 않는다.
- ${plan.s3 ? `④` : `③`}(장례 진행) = 독자가 읽는 중심 구간이다.
- 나머지 = 짧게.
