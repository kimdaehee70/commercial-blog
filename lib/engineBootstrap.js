...
import handleNursinghome from "../pages/api/generateNursinghome"; // ← 요양원(노인요양시설) (신규, 비의료·정보형 / 복사베이스 daycare·단일호출 / 10 CAT Purpose Spine · FORBIDDEN/FACT_GATE 2트랙)
...
//   ★ Purpose Spine — 10 CAT 각각 독립된 PURPOSE(question·solve·decide·nextStep) 보유.
//     동일 소개문에서 키워드만 교체한 생성은 QC purposeMiss 로 FAIL 판정.
//   ★ 2트랙 — FORBIDDEN(절대금지) / FACT_GATE(검증입력 시에만 사용). 두 축 합산 금지.
//     시설 고유사실 미입력 → 문장 생략. "입소 가능/대기 없음"·"2.1:1" 생성 금지.
register("nursinghome", handleNursinghome);   // ← 요양원(노인요양시설) (신규, 정보형·비의료·요양원화자·단일호출 / 관측 전)
