...
export async function executeBillingIssue({
  ...
  // ── [BILLING-CHARGE-CUSTOMER-REQUIRED-01] 청구 고객정보 ──
  //   ★ 실측 거절: customer.name violated the rule REQUIRED
  //                customer.phoneNumber violated the rule REQUIRED  (code=INVALID_REQUEST)
  //     KG이니시스는 빌링키 "발급"뿐 아니라 빌링키 "청구"에서도 이름·전화번호를 요구한다.
  //     발급만 대응한 BILLINGKEY-CUSTOMER-REQUIRED-01 은 이 경로를 덮지 못했다.
  //   ★ SoT = store_profiles. 발급 단계(PC: /api/me/store, QR: issue-params)와 같은 값을 쓴다.
  //     여기서 값을 새로 만들지 않는다(fallback 금지). 없으면 청구하지 않고 끊는다.
  //   ★ 실측: PortOne Customer 타입의 fullName 은 「구매자 전체 이름」이며 카드 명의 일치를
  //     요구하는 인증 필드가 아니다(인증 조건이 있는 birthYear 등과 달리 조건 서술이 없다).
  //     따라서 상호명이 실명과 달라도 거절 사유가 아니다 — STORE-NAME-NOT-REAL-NAME-01 판정 승계.
  const { data: stProfile, error: stErr } = await supabase
    .from('store_profiles')
    .select('store_name, phone')
    .eq('account_id', account.id)
    .maybeSingle();

  const custName  = stProfile?.store_name || '';
  const custPhone = stProfile?.phone || '';

  if (stErr || !custName || !custPhone) {
    console.error('[issue-billing-key] customer info missing', account.id, stErr);
    return {
      status: 400,
      body: {
        ok:      false,
        error:   'CUSTOMER_INFO_REQUIRED',
        charged: false,
        message: '업체정보에서 상호명과 연락처를 먼저 등록해 주세요.',
      },
    };
  }
  ...
  const charge = await chargeBillingKey({
    ...
    customer: {
      // ★ 브라우저 SDK 와 서버 REST 는 이름 필드의 키·형태가 다르다. 교차 적용 금지.
      //   실측 출처: @portone/server-sdk dist/generated/common/CustomerInput.d.ts
      //     CustomerInput.name?: CustomerNameInput
      //     CustomerNameInput = { full?: string; separated?: {...} }  「둘 중 하나만 입력」
      //   발급(브라우저)은 customer.fullName: string,
      //   청구(서버 REST)는 customer.name.full: string 이다.
      //   fullName 을 그대로 보내면 스키마에 없는 키라 무시되고 name REQUIRED 위반이 된다.
      //   phoneNumber · email 은 양쪽 동일한 평문 필드라 형태 변환이 필요 없다.
      name:        { full: custName },
