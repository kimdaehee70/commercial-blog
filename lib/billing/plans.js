...
//
// [BILLING-PLANS-DESC-MIRROR-DRIFT-01] description 도 DB 미러 대상이다.
//   실측: DB basic='소상공인 표준. 월 30건. 초과분은 건당 과금.'
//        fallback='꾸준한 운영. 월 30건(하루 1건).' — 두 값이 어긋난 채
//        「DB plans와 1:1 미러」라고만 적혀 있었다. 어긋나면 DB 가 정본.
//   [KG-QUOTA-POLICY-PATCH-01] 상품정책 A 반영 — 일별 한도 표현·초과 과금 표현 제거.
//     description 은 /api/billing/plans 를 통해 요금제 화면에 그대로 출력된다.
//     여기에 「하루 N건」·「초과 단가」가 남으면 제공하지 않는 정책이 화면에 노출된다.
//   ★ quota / price_krw / overage_per_post_krw / is_active / sort_order 무접촉.
...
export const PLANS = {
  free: {
    ...
    description: '가입 후 최초 3건 무료 체험.',  // [FREE-LIFETIME-TRIAL-3-01] DB plans.free.description 과 동시 갱신 필수
    is_active: true,
    sort_order: 1,
  },
  basic: {
  ...
  standard: {
    ...
    description: '적극적인 운영. 이용기간 60건. 발행 관리 + 월간 계획.',
    is_active: true,
    sort_order: 3,
  },
  pro: {
  ...
  enterprise: {
    ...
    description: '최고 성능. 이용기간 150건. 우선 생성 큐.',
    is_active: true,
    sort_order: 5,
  },
};
