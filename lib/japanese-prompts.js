// =============================================
// lib/japanese-prompts.js
// 일식(일본요리) 프롬프트 빌더 v1.0 — Japanese Engine 독립
//
// 기반: restaurant-prompts.js 구조 동형 (Restaurant 계열 엔진)
//   · 함수명 japanese화, import = ./japanese-data.js
//   · commercial(메뉴 정보형) 기본. personal 빌더는 보존(restaurant 동형)
//   · 한식·중식 전용 예시(들깨·새우젓·춘장·짜장)는 일식 톤으로 중립화
//   · cat 4계열(스시·면·튀김·덮밥) — 계열 무관 공통 빌더 (계열별 결은 data.tasteCore 주입)
...
import { buildDirection } from './japanese-data.js';

// ============================================================
// 0. DIRECTION 헬퍼 — 데이터 조합으로 동적 생성
// ============================================================
export function getJapaneseDirection(treatment, situation, purpose) {
  if (!treatment) {
...
// 1. 메인 빌더 (mode 분기)
// ============================================================
export function buildJapanesePrompt(section, treatment, region, options = {}) {
  const { mode = 'personal', situation = '', purpose = '' } = options;
  if (mode === 'commercial') {
    return buildCommercialJapanesePrompt(section, treatment, region, options);
  }
  return buildPersonalJapanesePrompt(section, treatment, region, options);
}

// ── personal 모드 ──
function buildPersonalJapanesePrompt(section, treatment, region, options = {}) {
...
${direction.extraDetail ? `- 추가 디테일: ${direction.extraDetail}` : ''}`;
  ...
  switch (section) {
    ...
    default: throw new Error(`[japanese-prompts] 알 수 없는 섹션: ${section}`);
  }
}
...
- 간장·고추냉이·텐츠유 곁들임 같은 본인의 행동 1~2회 포함
...
function buildCommercialJapanesePrompt(section, treatment, region, options = {}) {
  const { situation = '', purpose = '' } = options;
  const { name, cat = '일식', menu = treatment.menuRef || '', compareWith = '' } = treatment;
  const direction = getJapaneseDirection(treatment, situation, purpose);
  const genericName = direction.genericName || name || '이 식당';
  const unit = direction.servingUnit || '한 그릇';        // ★ 메뉴별 단위 (초밥·돈카츠=한 접시, 라멘·우동·덮밥=한 그릇 등)
...
[조건]
- 곁들임·추가 메뉴·조합을 정보형으로 (예: 절임 생강·된장국, 면에 곁들이는 튀김, 밥·미소시루 조합 ${sidedishes || ''})
...
// ============================================================
export function getJapaneseImageAlts(treatment, region, options = {}) {
  const { situation = '', purpose = '' } = options;
  const menu = treatment.menu || treatment.menuRef || '';
